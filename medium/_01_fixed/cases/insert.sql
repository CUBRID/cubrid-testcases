...
select * from joe.product_c p order by 1;
...
select shipment_id, product.product_code, quantity, to_location
 ...
 where product.product_code = 3 order by 1;
rollback work;
insert into joe.inventory_c ( product) values ( insert into joe.product_c ( product_code ) values ( 33 ) );
insert into joe.inventory_c ( product) values ( select p from joe.product_c p where product_code = 3 );
insert into joe.inventory_c ( product) values ( ( select p from joe.product_c p where product_code = 3 ) );
insert into joe.inventory_c ( product) values (( insert into joe.product_c ( product_code ) values ( 33 ) ));
rollback work;
