...
)   update t inner join cte1 on col2=a set col5='price low' where col3<b;
...
(
    select col2,avg(col3) from t group by col2
) update t inner join cte1 inner join t2 on col2=a and i=col1 set j=a,k='low' where col3<b;


with cte1(a,b) as
(
    select col2,avg(col3) from t group by col2
)   update t inner join cte1 inner join t2 on col2=a and i=col1 set j=a,k='high' where col3>b;
...
(
    select col2,avg(col3) from t group by col2
)   update t inner join cte1 inner join t2 on col2=a and i=col1 set j=a,k='equal' where col3=b;
