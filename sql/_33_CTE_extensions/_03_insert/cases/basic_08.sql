...
insert into foo
WITH a AS (SELECT * FROM t1) select a.* foo NATURAL JOIN a NATURAL JOIN t2 NATURAL JOIN t3  where a.a=t2.a and rownum<2;
select * from foo;

insert into foo
WITH a AS (SELECT * FROM t1) select a2.* foo FROM foo NATURAL JOIN a NATURAL JOIN t2 NATURAL JOIN t3 where a.a=t2.a;
select * from foo;
...
insert into foo
with t2 as (select * from v1) select a.* from v1 where a in (select a from t2 );
select * from foo;
