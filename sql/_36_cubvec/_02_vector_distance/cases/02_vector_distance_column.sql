...
INSERT INTO test_non_vector VALUES (1, 'Alice');
SELECT VECTOR_DISTANCE(name, name) FROM test_non_vector;
...
FROM test_vector_table t1, test_vector_table t2;
SELECT COSINE_DISTANCE(t1.vector_col, t2.vector_col)
FROM test_vector_table t1, test_vector_table t2;
SELECT INNER_PRODUCT(t1.vector_col, t2.vector_col)
FROM test_vector_table t1, test_vector_table t2;
SELECT L1_DISTANCE(t1.vector_col, t2.vector_col)
FROM test_vector_table t1, test_vector_table t2;
SELECT L2_DISTANCE(t1.vector_col, t2.vector_col)
FROM test_vector_table t1, test_vector_table t2;

-- Execution/ Column and a literal/ Valid
SELECT
    VECTOR_DISTANCE(vector_col, '[2, 3, 4]', COSINE),
    COSINE_DISTANCE(vector_col, '[2, 3, 4]'),
    INNER_PRODUCT(vector_col, '[2, 3, 4]'),
    L1_DISTANCE(vector_col, '[2, 3, 4]'),
    L2_DISTANCE(vector_col, '[2, 3, 4]')
FROM test_vector_table;

SELECT
    VECTOR_DISTANCE('[2, 3, 4]', vector_col, COSINE),
    COSINE_DISTANCE('[2, 3, 4]', vector_col),
    INNER_PRODUCT('[2, 3, 4]', vector_col),
    L1_DISTANCE('[2, 3, 4]', vector_col),
    L2_DISTANCE('[2, 3, 4]', vector_col)
FROM test_vector_table;

-- -- Execution/ NULL and column/ Valid
-- SELECT 
--     VECTOR_DISTANCE(NULL, vector_col),
--     VECTOR_DISTANCE(NULL, vector_col, COSINE),
--     COSINE_DISTANCE(NULL, vector_col),
--     INNER_PRODUCT(NULL, vector_col),
--     L1_DISTANCE(NULL, vector_col),
--     L2_DISTANCE(NULL, vector_col)
-- FROM test_vector_table;

-- -- Execution/ column and NULL/ Valid
-- SELECT 
--     VECTOR_DISTANCE(vector_col, NULL),
--     VECTOR_DISTANCE(vector_col, NULL, COSINE),
--     INNER_PRODUCT(vector_col, NULL),
--     L1_DISTANCE(vector_col, NULL),
--     L2_DISTANCE(vector_col, NULL)
-- FROM test_vector_table;
...
SELECT
    VECTOR_DISTANCE(t1.vector_col, t1.vector_col, COSINE),
    COSINE_DISTANCE(t1.vector_col, t1.vector_col),
    INNER_PRODUCT(t1.vector_col, t1.vector_col),
    L1_DISTANCE(t1.vector_col, t1.vector_col),
    L2_DISTANCE(t1.vector_col, t1.vector_col)
...
-- SELECT
--     VECTOR_DISTANCE('[1,2,3]', t2.vector_col, COSINE),
--     COSINE_DISTANCE('[1,2,3]',t2.vector_col),
--     INNER_PRODUCT('[1,2,3]', t2.vector_col),
--     L1_DISTANCE('[1,2,3]', t2.vector_col),
--     L2_DISTANCE('[1,2,3]', t2.vector_col)
-- FROM test_vector_table t1, test_vector_table t2;

-- Execution / Different vector dimensions/ With table of different dimensions/ Error
-- TODO: disable due to CTP crash (with assert()). Enable after error handling.
-- SELECT
--     VECTOR_DISTANCE(t1.vector_col, t2.vector_col, COSINE),
--     COSINE_DISTANCE(t1.vector_col, t2.vector_col),
--     INNER_PRODUCT(t1.vector_col, t2.vector_col),
--     L1_DISTANCE(t1.vector_col, t2.vector_col),
--     L2_DISTANCE(t1.vector_col, t2.vector_col)
