...
execute stmt;
prepare stmt from 'SELECT /*+ RECOMPILE */ * FROM u, t WHERE u.i = t.i AND u.j > 10 using index i_u_j keyLIMIT 2-?,?/2 order by u.i';
execute stmt using 1,10;
prepare stmt from 'SELECT /*+ RECOMPILE */ * FROM u, t WHERE u.i = t.i AND u.j > 10 using index i_u_j keyLIMIT ?,? order by u.i';
execute stmt using 0,5;
prepare stmt from 'SELECT /*+ RECOMPILE */ * FROM u, t WHERE u.i = t.i AND u.j > 10 using index i_u_j keyLIMIT ?-?,?*? order by u.i';
execute stmt using 2,1,5,1;
