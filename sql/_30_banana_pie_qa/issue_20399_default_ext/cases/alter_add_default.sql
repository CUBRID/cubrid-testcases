...
insert into t1(c1, c2) values(2, {'cba'});
select attr_name, default_value from db_attribute where class_name = 't1' order by 1,2;
select c1, c2, c3=to_char(sysdatetime, 'YYYYMMDD'), c3=to_char(sysdatetime, 'YYYYMMDD') from t1 order by 1,2,3,4;
...
select attr_name, default_value from db_attribute where class_name = 't1' order by 1,2;
select c1, c2, c3=to_char(sysdatetime, 'YYYYMMDD'), nvl2(c4, c4=to_char(sysdatetime, 'YYYYMMDD'), c4) from t1 order by 1,2,3,4;
update t1 set c3=default where c1 is not null;
show create table t1;
alter t1 drop column c3;
...
select c1, c2, nvl2(c4, c4=to_char(sysdatetime, 'YYYYMMDD'), c4), c5=c5_answer, c6=c6_answer, c9=c9_answer from t1 order by 1,2,3,4;
...
select if(c10 is not null, 'ok', 'nok') from  t1;
select attr_name, default_value from db_attribute where class_name = 't1' and attr_name='c10' order by 1,2;
set names iso88591;
