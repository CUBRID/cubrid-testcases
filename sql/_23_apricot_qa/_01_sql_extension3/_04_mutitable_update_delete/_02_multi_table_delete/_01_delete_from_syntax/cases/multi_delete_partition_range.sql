...
delete md_p_range1, md_p_range2 from md_p_range1, md_p_range2 where md_p_range1.col1=md_p_range2.col2 and md_p_range1.id1=md_p_range2.id2 and length(trim(md_p_range2.col1))>2;
select * from md_p_range1 order by 1;
select * from md_p_range2 order by 1;


--TEST: error, group by clause in delete statement
delete m1, m2 from md_p_range1 m1, md_p_range2 m2 where m1.col1 > m2.col2 group by m2.col1;
select if (count(*) = 6, 'ok', 'nok') from md_p_range1;
select if (count(*) = 11, 'ok', 'nok') from md_p_range2;


--TEST: inner join, 2 rows deleted
delete m1.* from md_p_range1 as m1 inner join (select id2, time(col2) as col2 from md_p_range2 where second(col2)=15) as m2 on m1.col1=m2.col2;
select * from md_p_range1 order by 1;
select if (count(*) = 11, 'ok', 'nok') from md_p_range2;


--TEST: delete and select from the same table, 5 rows deleted.
delete m1, md_p_range2 from md_p_range1 m1 left outer join md_p_range2 on m1.col1=md_p_range2.col2 where m1.id1 > (select avg(id1) from md_p_range1);
...
delete m1, m2 from md_p_range1 m1 right outer join md_p_range2 m2 on m1.col1=m2.col2 where m1.id1 < (select avg(id2) from md_p_range2);
...
delete m1, m2 from md_p_range1__p__p1 m1, md_p_range2__p__p2 m2 where m1.col1=m2.col2;
