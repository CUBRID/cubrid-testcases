--TEST: delete from 2 tables with date type column for join condition
...
delete md_date2 from md_date1, md_date2 where md_date1.col1=md_date2.col2 and md_date1.col1=date'2000-01-01';
select if (count(*) = 12, 'ok', 'nok') from md_date1;
select * from md_date2 order by 1;


--TEST: with table aliases, 1 row deleted
delete m1 from md_date1 m1, (select id, col1, col2 from md_date2) m2 where m1.col1=m2.col2 and m2.col1 < 20;
select * from md_date1 order by 1;
select if (count(*) = 5, 'ok', 'nok') from md_date2;


--TEST: inner join
delete m1, m2 from md_date1 as m1 inner join md_date2 as m2 on m1.col1=m2.col2 where date_add(m1.col1, interval -2 day) < '2001-01-01';
select * from md_date1 order by 1;
select * from md_date2 order by 1;


--TEST: left outer join, one table with an alias and the other without
delete m1, md_date2 from md_date1 m1 left outer join md_date2 on m1.col1=md_date2.col2 where md_date2.id > (select avg(id) from md_date2);
select * from md_date1 order by 1;
select if (count(*) = 3, 'ok', 'nok') from md_date2;


--TEST: right outer join
delete m1, m2 from md_date1 m1 right outer join md_date2 m2 on m1.col1=m2.col2 where m2.col1 < (select avg(id) from md_date2);
