--TEST: delete from 3 tables with datetime type column for join condition
...
delete m1, m2, m3 from md_datetime1 m1, md_datetime2 m2, md_datetime3 m3 where m2.col1=m1.col2 and m1.col2=m3.col2;


--TEST: without table aliases, no matched results
delete md_datetime1, md_datetime2, md_datetime3 from md_datetime1, md_datetime2, md_datetime3 where md_datetime1.col1=md_datetime2.col2 and md_datetime1.col1=md_datetime3.col2 and truncate(md_datetime1.id1, 2)=truncate(md_datetime2.id2, 2);
...
delete m2, m3 from (select id1, col1 from (select * from md_datetime1 where month(col1)=9) md1) m1, md_datetime2 m2, md_datetime3 m3 where m1.id1=m2.id2 and m3.col2=m1.col1;
select if (count(*) = 10, 'ok', 'nok') from md_datetime1;
...
delete m1 from md_datetime1 as m1 inner join (select id2, substr(col1, 1, 6), col2 from md_datetime2) m2 on m1.col1=m2.col2 inner join (select col2 from md_datetime3) m3 on m3.col2=m2.col2 where m1.id1 > (select sum(id2)/5 from md_datetime2);
...
delete m1, m2, m3 from md_datetime2 m2 left outer join md_datetime1 m1 on m1.col1=m2.col2 where m1.id1 > (select max(id1)/3 from md_datetime1) inner join md_datetime3 m3 on m2.col2=m1.col1;
--TEST: success, 7 rows deleted
delete m1, m2, m3 from md_datetime2 m2 left outer join md_datetime1 m1 on m1.col1=m2.col2 inner join md_datetime3 m3 on m3.col2=m1.col1 where m3.id3 < (select max(id2)/100 from md_datetime2);
...
--TEST: 2 rows deleted 2 rows deleted 
delete m2, m1 from md_datetime1 m1 right outer join md_datetime2 m2 on m1.col1=m2.col2 right outer join md_datetime3 m3 on m3.col2=m2.col2 where m3.id3 < (select abs(avg(id2)) from md_datetime2);
--TEST: success, 5 rows deleted
delete m2, m1, m3 from md_datetime1 m1 right outer join md_datetime2 m2 on m1.col1=m2.col2 right outer join md_datetime3 m3 on m3.col2=m2.col2 where m3.id3 < (select abs(avg(id2)) from md_datetime2);
select if (count(*) = 7, 'ok', 'nok') from md_datetime1;
