--TEST: delete from 2 tables with double type column for join condition
...
delete m1 from md_double1 m1, md_double2 m2 where m2.col1=m1.col1;


--TEST: without table aliases
delete md_double1 from md_double1, md_double2 where truncate(md_double1.id1, -2)=truncate(md_double2.id2, -2) and second(md_double1.col1) < 14;
select * from md_double1 order by 1;
select if (count(*) = 12, 'ok', 'nok') from md_double2;


--TEST: with table aliases
delete m2 from (select id1, col1 from (select * from md_double1 where month(col1)=9) md1) m1, md_double2 m2 where m1.id1=m2.id2 and time(m1.col1) <= '12:12:18';
select if (count(*) = 9, 'ok', 'nok') from md_double1;
select * from md_double2 order by 1;


--TEST: error, delete from a derived table
delete m1, m2 from md_double1 as m1 inner join (select id2, substr(col1, 1, 6) from md_double2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
--TEST: inner join, no matched results
delete m1 from md_double1 as m1 inner join (select id2, substr(col1, 1, 6) from md_double2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
select if (count(*) = 9, 'ok', 'nok') from md_double1;
select if (count(*) = 11, 'ok', 'nok') from md_double2;


--TEST: left outer join, delete and select from the same table
delete m1, m2 from md_double2 m2 left outer join md_double1 m1 on m1.id1=m2.id2 where m1.id1 > (select avg(id1) from md_double1);
select if (count(*) = 9, 'ok', 'nok') from md_double1;
select * from md_double2 order by 1;


--TEST: right outer join
delete m2, m1 from md_double1 m1 right outer join md_double2 m2 on m1.id1=m2.id2 where m1.id1 < (select abs(avg(id2)) from md_double2);
select if (count(*) = 9, 'ok', 'nok') from md_double1;
select if (count(*) = 11, 'ok', 'nok') from md_double2;
