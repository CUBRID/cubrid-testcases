--TEST: delete from 2 tables with list partition
...
delete m2 from md_char2 as m2 inner join (select id1, trim(substr(col2, 1, 6)) as col2 from md_char1 where left(col2, 3) = 'abc') m1 on left(m1.col2, 3)=left(m2.col1, 3);
select if (count(*) = 5, 'ok', 'nok') from md_char1;
select * from md_char2 order by 1;
...
select if (count(*) = 6, 'ok', 'nok') from md_char2;
...
select if (count(*) = 1, 'ok', 'nok') from md_char1;
