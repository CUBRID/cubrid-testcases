--TEST: delete from 2 tables with timestamp type column for join condition
...
delete m1 from md_timestamp1 m1, md_timestamp2 m2 where m2.col1=m1.col1;


--TEST: without table aliases, no matched results
delete md_timestamp1 from md_timestamp1, md_timestamp2 where md_timestamp1.col1=md_timestamp2.col2 and truncate(md_timestamp1.id1, 2)=truncate(md_timestamp2.id2, 2);
select if (count(*) = 10, 'ok', 'nok') from md_timestamp1;
select if (count(*) = 12, 'ok', 'nok') from md_timestamp2;


--TEST: with table aliases
delete m2 from (select id1, col1 from (select * from md_timestamp1 where month(col1)=9) md1) m1, md_timestamp2 m2 where m1.col1=m2.col2 and (m2.id2>3333333456 or m2.id2<1235);
select if (count(*) = 10, 'ok', 'nok') from md_timestamp1;
select * from md_timestamp2 order by 1;


--TEST: inner join, 1 row deleted
delete m1 from md_timestamp1 as m1 inner join (select id2, substr(col1, 1, 6), col2 from md_timestamp2) m2 on m1.col1=m2.col2 where m1.id1 > (select sum(id2)/5 from md_timestamp2);
select * from md_timestamp1 order by 1;
select if (count(*) = 10, 'ok', 'nok') from md_timestamp2;


--TEST: left outer join, delete and select from the same table
delete m1, m2 from md_timestamp2 m2 left outer join md_timestamp1 m1 on m1.col1=m2.col2 where m1.id1 > (select max(id1)/3 from md_timestamp1);
select if (count(*) = 8, 'ok', 'nok') from md_timestamp1;
select if (count(*) = 8, 'ok', 'nok') from md_timestamp2;


--TEST: right outer join
delete m2, m1 from md_timestamp1 m1 right outer join md_timestamp2 m2 on m1.col1=m2.col2 where m1.id1 < (select abs(avg(id2)) from md_timestamp2);
