...
prepare stmt from 'delete md_set2 from md_set1, md_set2 where md_set1.col1=md_set2.col2 and md_set1.col1=?';
...
prepare stmt from 'delete m1 from md_set1 m1, (select id, col1, col2 from md_set2) m2 where m1.col1=m2.col2 and m2.col1 < ?'
execute stmt using 20
deallocate prepare stmt;
select if (count(*) = 11, 'ok', 'nok') from md_set1;
select if (count(*) = 5, 'ok', 'nok') from md_set2;


--TEST: multi-table delete with prepare statement, inner join, 1 row deleted
prepare stmt from 'delete m1, m2 from md_set1 as m1 inner join md_set2 as m2 on m1.col1=m2.col2 where m1.col2=?'
execute stmt using 'ccc'
deallocate prepare stmt;
select if (count(*) = 10, 'ok', 'nok') from md_set1;
select if (count(*) = 4, 'ok', 'nok') from md_set2;
...
prepare stmt from 'delete m1, md_set2 from md_set1 m1 left outer join md_set2 on m1.col1=md_set2.col2 where md_set2.id > (select avg(id) from md_set2)'
execute stmt
deallocate prepare stmt;
...
prepare stmt from 'delete m1, m2 from md_set1 m1 right outer join md_set2 m2 on m1.col1=m2.col2 where m2.col1 > (select avg(id) from md_set2)'
execute stmt
deallocate prepare stmt;
select if (count(*) = 9, 'ok', 'nok') from md_set1;
select if (count(*) = 0, 'ok', 'nok') from md_set2;
