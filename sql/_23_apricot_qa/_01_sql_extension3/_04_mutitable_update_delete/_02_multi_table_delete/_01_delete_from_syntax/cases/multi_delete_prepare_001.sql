...
prepare stmt from 'delete md_time1, md_time2 from md_time1, md_time2 where md_time1.col1=md_time2.col2 and md_time1.id1=md_time2.id2 and length(trim(md_time2.col1))>?';
...
prepare stmt from 'delete m1, m2 from md_time1 m1, md_time2 m2 where m1.col1 > m2.col2 group by m2.col1';
execute stmt;
deallocate prepare stmt;
...
prepare stmt from 'delete m1.* from md_time1 as m1 inner join (select id2, time(col2) as col2 from md_time2 where second(col2)=?) as m2 on m1.col1=m2.col2';
...
prepare stmt from 'delete m1, md_time2 from md_time1 m1 left outer join md_time2 on m1.col1=md_time2.col2 where m1.id1 > (select avg(id1) from md_time1)';
execute stmt;
deallocate prepare stmt;
...
--TEST: delete with prepare statement, delete and select from the same table
prepare stmt from 'delete m1, m2 from md_time1 m1 right outer join md_time2 m2 on m1.col1=m2.col2 where m1.id1 < (select avg(id2) from md_time2)';
execute stmt;
deallocate prepare stmt;
