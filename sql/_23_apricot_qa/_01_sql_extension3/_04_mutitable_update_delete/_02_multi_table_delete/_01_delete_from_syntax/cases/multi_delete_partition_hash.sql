...
delete md_p_hash1 from md_p_hash1, md_p_hash2 where md_p_hash1.id1 > (select min(col1) from md_p_hash2) and md_p_hash1.col1='lll';
--TEST: delete and select from the same table, no matched rows
delete md_p_hash1, md_p_hash2 from md_p_hash1, md_p_hash2 where md_p_hash1.id1 > (select min(col1) from md_p_hash2) and md_p_hash1.col1='lll';
select * from md_p_hash1 order by 1;
select if (count(*) = 6, 'ok', 'nok') from md_p_hash2;


--TEST: with table aliases, derived table, 1 row deleted
delete m2 from (select id1, col1 from md_p_hash1) as m1, md_p_hash2 m2 where m1.id1=m2.id2 and m1.col1='fff';
select if (count(*) = 11, 'ok', 'nok') from md_p_hash1;
select * from md_p_hash2 order by 1;


--TEST: inner join, no matched results
delete m1, m2, m1 from md_p_hash1 as m1 inner join md_p_hash2 as m2 on m1.id1=m2.id2 where m2.col1 > 100;
select if (count(*) = 11, 'ok', 'nok') from md_p_hash1;
...
delete m1, md_p_hash2 from md_p_hash1 m1 left outer join md_p_hash2 on m1.id1=md_p_hash2.id2 where m1.id1 > (select avg(id1) from md_p_hash1);
...
delete m1, m2 from md_p_hash1 m1 right outer join md_p_hash2 m2 on m1.id1=m2.id2 where m1.id1 < (select avg(id2) from md_p_hash2);
