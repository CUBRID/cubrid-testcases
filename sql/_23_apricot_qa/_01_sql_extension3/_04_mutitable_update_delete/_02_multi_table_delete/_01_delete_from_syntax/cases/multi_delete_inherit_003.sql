...
delete md_set2 from md_set1, md_set2 where md_set1.col1=md_set2.col1 and md_set1.col1={1, date'2000-01-01', 2};
select if (count(*) = 12, 'ok', 'nok') from md_set1;
select * from md_set2 order by 1;


--TEST: with table aliases, 1 row deleted
delete m1 from md_set1 m1, (select id, col1, col3 from md_set2) m2 where m1.col1=m2.col1 and m2.col3 < 20;
select * from md_set1 order by 1;
select if (count(*) = 5, 'ok', 'nok') from md_set2;


--TEST: inner join, 2 rows deleted
delete m1, m2 from md_set1 as m1 inner join md_set2 as m2 on m1.col1=m2.col1 where m1.col2='ccc';
select * from md_set1 order by 1;
select * from md_set2 order by 1;
...
delete m1, md_set2 from md_set1 m1 left outer join md_set2 on m1.col1=md_set2.col1 where md_set2.id > (select avg(id) from md_set2);
select * from md_set1 order by 1;
...
delete m1, m2 from md_set1 m1 right outer join md_set2 m2 on m1.col1=m2.col1 where m2.col3 < (select avg(id) from md_set2);
