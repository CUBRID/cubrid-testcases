--TEST: delete from 2 tables with time type column for join condition, with composed index
...
delete md_time1, md_time2 from md_time1, md_time2 where md_time1.col1=md_time2.col2 and md_time1.id1=md_time2.id2 and length(trim(md_time2.col1))>2;
select * from md_time1 order by 1;
select * from md_time2 order by 1;


--TEST: error, group by clause in delete statement
delete m1, m2 from md_time1 m1, md_time2 m2 where m1.col1 > m2.col2 group by m2.col1;
select if (count(*) = 6, 'ok', 'nok') from md_time1;
select if (count(*) = 11, 'ok', 'nok') from md_time2;


--TEST: inner join, 2 rows deleted
delete m1.* from md_time1 as m1 inner join (select id2, time(col2) as col2 from md_time2 where second(col2)=15) as m2 on m1.col1=m2.col2;
select * from md_time1 order by 1;
select if (count(*) = 11, 'ok', 'nok') from md_time2;


--TEST: left outer join, delete and select from the same table
delete m1, md_time2 from md_time1 m1 left outer join md_time2 on m1.col1=md_time2.col2 where m1.id1 > (select avg(id1) from md_time1);
select if (count(*) = 2, 'ok', 'nok') from md_time1;
select if (count(*) = 8, 'ok', 'nok') from md_time2;


--TEST: right outer join
delete m1, m2 from md_time1 m1 right outer join md_time2 m2 on m1.col1=m2.col2 where m1.id1 < (select avg(id2) from md_time2);
