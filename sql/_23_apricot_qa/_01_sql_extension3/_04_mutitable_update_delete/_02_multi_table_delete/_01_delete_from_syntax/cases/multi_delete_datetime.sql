--TEST: delete from 2 tables with datetime type column for join condition
...
delete m1 from md_datetime1 m1, md_datetime2 m2 where m2.col1=m1.col1;


--TEST: without table aliases, no matched results
delete md_datetime1 from md_datetime1, md_datetime2 where md_datetime1.col1=md_datetime2.col2 and truncate(md_datetime1.id1, 2)=truncate(md_datetime2.id2, 2);
select if (count(*) = 10, 'ok', 'nok') from md_datetime1;
select if (count(*) = 12, 'ok', 'nok') from md_datetime2;


--TEST: with table aliases
delete m2 from (select id1, col1 from (select * from md_datetime1 where month(col1)=9) md1) m1, md_datetime2 m2 where m1.col1=m2.col2 and (m2.id2>3333333456 or m2.id2<1235);
select if (count(*) = 10, 'ok', 'nok') from md_datetime1;
select * from md_datetime2 order by 1;


--TEST: inner join, 1 row deleted
delete m1 from md_datetime1 as m1 inner join (select id2, substr(col1, 1, 6), col2 from md_datetime2) m2 on m1.col1=m2.col2 where m1.id1 > (select sum(id2)/5 from md_datetime2);
select * from md_datetime1 order by 1;
select if (count(*) = 10, 'ok', 'nok') from md_datetime2;


--TEST: left outer join
--TEST: success, 2 rows deleted
delete m1 from md_datetime2 m2 left outer join md_datetime1 m1 on m1.col1=m2.col2 where m1.id1 > (select max(id2)/100 from md_datetime2);
select * from md_datetime1 order by 1;
select if (count(*) = 10, 'ok', 'nok') from md_datetime2;
...
delete m2 from md_datetime2 m2 right outer join (select round(id1) as id1, col1 from md_datetime1 where length(col1) < 100) m1 on time(m1.col1)=time(m2.col2);
