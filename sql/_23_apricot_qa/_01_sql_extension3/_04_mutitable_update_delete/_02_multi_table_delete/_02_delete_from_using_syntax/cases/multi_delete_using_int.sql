--delete from 2 tables with int type column for join condition
...
delete from md_int1 using md_int1, md_int2 where md_int1.id1=md_int2.id2 and md_int2.col1 < 10;
select * from md_int1 order by 1;
select if (count(*) = 6, 'ok', 'nok') from md_int2;


--with table aliases
delete from m2 using md_int1 m1, md_int2 m2 where m1.id1=m2.id2 and m1.col1='fff';
select if (count(*) = 11, 'ok', 'nok') from md_int1;
select * from md_int2 order by 1;


--inner join, no matched results
delete from m1, m2 using md_int1 as m1 inner join md_int2 as m2 on m1.id1=m2.id2 where m2.col1 > 100;
select if (count(*) = 11, 'ok', 'nok') from md_int1;
select if (count(*) = 5, 'ok', 'nok') from md_int2;


--left outer join, one table with an alias and the other without
delete from m1, md_int2 using md_int1 m1 left outer join md_int2 on m1.id1=md_int2.id2 where m1.id1 > (select avg(id1) from md_int1);
select if (count(*) = 5, 'ok', 'nok') from md_int1;
select * from md_int2 order by 1;


--right outer join
delete from m1, m2 using md_int1 m1 right outer join md_int2 m2 on m1.id1=m2.id2 where m1.id1 < (select avg(id2) from md_int2);
select if(count(*)=5, 'ok', 'nok') from md_int1;
