--delete from 2 tables with monetary type column for join condition, with composed index
...
delete from m1 using md_monetary1 m1, md_monetary2 m2 where m1.col1=m2.col1;


--without table aliases
delete from md_monetary1 using md_monetary1, md_monetary2 where md_monetary1.id1=md_monetary2.id2 and minute(md_monetary1.col1) < 14;
select * from md_monetary1 order by 1;
select if (count(*) = 12, 'ok', 'nok') from md_monetary2;


--with table aliases
delete from m2 using md_monetary1 m1, md_monetary2 m2 where m1.id1=m2.id2 and time(m1.col1) <= '12:15:16' and last_day(m1.col1) = '2011-09-30';
select if (count(*) = 5, 'ok', 'nok') from md_monetary1;
select * from md_monetary2 order by 1;


--inner join, no matched results
delete from m1, m2 using (select id1, adddate(col1, 10) from md_monetary1) as m1 inner join (select id2, substr(col1, 1, 6) from md_monetary2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
delete from m1 using (select id1, adddate(col1, 10) from md_monetary1) as m1 inner join (select id2, substr(col1, 1, 6) from md_monetary2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
select if (count(*) = 5, 'ok', 'nok') from md_monetary1;
select if (count(*) = 11, 'ok', 'nok') from md_monetary2;

--left outer join, delete and select from the same table
delete from m1, md_monetary2 using md_monetary2 left outer join md_monetary1 m1 on m1.id1=md_monetary2.id2 where m1.id1 > (select avg(id1) from md_monetary1);
select if (count(*) = 3, 'ok', 'nok') from md_monetary1;
select * from md_monetary2 order by 1;


--right outer join
delete from m2, m1 using md_monetary1 m1 right outer join md_monetary2 m2 on m1.id1=m2.id2 where m1.id1 < (select abs(avg(id2)) from md_monetary2);
select if (count(*) = 3, 'ok', 'nok') from md_monetary1;
