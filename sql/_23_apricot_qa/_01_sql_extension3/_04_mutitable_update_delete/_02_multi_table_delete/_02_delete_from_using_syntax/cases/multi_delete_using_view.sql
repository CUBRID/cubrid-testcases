...
create view md_v1 as select * from md_int1 order by 1;
create view md_v2 as select * from md_int2 order by 1;


--TEST: without table aliases
delete from md_v1 using md_v1, md_v2 where md_v1.id1=md_v2.id2 and md_v2.col1 < 10;
select * from md_v1 order by 1;
select if (count(*) = 6, 'ok', 'nok') from md_v2;


--TEST: with table aliases
delete from v2 using md_v1 v1, md_v2 v2 where v1.id1=v2.id2 and v1.col1='fff';
select if (count(*) = 11, 'ok', 'nok') from md_v1;
select * from md_v2 order by 1;


--TEST: inner join, no matched results
delete from v1, v2 using md_v1 as v1 inner join md_v2 as v2 on v1.id1=v2.id2 where v2.col1 > 100;
select if (count(*) = 11, 'ok', 'nok') from md_v1;
...
delete from v1, md_v2 using md_v1 v1 left outer join md_v2 on v1.id1=md_v2.id2 where v1.id1 > (select avg(id1) from md_v1);
...
--TEST: delete and select from the same table
delete from v1, v2 using md_v1 v1 right outer join md_v2 v2 on v1.id1=v2.id2 where v1.id1 < (select avg(id2) from md_v2);
select if (count(*) = 5, 'ok', 'nok') from md_v1;
select if (count(*) = 2, 'ok', 'nok') from md_v2;
