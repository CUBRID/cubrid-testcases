--delete from 2 tables with numeric type column for join condition, with composed index
...
delete from m1, m2 using md_numeric1 m1, md_numeric2 m2 where m1.id1=m2.col1;


--without table aliases
delete from md_numeric1 using md_numeric1, md_numeric2 where md_numeric1.id1=md_numeric2.id2 and char_length(md_numeric2.col1) < 5;
select * from md_numeric1 order by 1;
select if (count(*) = 12, 'ok', 'nok') from md_numeric2;


--with table aliases
delete from m2 using md_numeric1 m1, md_numeric2 m2 where m1.id1=m2.id2 and m1.col1='2011-09-01 12:12:13';
select if (count(*) = 5, 'ok', 'nok') from md_numeric1;
select * from md_numeric2 order by 1;


--inner join, no matched results
delete from m1, m2 using md_numeric1 as m1 inner join (select id2, substr(col1, 1, 6) from md_numeric2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
delete from m1 using md_numeric1 as m1 inner join (select id2, substr(col1, 1, 6) from md_numeric2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
select if (count(*) = 5, 'ok', 'nok') from md_numeric1;
select if (count(*) = 11, 'ok', 'nok') from md_numeric2;


--left outer join, delete and select from the same table
delete from m1, md_numeric2 using md_numeric1 m1 left outer join md_numeric2 on m1.id1=md_numeric2.id2 where m1.id1 > (select avg(id1) from md_numeric1);
select if (count(*) = 3, 'ok', 'nok') from md_numeric1;
select * from md_numeric2 order by 1;


--right outer join
delete from m1, m2 using md_numeric1 m1 right outer join md_numeric2 m2 on m1.id1=m2.id2 where m1.id1 < (select abs(avg(id2)) from md_numeric2);
select if (count(*) = 3, 'ok', 'nok') from md_numeric1;
