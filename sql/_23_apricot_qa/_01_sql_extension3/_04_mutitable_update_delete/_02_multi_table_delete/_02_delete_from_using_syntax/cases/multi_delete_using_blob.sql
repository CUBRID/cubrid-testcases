...
delete from md_blob1 using md_blob1, md_blob2 where blob_to_bit(md_blob1.col2) = col3 and md_blob1.col1='lll';
select id1, col1, blob_to_bit(col2) from md_blob1 order by 1;
...
--delete from md_blob1, md_blob2 using md_blob1, md_blob2 where blob_to_bit(md_blob1.col2) > (select min(blob_to_bit(col2)) from md_blob2) and md_blob1.col1='lll';
--select if (count(*) = 11, 'ok', 'nok') from md_blob1;
--select if (count(*) = 6, 'ok', 'nok') from md_blob2;


--TEST: with table aliases, derived table, 1 row deleted
delete from m2 using (select id1, col2 from md_blob1) as m1, md_blob2 m2 where blob_to_bit(m1.col2)=blob_to_bit(m2.col2) and m1.id1=(select max(id1) from md_blob1);
select if (count(*) = 11, 'ok', 'nok') from md_blob1;
select if (count(*) = 6, 'ok', 'nok') from md_blob2;


--TEST: inner join, no matched results
delete from m1 using md_blob1 as m1 inner join md_blob2 as m2 on blob_to_bit(m1.col2)=m2.col3 where m2.col1 > (select 100);
select if (count(*) = 11, 'ok', 'nok') from md_blob1;
select if (count(*) = 6, 'ok', 'nok') from md_blob2;
...
--TEST: delete and select from the same table
delete from m1 using md_blob1 m1 left outer join md_blob2 on blob_to_bit(m1.col2)=blob_to_bit(md_blob2.col2) where m1.id1 < (select count(concat(m1.col1, m2.col1)) from md_blob1 m1 inner join md_blob2 m2 on m1.id1=m2.id2);
...
delete from m1, m2 using md_blob1 m1 right outer join md_blob2 m2 on blob_to_bit(m1.col2)=m2.col3 where m1.id1 > (select count(concat(col1, 'a')) from md_blob2 m2 where id2 < 9);
select if (count(*) = 4, 'ok', 'nok') from md_blob1;
select if (count(*) = 3, 'ok', 'nok') from md_blob2;


--TEST: delete all data
delete from m1, m2 using md_blob1 m1, md_blob2 m2;
