--delete from 2 tables with trigger
...
select * from md_trigg1 order by 1, 2;
...
delete from m1, m2 using md_trigg1 m1, md_trigg2 m2 where m1.col2=m2.id2;
--TEST: no trigger actions
select if (count(*) = 0, 'ok', 'nok') from action1 order by 1;
select if (count(*) = 0, 'ok', 'nok') from action2 order by 1;


--without table aliases, 1 row deleted in each table
delete from md_trigg1, md_trigg2 using md_trigg1, md_trigg2 where md_trigg1.col2=md_trigg2.col1 and char_length(md_trigg1.col1) > 5 and md_trigg1.id1=md_trigg2.id2;
select * from md_trigg1 order by 1,2;
select * from md_trigg2 order by 1,2;
--TEST: trigger actions
select if (count(*) = 1, 'ok', 'nok') from action1 order by 1;
select if (count(*) = 1, 'ok', 'nok') from action2 order by 2;


--with table aliases, 2 rows deleted
delete from m2 using md_trigg1 m1, md_trigg2 m2 where m1.col2=m2.col1 and round(m2.id2) in (1234, 5555, 5657, 667);
select if (count(*) = 5, 'ok', 'nok') from md_trigg1;
select * from md_trigg2 order by 1,2;
--TEST: trigger actions
select if (count(*) = 1, 'ok', 'nok') from action1 order by 1;
select if (count(*) = 3, 'ok', 'nok') from action2 order by 1;


--inner join, 3 rows deleted
delete from m2 using md_trigg2 as m2 inner join (select id1, trim(substr(col2, 1, 6)) as col2 from md_trigg1 where left(col2, 3) = 'abc') m1 on left(m1.col2, 3)=left(m2.col1, 3);
select if (count(*) = 5, 'ok', 'nok') from md_trigg1 order by 1;
select * from md_trigg2 order by 1,2;
--TEST: trigger actions
select if (count(*) = 1, 'ok', 'nok') from action1 order by 1;
select if (count(*) = 6, 'ok', 'nok') from action2 order by 1;


--TEST: left outer join, delete and select from the same table
delete from m1, md_trigg2 using md_trigg1 m1 left outer join md_trigg2 on m1.col2=md_trigg2.col1 where m1.id1 > (select avg(id1) from md_trigg1);
select if (count(*) = 2, 'ok', 'nok') from md_trigg1 order by 1;
select if (count(*) = 5, 'ok', 'nok') from md_trigg2 order by 1;
--TEST: TEST: no trigger actions
select if (count(*) = 4, 'ok', 'nok') from action1 order by 1;
select if (count(*) = 7, 'ok', 'nok') from action2 order by 1;


--TEST: right outer join
--TEST: delete and select from the same table
delete from m1, m2 using md_trigg1 m1 right outer join md_trigg2 m2 on m1.col2=m2.col1 where m1.id1 < (select abs(avg(id2)) from md_trigg2);
select if (count(*) = 2, 'ok', 'nok') from md_trigg1 order by 1;
select if (count(*) = 5, 'ok', 'nok') from md_trigg2 order by 1;
--TEST: TEST: no trigger actions
select if (count(*) = 4, 'ok', 'nok') from action1 order by 1;
