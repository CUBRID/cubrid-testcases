--delete from 2 tables with float type column for join condition, with composed index
...
delete from m1 using md_float1 m1, md_float2 m2 where m2.id2=m1.col1;


--without table aliases
delete from md_float1 using md_float1, md_float2 where md_float1.id1=md_float2.id2 and second(md_float1.col1) < 14;
select * from md_float1 order by 1;
select if (count(*) = 12, 'ok', 'nok') from md_float2;


--with table aliases
delete from m2 using md_float1 m1, md_float2 m2 where m1.id1=m2.id2 and time(m1.col1) <= '12:12:13';
select if (count(*) = 5, 'ok', 'nok') from md_float1;
select * from md_float2 order by 1;


--inner join, no matched results
delete from m1, m2 using md_float1 as m1 inner join (select id2, substr(col1, 1, 6) from md_float2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
delete from m1 using md_float1 as m1 inner join (select id2, substr(col1, 1, 6) from md_float2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
select if (count(*) = 5, 'ok', 'nok') from md_float1;
select if (count(*) = 12, 'ok', 'nok') from md_float2;


--left outer join, delete and select from the same table
delete from m1, md_float2 using md_float2 left outer join md_float1 m1 on m1.id1=md_float2.id2 where m1.id1 > (select avg(id1) from md_float1);
select if (count(*) = 2, 'ok', 'nok') from md_float1;
select * from md_float2 order by 1;


--right outer join
delete from m2, m1 using md_float1 m1 right outer join md_float2 m2 on m1.id1=m2.id2 where m1.id1 < (select abs(avg(id2)) from md_float2);
select if (count(*) = 2, 'ok', 'nok') from md_float1;
