...
create table md_ro2(col1 varchar(10), id2 numeric(10, 5), index i(id2 desc, col1)) reuse_oid;
...
delete from m1, m2 using md_ro1 m1, md_ro2 m2 where m1.col2=m2.id2;


--TEST: without table aliases, 1 row deleted in eache table
delete from md_ro1, md_ro2 using md_ro1, md_ro2 where md_ro1.col2=md_ro2.col1 and char_length(md_ro1.col1) > 5 and md_ro1.id1=md_ro2.id2;
select * from md_ro1 order by 1, 2;
select * from md_ro2 order by 1, 2;


--TEST: with table aliases, 2 rows deleted
delete from m2 using md_ro1 m1, md_ro2 m2 where m1.col2=m2.col1 and round(m2.id2) in (1234, 5555, 5657, 667);
select if (count(*) = 5, 'ok', 'nok') from md_ro1 order by 1;
select * from md_ro2 order by 1,2;
...
delete from m2 using md_ro2 as m2 inner join (select id1, trim(substr(col2, 1, 6)) as col2 from md_ro1 where left(col2, 3) = 'abc') m1 on left(m1.col2, 3)=left(m2.col1, 3);
select if (count(*) = 7, 'ok', 'nok') from md_ro1 order by 1;
select * from md_ro2 order by 1,2;
...
delete from m1, m2 using md_ro1 m1 left outer join md_ro2 m2 on m1.col2=m2.col1 where m1.id1 > (select abs(avg(id2)) from md_ro2);
select * from md_ro1 order by 1,2;
select if (count(*) = 8, 'ok', 'nok') from md_ro2 order by 1;
...
delete from m1, m2 using md_ro1 m1 right outer join md_ro2 m2 on m1.col2=m2.col1 where m1.id1 < (select abs(avg(id2)) from md_ro2);
--TEST: success, 4 rows deleted
delete from m2 using md_ro1 m1 right outer join md_ro2 m2 on m1.col2=m2.col1 where m2.id2 < (select avg(id1) from md_ro1);
select if (count(*) = 4, 'ok', 'nok') from md_ro1 order by 1;
