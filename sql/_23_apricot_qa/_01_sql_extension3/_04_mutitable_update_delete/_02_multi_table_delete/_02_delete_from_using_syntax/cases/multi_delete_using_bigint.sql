--delete from 2 tables with bigint type column for join condition, with composed index
...
delete from md_bigint1 using md_bigint1, md_bigint2 where md_bigint1.id1=md_bigint2.id2 and char_length(md_bigint2.col1) < 5;
select * from md_bigint1 order by 1;
select if (count(*) = 12, 'ok', 'nok') from md_bigint2;


--TEST: with table aliases
delete from m2 using md_bigint1 m1, md_bigint2 m2 where m1.id1=m2.id2 and m1.col1='2011-09-01 12:12:13';
select if (count(*) = 5, 'ok', 'nok') from md_bigint1;
select * from md_bigint2 order by 1;

--TEST: [er] delete from a derived table
delete from m1, m2, m1 using md_bigint1 as m1 inner join (select id2 from md_bigint2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
--TEST: inner join, no matched results
delete from m1 using md_bigint1 as m1 inner join (select id2 from md_bigint2 where left(col1, 6) = 'cubrid') m2 on m1.id1=m2.id2;
select if (count(*) = 5, 'ok', 'nok') from md_bigint1;
select if (count(*) = 11, 'ok', 'nok') from md_bigint2;

--TEST: left outer join, delete and select from the same table
delete from m1, md_bigint2 using md_bigint1 m1 left outer join md_bigint2 on m1.id1=md_bigint2.id2 where m1.id1 > (select avg(id1) from md_bigint1);
select if (count(*) = 3, 'ok', 'nok') from md_bigint1;
select * from md_bigint2 order by 1;


--TEST: right outer join
delete from m1, m2 using md_bigint1 m1 right outer join md_bigint2 m2 on m1.id1=m2.id2 where m1.id1 < (select avg(id2) from md_bigint2);
select if (count(*) = 3, 'ok', 'nok') from md_bigint1;
