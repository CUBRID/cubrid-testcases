...
delete from md_list2 using md_list1, md_list2 where md_list1.col1=md_list2.col2 and md_list1.col1={date'2000-01-01', 1, 2};
select if (count(*) = 12, 'ok', 'nok') from md_list1;
select * from md_list2 order by 1;


--with table aliases, 1 row deleted
delete from m1 using md_list1 m1, (select id, col1, col2 from md_list2) m2 where m1.col1=m2.col2 and m2.col1 < 20;
select * from md_list1 order by 1;
select if (count(*) = 5, 'ok', 'nok') from md_list2;


--inner join, 1 row deleted
delete from m1, m2 using md_list1 as m1 inner join md_list2 as m2 on m1.col1=m2.col2 where m1.col2='ccc';
select * from md_list1 order by 1;
select * from md_list2 order by 1;
...
delete from m1, md_list2 using md_list1 m1 left outer join md_list2 on m1.col1=md_list2.col2 where md_list2.id > (select avg(id) from md_list2);
...
--right outer join
--delete and select from the same table
delete from m1, m2 using md_list1 m1 right outer join md_list2 m2 on m1.col1=m2.col2 where m2.col1 < (select avg(id) from md_list2);
select if (count(*) = 9, 'ok', 'nok') from md_list1;
select if (count(*) = 3, 'ok', 'nok') from md_list2;
