--update 2 tables, 1 of which is with reuse_oid option



create table md_blob1(id1 smallint primary key, col1 char(20), col2 blob) reuse_oid;
...
--update (select id1, blob_to_bit(col2) col2 from md_blob1) as m1, md_blob2 m2 set m2.id2=(m1.id1+m2.col1)/3 where m1.col2=blob_to_bit(m2.col2) and bit_count(m2.id2)=2;
...
--TEST: inner join, 4 rows updated
update md_blob1 as m1 inner join md_blob2 as m2 on blob_to_bit(m1.col2)=blob_to_bit(m2.col2) set m2.col2=bit_to_blob(blob_to_bit(m1.col2)), m1.id1=(m1.id1*m2.id2) where m1.id1 > (select 9);
select if(count(*)=2, 'ok', 'nok') from md_blob1 where id1 in (80, 228);


--TEST: left outer join, one table with an alias and the other without
--TEST: update and select from the same table, 1 row updated.
update md_blob1 m1 left outer join md_blob2 m2 on blob_to_bit(m1.col2)=blob_to_bit(m2.col2) set m2.col2=bit_to_blob(blob_to_bit(m1.col2)+B'101'), m1.col1=m2.col1+5 where m1.id1 < (select count(concat(m1.col1, m2.col1)) from md_blob1 m1 inner join md_blob2 m2 on m1.id1=m2.id2);
select if (count(*) = 2, 'ok', 'nok') from md_blob1 where col1 is null;


--TEST: right outer join
--TEST: success, 4 rows updated
update md_blob1 m1 right outer join md_blob2 m2 on blob_to_bit(m1.col2)=blob_to_bit(m2.col2) set m1.col1='updated2', m2.col2=bit_to_blob(blob_to_bit(m1.col2)+B'101') where m1.id1 > (select count(concat(col1, 'a')) from md_blob2 m2 where id2 < 4);
select if (count(*) = 4, 'ok', 'nok') from md_blob1 where col1='updated2';
