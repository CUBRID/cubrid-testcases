--update 2 tables with set type column for join condition
...
update md_set1 m1 left outer join md_set2 m2 on m1.col1=m2.col2 set m1.col2=concat(trim(m1.col2), m2.col3), m1.col1=m2.col2*m1.col1, m2.col3='updated2' where m2.id > (select avg(id) from md_set2);
--TEST: check update result
select if (col2 = 'lllsqlite', 'ok', 'nok') from md_set1 where col1={2, 1, date'2010-12-12'};
select case when count(*)=1 then 'ok' else 'nok' end from md_set2 where col3='updated2';


--TEST: right outer join
--TEST: success, 10 rows updated
update md_set1 m1 right outer join md_set2 m2 on m1.col1=m2.col2 set m2.col3='updated3', m2.col2=m1.col1+m2.col2, m1.col2='updated4' where m2.col1 > (select min(id) from md_set2);
--TEST: check update result
select if (count(*) = 7, 'ok', 'nok') from md_set2 where col3='updated3';
select if (count(*) = 3, 'ok', 'nok') from md_set2 where col2 is null;
select if (count(*) = 3, 'ok', 'nok') from md_set1 where col2='updated4';
