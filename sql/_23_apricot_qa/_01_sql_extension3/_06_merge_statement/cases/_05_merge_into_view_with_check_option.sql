...
--create target view 
create view target_view as select * from target where id > 'db130' with check option;
...
--TEST: check option error, with insert clause, without update clause
merge into target_view t
using source_view s
on (t.email = s.email)
when not matched then
...
--TEST: with both update and insert clause with where conditions, 5 rows merged
merge into target_view t
using source_view s
on (t.email = s.email)
when matched then
...
--TEST: check merge result
select if(count(*)=0, 'ok', 'nok') from target_view where name in ('hhhh', 'eeee');
select if(count(*)=9, 'ok', 'nok') from target_view;
select if(count(*)=12, 'ok', 'nok') from target;
rollback;


--TEST: check option error, with delete clause
merge into target_view t
using source_view s
on (t.email = s.email)
when matched then
