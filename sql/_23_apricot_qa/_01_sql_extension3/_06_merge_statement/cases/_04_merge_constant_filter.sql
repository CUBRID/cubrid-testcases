--test with constant filter predicate,  e.g. ON (1 = 0)
...
--TEST: insert with no condition + where clause
merge into target t
using source s
on ('a' = 'aa')
when not matched then
...
--TEST: check merge result
select col1, col2, col3, col4, clob_to_char(col5) from target order by 1;
select count(*), if(count(*) = 23, 'ok', 'nok') from target;
rollback;


--TEST: equal constant condition + where clause
merge into target t
using source s
on ('a' = 'a')
when not matched then
...
--TEST: check merge result
select col1, col2, col3, col4, clob_to_char(col5) from target order by 1;
select count(*), if(count(*) = 14, 'ok', 'nok') from target;
rollback;



--TEST: insert with no condition + update clause
merge into target t
using source s
...
values(s.col2, s.col3, s.col4, s.col5);
--TEST: check merge result
select if(col2=555, 'nok', 'ok') from target;
rollback;
