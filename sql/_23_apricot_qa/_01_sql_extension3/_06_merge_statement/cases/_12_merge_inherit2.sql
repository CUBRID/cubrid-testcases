--test with inheritance: merge into a super/sub class, with delete clause
...
update
set p.col1=b.col1
delete where p.id < 10
when not matched then 
insert
values(b.id, b.col1);

select if (count(*) = 3, 'ok', 'nok') from sup where col1 in (111, 222);
select if (count(*) = 7, 'ok', 'nok') from sup;
...
select if (count(*) = 6, 'ok', 'nok') from sub;
...
update
set p.col3=t.col1
delete where p.id > 3 and t.id < 10
when not matched then
insert(p.id, p.col1, p.col3)
values(t.id, 999, t.col1);

select if (count(*)=3, 'ok', 'nok') from sup where col3='updated';
