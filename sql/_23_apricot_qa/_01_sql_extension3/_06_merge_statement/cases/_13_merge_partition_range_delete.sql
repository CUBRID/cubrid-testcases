--merge into range partition table, with delete clause
...
insert into t2 values('cubridcubrid', 6666666, '13:13:13'), ('mysqlmysql', 4000000, '12:12:16'), ('aaaa', 565656565, '12:12:12'), ('abcabcabc', 6000000, '12:12:19'), ('hello', 7788777, '14:14:14'), ('hellohello', 90000909, '12:12:17');
...
create trigger trigg4 after update on t2 execute insert into trigger_actions2(info) values ('updated');
create trigger trigg5 before insert on t2 execute insert into trigger_actions2(info) values ('inserted');
...
--TEST: error, no appropriate partition error in update clause, merge into range partition table
merge into t1
using t2
on (t1.id1=t2.id2)
when matched then
...
--TEST: error, no appropriate partition error in insert clause, merge into range partition table
merge into t1
using t2
on (t1.id1=t2.id2)
when matched then
...
delete where t2.col2='12:12:16'
when not matched then
insert
values('inserted', t1.id1, t1.col1);

select if(count(*)=12, 'ok', 'nok') from t2;
...
select if(count(*)=4, 'ok', 'nok') from t2__p__p3;
select if(count(*)=8, 'ok', 'nok') from trigger_actions2;
