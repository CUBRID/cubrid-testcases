...
--TEST: check merge result
select if(count(*)=13, 'ok', 'nok') from target_view;
rollback;
...
select * from source_view order by 1,2,3,4;
select * from target_view order by 1,2,3,4;
merge into target_view t
using source_view s
on (t.email = s.email)
when matched then
...
values(s.name, s.id, s.edate, s.email);
--TEST: check merge result
select * from target_view order by 1,2;
