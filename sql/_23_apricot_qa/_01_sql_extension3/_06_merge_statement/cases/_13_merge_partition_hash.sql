...
create table t2(col1 varchar(1024), id2 double, primary key(id2,col1))
...
select if (count(*)=20, 'ok', 'nok') from t1;
select if (count(*)=12, 'ok', 'nok') from trigger_actions1;
...
select if(count(*)=7, 'ok', 'nok') from t2 where col1='inserted';
select if(count(*)=19, 'ok', 'nok') from t2;
select if(count(*)=9, 'ok', 'nok') from trigger_actions2;
