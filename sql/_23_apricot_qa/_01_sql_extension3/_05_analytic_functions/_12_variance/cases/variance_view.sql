...
--create view
create view variance_view as select * from variance_collection where 1 > 0;


--TEST: OVER() clause
--TEST: set 
select col1, col2, variance(col5) over() from variance_view order by 1, 2;
--TEST: multiset, with alias+all
select col2, col3, variance(all col5) over() variance from variance_view order by 1, 2;
--TEST: list, with where clause
select col3, col4, variance(unique col5) over() variance from variance_view where col1 > {1, 2, 3} order by col3, col4;
--TEST: sequence, distinct
select col4, col1, variance(distinct col5) over() variance from variance_view order by 1, 2;
--TEST: syntax error select *, variance(col5) over() variance from count)numeric;
select col1, col3, col4, variance(distinctrow col5) over variance from variance_view;
select col1, col3, col2, variance(col5) over(1) variance from variance_view;



--TEST: OVER(PARTITION BY) clause
--TEST: partition by set
select col1, col2, col3, col4, variance(unique col5) over(partition by col1) variance from variance_view order by 1, 2, 3, 4;
--TEST: partition by multiset
select col1, col2, col3, col4, variance(col5) over(partition by col2) variance from variance_view order by 2, 1, 3, 4;
--TEST: partition by list
select col1, col2, col3, col4, variance(all col5) over(partition by col3) variance from variance_view order by 3, 1, 2, 4;
--TEST: partition by sequence
select col1, col2, col3, col4, variance(col5) over(partition by col4) variance from variance_view order by 4, 1, 2, 3, 5;



--TEST: OVER(ORDER BY) clause
--TEST: order by 1 column name
select col1, variance(distinct col5) over(order by col1) variance from variance_view;
--TEST: order by 2 column names
select col2, col3, variance(col5) over(order by col2, col3 asc) variance from variance_view;
--TEST: order by more than 2 column names
select col1, col2, col3, col4, variance(distinctrow col5) over(order by col1, col2 desc, col3, col4 asc) variance from variance_view;
--TEST: order by columns not selected
select col3, variance(col5) over(order by col4 desc, col2, col1 asc) variance from variance_view;
--TEST: order by 1 position
select col4, variance(all col5) over(order by 1) variance from variance_view;
--TEST: order by 3 positions
select col3, col2, variance(col5) over(order by 1, 2 desc, 3 asc) variance from variance_view;
--TEST: order by more than 3 positions
select col1, col2, col3, col4, variance(unique col5) over(order by 3, 2 asc, 1 desc, 4) variance from variance_view;
--TEST: order by positions not existed
select col2, variance(col5) over(order by 2) variance from variance_view;
select col1, variance(col5) over(order by 3, 4, 1 desc) variance, col2, col4 from variance_view;
select col3, col4, variance(col5) over(order by 5) variance from variance_view;
select col3, col1, variance(col5) over(order by 100) variance from variance_view;
--TEST: order by column names and positions
select col1, col2, col3, col4, variance(col5) over(order by 1, col2 desc, 3, col4 asc) variance from variance_view;
...
--TEST: partition by set
select col1, col2, col3, variance(unique col5) over(partition by col1 order by col1, col2, col3) from variance_view;
--TEST: partition by multiset
select col2, col4, col1, variance(col5) over(partition by col2 order by col2, col4, col1 desc) from variance_view;
--TEST: partition by list
select col3, variance(all col5) over(partition by col3 order by col1, col2, col3) variance, col2 from variance_view;
--TEST: partition by sequence
select col4, col1, variance(distinct col5) over(partition by col4 order by col1, col2 desc) variance from variance_view;
--TEST: syntax error
select col1, col2, variance(col5) over(order by col1, col2 partition by col2) from variance_view;


drop view variance_view;
