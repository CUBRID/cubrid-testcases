...
select cnt from (select *, count(*) over() cnt from count_collection) order by 1;
--TEST: syntax error
...
select cnt from (select col1, col2, col3, col4, count(*) over(partition by col1) cnt from count_collection) order by 1;
