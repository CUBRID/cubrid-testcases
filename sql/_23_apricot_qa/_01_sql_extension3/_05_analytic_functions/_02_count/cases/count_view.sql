...
--create view
create view count_view as select * from count_collection where 1 > 0;


--TEST: OVER() clause
--TEST: set 
select col1, col2, count(col1) over() from count_view order by 1, 2;
--TEST: multiset, with alias+all
select col2, col3, count(all col2) over() cnt from count_view order by 1, 2;
--TEST: list, with where clause
select col3, col4, count(col3) over() cnt from count_view where col1 > {1, 2, 3} order by col3, col4;
--TEST: sequence, distinct
select col4, col1, count(distinct col4) over() cnt from count_view order by 1, 2;
--TEST: syntax error
select col1, col3, col4, count(*) over cnt from count_view;
select col1, col3, col2, count(*) over(1) cnt from count_view;



--TEST: OVER(PARTITION BY) clause
--TEST: partition by set
select col1, col2, col3, col4, count(*) over(partition by col1) cnt from count_view order by 1, 2, 3, 4;
--TEST: partition by multiset
select col1, col2, col3, col4, count(distinct col4) over(partition by col2) cnt from count_view order by 2, 1, 3, 4;
--TEST: partition by list
select col1, col2, col3, col4, count(*) over(partition by col3) cnt from count_view order by 3, 1, 2, 4;
--TEST: partition by sequence
select col1, col2, col3, col4, count(all col2) over(partition by col4) cnt from count_view order by 4, 1, 2, 3, 5;



--TEST: OVER(ORDER BY) clause
--TEST: order by 1 column name
select col1, count(col2) over(order by col1) cnt from count_view;
--TEST: order by 2 column names
select col2, col3, count(all col4) over(order by col2, col3 asc) cnt from count_view;
--TEST: order by more than 2 column names
select col1, col2, col3, col4, count(*) over(order by col1, col2 desc, col3, col4 asc) cnt from count_view;
--TEST: order by columns not selected
select col3, count(distinct col2) over(order by col4 desc, col2, col1 asc) cnt from count_view;
--TEST: order by 1 position
select col4, count(all col2) over(order by 1) cnt from count_view;
--TEST: order by 3 positions
select col3, col2, count(*) over(order by 1, 2 desc, 3 asc) cnt from count_view;
--TEST: order by more than 3 positions
select col1, col2, col3, col4, count(*) over(order by 3, 2 asc, 1 desc, 4) cnt from count_view;
--TEST: order by positions not existed
select col2, count(distinct col1) over(order by 2) cnt from count_view;
select col1, count(all col3) over(order by 3, 4, 1 desc) cnt, col2, col4 from count_view;
select col3, col4, count(*) over(order by 5) cnt from count_view;
select col3, col1, count(*) over(order by 100) cnt from count_view;
--TEST: order by column names and positions
select col1, col2, col3, col4, count(*) over(order by 1, col2 desc, 3, col4 asc) cnt from count_view;
...
--TEST: partition by set
select col1, col2, col3, count(distinct col4) over(partition by col1 order by 1, 2, 3) from count_view;
--TEST: partition by multiset
select col2, col4, col1, count(col3) over(partition by col2 order by col2, col4, col1 desc) from count_view;
--TEST: partition by list
select col3, count(all col4) over(partition by col3 order by 1, 2, 3) cnt, col2 from count_view;
--TEST: partition by sequence
select col4, col1, count(*) over(partition by col4 order by 1, 2 desc) cnt from count_view;
--TEST: syntax error
select col1, col2, count(distinct col3) over(order by col1, col2 partition by col2) from count_view;



drop view count_view; 
