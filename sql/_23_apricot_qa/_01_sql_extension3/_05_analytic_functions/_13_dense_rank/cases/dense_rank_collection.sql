...
--TEST: OVER() clause
--TEST: no alias
select col1, col2, dense_rank() over() from dense_rank_collection order by 1, 2;
--TEST: with alias
select col2, col3, dense_rank() over() d_rank from dense_rank_collection order by 1, 2;
--TEST: with where clause
select col3, col4, dense_rank() over() d_rank from dense_rank_collection where col1 > {1, 2, 3} order by col3, col4;
--TEST: sequence
select col4, col1, dense_rank() over() d_rank from dense_rank_collection order by 1, 2;
--TEST: syntax error
select col1, col3, col4, dense_rank() over d_rank from dense_rank_collection;
select col1, col3, col2, dense_rank() over(1) d_rank from dense_rank_collection;



--TEST: OVER(PARTITION BY) clause
--TEST: partition by set
select col1, col2, col3, col4, dense_rank() over(partition by col1) d_rank from dense_rank_collection order by 1, 2, 3, 4;
--TEST: partition by multiset
select col1, col2, col3, col4, dense_rank() over(partition by col2) d_rank from dense_rank_collection order by 2, 1, 3, 4;
--TEST: partition by list
select col1, col2, col3, col4, dense_rank() over(partition by col3) d_rank from dense_rank_collection order by 3, 1, 2, 4;
--TEST: partition by sequence
select col1, col2, col3, col4, dense_rank() over(partition by col4) d_rank from dense_rank_collection order by 4, 1, 2, 3, 5;



--TEST: OVER(ORDER BY) clause
--TEST: order by 1 column name
select col1, dense_rank() over(order by col1) d_rank from dense_rank_collection;
--TEST: order by 2 column names
select col2, col3, dense_rank() over(order by col2, col3 asc) d_rank from dense_rank_collection;
--TEST: order by more than 2 column names
select col1, col2, col3, col4, dense_rank() over(order by col1, col2 desc, col3, col4 asc) d_rank from dense_rank_collection;
--TEST: order by columns not selected
select col3, dense_rank() over(order by col4 desc, col2, col1 asc) d_rank from dense_rank_collection;
--TEST: order by 1 position
select col4, dense_rank() over(order by 1) d_rank from dense_rank_collection;
--TEST: order by 3 positions
select col3, col2, dense_rank() over(order by 1, 2 desc, 3 asc) d_rank from dense_rank_collection;
--TEST: order by more than 3 positions
select col1, col2, col3, col4, dense_rank() over(order by 3, 2 asc, 1 desc, 4) d_rank from dense_rank_collection;
--TEST: order by positions not existed
select col2, dense_rank() over(order by 2) d_rank from dense_rank_collection;
select col1, dense_rank() over(order by 3, 4, 1 desc) rank, col2, col4 from dense_rank_collection;
select col3, col4, dense_rank() over(order by 5) d_rank from dense_rank_collection;
select col3, col1, dense_rank() over(order by 100) d_rank from dense_rank_collection;
--TEST: order by column names and positions
select col1, col2, col3, col4, dense_rank() over(order by 1, col2 desc, 3, col4 asc) d_rank from dense_rank_collection;
...
--TEST: partition by set
select col1, col2, col3, dense_rank() over(partition by col1 order by col1, col2, col3) from dense_rank_collection;
--TEST: partition by multiset
select col2, col4, col1, dense_rank() over(partition by col2 order by col2, col4, col1 desc) from dense_rank_collection;
--TEST: partition by list
select col3, dense_rank() over(partition by col3 order by col3 desc) rank, col2 from dense_rank_collection;
--TEST: partition by sequence
select col4, col1, dense_rank() over(partition by col4 order by 1, 2 desc) d_rank from dense_rank_collection;
--TEST: syntax error
select col1, col2, dense_rank() over(order by col1, col2 partition by col2) from dense_rank_collection;
