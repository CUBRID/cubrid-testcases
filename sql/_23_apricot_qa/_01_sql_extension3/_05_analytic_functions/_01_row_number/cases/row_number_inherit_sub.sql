...
--create subclass
create table rownum_string under rownum_super(col3 nchar(20), col4 nchar varying, col5 string);
...
select col3, row_number() over(order by col4 desc, col2, col1 asc, col3) row_num from rownum_string order by 1, 2;
--TEST: order by 1 position
select col4, row_number() over(order by 1) row_num from rownum_string order by 1, 2;
--TEST: order by 3 positions
select col3, col2, row_number() over(order by 1, 2 desc, 3 asc) row_num from rownum_string orderby 1, 2, 3;
...
select col3, col4, row_number() over(order by 5) row_num from rownum_string;
select col3, col1, row_number() over(order by 100) row_num from rownum_string;
...
--TEST: partition by char(n)
select col1, col2, col3, row_number() over(partition by col1 order by col1, col2, col3) from rownum_string;
--TEST: partition by varchar(n)
select col2, col4, col1, row_number() over(partition by col2 order by col2, col4, col1 desc) from rownum_string;
--TEST: partition by nchar(n)
select col3, row_number() over(partition by col3 order by col3) row_num, col2 from rownum_string;
--TEST: partition by nchar varying
select col4, col1, row_number() over(partition by col4 order by col1, col2 desc) row_num from rownum_string;
--TEST: partition by string
select col5, col2, col4, row_number() over(partition by col5 order by col4, 1, 2 asc) from rownum_string;
--TEST: syntax error
