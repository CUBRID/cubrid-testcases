...
--TEST: OVER() clause
--TEST: no alias 
select row_num from (select  col1, col2, row_number() over() row_num from rownum_bbc) order by 1;
--TEST: with alias
...
select col2, blob_to_bit(col3), row_number() over(order by col3) row_num from rownum_bbc order by 1, 2, 3;
--TEST: order by clob column
select col2, blob_to_bit(col3), clob_to_char(col4), row_number() over(order by col4 desc) row_num from rownum_bbc order by 1, 2, 3, 4;
...
select blob_to_bit(col3), row_number() over(order by clob_to_char(col4) desc, col2, col1 asc) row_num from rownum_bbc order by 1, 2;
