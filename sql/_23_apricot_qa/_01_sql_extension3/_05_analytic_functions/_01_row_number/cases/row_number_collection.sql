...
--TEST: OVER() clause
--TEST: no alias
select row_num from (select  col1, col2, row_number() over() row_num from rownum_collection) order by 1;
--TEST: with alias
select row_num from (select  col2, col3, row_number() over() row_num from rownum_collection) order by 1;
--TEST: with where clause
select row_num from (select  col3, col4, row_number() over() row_num from rownum_collection where col1 > {1, 2, 3}) order by 1;
--TEST: sequence
select row_num from (select  col4, col1, row_number() over() row_num from rownum_collection) order by 1;
select row_num from (select  *, row_number() over() row_num from rownum_collection) order by 1;
--TEST: syntax error
select col1, col3, col4, row_number() over row_num from rownum_collection;
select col1, col3, col2, row_number() over(1) row_num from rownum_collection;



--TEST: OVER(PARTITION BY) clause
--TEST: partition by set
select row_num from (select  col1, col2, col3, col4, row_number() over(partition by col1) row_num from rownum_collection) order by 1;
--TEST: partition by multiset
select row_num from (select  col1, col2, col3, col4, row_number() over(partition by col2) row_num from rownum_collection) order by 1;
--TEST: partition by list
select row_num from (select  col1, col2, col3, col4, row_number() over(partition by col3) row_num from rownum_collection) order by 1;
--TEST: partition by sequence
select row_num from (select  col1, col2, col3, col4, row_number() over(partition by col4) row_num from rownum_collection) order by 1;



--TEST: OVER(ORDER BY) clause
--TEST: order by 1 column name
select col1, row_number() over(order by col1) row_num from rownum_collection;
--TEST: order by 2 column names
select col2, col3, row_number() over(order by col2, col3 asc) row_num from rownum_collection;
--TEST: order by more than 2 column names
select col1, col2, col3, col4, row_number() over(order by col1, col2 desc, col3, col4 asc) row_num from rownum_collection;
--TEST: order by columns not selected
select col3, row_number() over(order by col4 desc, col2, col1 asc) row_num from rownum_collection;
--TEST: order by 1 position
select col4, row_number() over(order by 1) row_num from rownum_collection;
--TEST: order by 3 positions
select col3, col2, row_number() over(order by 1, 2 desc, 3 asc) row_num from rownum_collection;
--TEST: order by more than 3 positions
select col1, col2, col3, col4, row_number() over(order by 3, 2 asc, 1 desc, 4) row_num from rownum_collection;
--TEST: order by positions not existed
select col2, row_number() over(order by 2) row_num from rownum_collection;
select col1, row_number() over(order by 3, 4, 1 desc) row_num, col2, col4 from rownum_collection;
select col3, col4, row_number() over(order by 5) row_num from rownum_collection;
select col3, col1, row_number() over(order by 100) row_num from rownum_collection;
--TEST: order by column names and positions
select col1, col2, col3, col4, row_number() over(order by 1, col2 desc, 3, col4 asc) row_num from rownum_collection;
...
--TEST: partition by set
select col1, col2, col3, row_number() over(partition by col1 order by col1, col2, col3) from rownum_collection;
--TEST: partition by multiset
select col2, col4, col1, row_number() over(partition by col2 order by col2, col4, col1 desc) from rownum_collection;
--TEST: sorting confliction
select col3, row_number() over(partition by col3 order by col3 desc) row_num, col2 from rownum_collection;
--TEST: partition by sequence
select col4, col1, row_number() over(partition by col4 order by 1, 2 desc) row_num from rownum_collection;
--TEST: syntax error
select col1, col2, row_number() over(order by col1, col2 partition by col2) from rownum_collection;
