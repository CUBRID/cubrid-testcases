...
--TEST: OVER() clause
--TEST: no alias
select col1, col2, rank() over() from rank_collection order by 1, 2;
--TEST: with alias
select col2, col3, rank() over() rank from rank_collection order by 1, 2;
--TEST: with where clause
select col3, col4, rank() over() rank from rank_collection where col1 > {1, 2, 3} order by col3, col4;
--TEST: sequence
select col4, col1, rank() over() rank from rank_collection order by 1, 2;
--TEST: syntax error
select rank from (select *, rank() over() rank from rank_collection) order by 1;
select col1, col3, col4, rank() over rank from rank_collection;
select col1, col3, col2, rank() over(1) rank from rank_collection;



--TEST: OVER(PARTITION BY) clause
--TEST: partition by set
select col1, col2, col3, col4, rank() over(partition by col1) rank from rank_collection order by 1, 2, 3, 4;
--TEST: partition by multiset
select col1, col2, col3, col4, rank() over(partition by col2) rank from rank_collection order by 2, 1, 3, 4;
--TEST: partition by list
select col1, col2, col3, col4, rank() over(partition by col3) rank from rank_collection order by 3, 1, 2, 4;
--TEST: partition by sequence
select col1, col2, col3, col4, rank() over(partition by col4) rank from rank_collection order by 4, 1, 2, 3, 5;



--TEST: OVER(ORDER BY) clause
--TEST: order by 1 column name
select col1, rank() over(order by col1) rank from rank_collection;
--TEST: order by 2 column names
select col2, col3, rank() over(order by col2, col3 asc) rank from rank_collection;
--TEST: order by more than 2 column names
select col1, col2, col3, col4, rank() over(order by col1, col2 desc, col3, col4 asc) rank from rank_collection;
--TEST: order by columns not selected
select col3, rank() over(order by col4 desc, col2, col1 asc) rank from rank_collection;
--TEST: order by 1 position
select col4, rank() over(order by 1) rank from rank_collection;
--TEST: order by 3 positions
select col3, col2, rank() over(order by 1, 2 desc, 3 asc) rank from rank_collection;
--TEST: order by more than 3 positions
select col1, col2, col3, col4, rank() over(order by 3, 2 asc, 1 desc, 4) rank from rank_collection;
--TEST: order by positions not existed
select col2, rank() over(order by 2) rank from rank_collection;
select col1, rank() over(order by 3, 4, 1 desc) rank, col2, col4 from rank_collection;
select col3, col4, rank() over(order by 5) rank from rank_collection;
select col3, col1, rank() over(order by 100) rank from rank_collection;
--TEST: order by column names and positions
select col1, col2, col3, col4, rank() over(order by 1, col2 desc, 3, col4 asc) rank from rank_collection;
...
--TEST: partition by set
select col1, col2, col3, rank() over(partition by col1 order by col1, col2, col3) from rank_collection;
--TEST: partition by multiset
select col2, col4, col1, rank() over(partition by col2 order by col2, col4, col1 desc) from rank_collection;
--TEST: partition by list
select col3, rank() over(partition by col3 order by col3 desc) rank, col2 from rank_collection;
--TEST: partition by sequence
select col4, col1, rank() over(partition by col4 order by 1, 2 desc) rank from rank_collection;
--TEST: syntax error
select col1, col2, rank() over(order by col1, col2 partition by col2) from rank_collection;
