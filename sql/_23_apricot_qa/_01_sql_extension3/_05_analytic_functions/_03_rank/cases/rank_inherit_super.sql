...
--create super class
create table rank_super(
	col1 int not null
);


create table rank_constraints under rank_super(
...
--create sub class
create table rank_sub1 under rank_constraints(col8 char(100), col9 set(int));
create table rank_sub2 under rank_constraints(col10 bit, col11 clob);
...
--TEST: syntax error
select rank from (select *, rank() over() rank from rank_constraints) order by 1;
...
select col1, rank() over(order by col1) rank from rank_constraints;
--TEST: order by 2 column names
select col2, col3, rank() over(order by col2, col3 asc) rank from rank_constraints;
--TEST: order by more than 2 column names
select col1, col2, col3, col4, col5, col6, col7, rank() over(order by col1, col2 desc, col3, col4 asc, col5, col6 desc) rank from rank_constraints;
--TEST: order by columns not selected
select col3, rank() over(order by col4, col6 desc, col2, col1 asc) rank from rank_constraints;
--TEST: order by 1 position
select col5, rank() over(order by 1) rank from rank_constraints;
--TEST: order by 3 positions
select col3, col5, rank() over(order by 1, 2 desc, 3 asc) rank from rank_constraints;
--TEST: order by more than 3 positions
select col1, col2, col3, col4, col5, col6, col7, rank() over(order by 5, 3, 7, 2 asc, 1 desc, 6 desc, 4) rank from rank_constraints;
--TEST: order by positions not existed
select col5, rank() over(order by 2) rank from rank_constraints;
select col1, rank() over(order by 3, 4, 1 desc) rank, col5, col7 from rank_constraints;
select col3, col5, rank() over(order by 5) rank from rank_constraints;
select col3, col5, rank() over(order by 100) rank from rank_constraints;
--TEST: order by column names and positions
select col1, col2, col3, col4, col5, col6, col7, rank() over(order by 1, col2 desc, 3, col4 asc, 5, col6 desc, 7) rank from rank_constraints;
...
--TEST: partition by int
select col1, col2, col3, rank() over(partition by col1 order by 1, 2, 3) from rank_constraints;
--TEST: partition by smallint
select col2, col4, col5, rank() over(partition by col2 order by col2, col4, col5 desc) from rank_constraints;
--TEST: partition by bigint
select col3, rank() over(partition by col3 order by 1, 2, 3) rank, col5 from rank_constraints;
--TEST: partition by numeric
select col4, col1, rank() over(partition by col4 order by 1, 2 desc) rank from rank_constraints;
--TEST: partition by float
select col5, col6, rank() over(partition by col6 order by col5 desc, col6 asc) rank from rank_constraints;
--TEST: partition by double
select col6, col1, col2, col3, rank() over(partition by col5 order by col6) rank from rank_constraints;
--TEST: partition by monetary
select col7, col1, col2, col3, col4, col5, col6, col7, rank() over(partition by col7 order by col1, col2, col3 asc, col6) rank from rank_constraints;
--TEST: syntax error
select col1, col2, rank() over(order by col1, col2 partition by col2) from rank_constraints;



drop table rank_super;
drop table rank_constraints; 
drop table rank_sub1, rank_sub2;
