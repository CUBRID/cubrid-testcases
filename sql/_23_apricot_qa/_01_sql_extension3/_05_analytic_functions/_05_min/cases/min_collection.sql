...
select col3, col4, min(unique col5) over() minimum from min_collection where col1 > {1, 2, 3} order by col3, col4;
--TEST: sequence, distinct
select col4, col1, min(distinct col5) over() minimum from min_collection order by 1, 2;
--TEST: syntax error
select minimum from (select *, min(col5) over() minimum from min_collection) order by 1;
select col1, col3, col4, min(distinctrow col5) over minimum from min_collection;
select col1, col3, col2, min(col5) over(1) minimum from min_collection;
--TEST: wrong type of arguments
select min(col1) over() from min_collection;
select min(col2) over() from min_collection;
select min(col3) over() from min_collection;
select min(col4) over() from min_collection;
...
select col1, col2, col3, col4, min(col5) over(partition by col2) minimum from min_collection order by 2, 1, 3, 4;
--TEST: partition by list
select col1, col2, col3, col4, min(all col5) over(partition by col3) minimum from min_collection order by 3, 1, 2, 4;
--TEST: partition by sequence
select col1, col2, col3, col4, min(col5) over(partition by col4) minimum from min_collection order by 4, 1, 2, 3, 5;



--TEST: OVER(ORDER BY) clause
--TEST: order by 1 column name
select col1, min(distinct col5) over(order by col1) minimum from min_collection order by 1, 2;
--TEST: order by 2 column names
select col2, col3, min(col5) over(order by col2, col3 asc) minimum from min_collection;
--TEST: order by more than 2 column names
select col1, col2, col3, col4, min(distinctrow col5) over(order by col1, col2 desc, col3, col4 asc) minimum from min_collection;
--TEST: order by columns not selected
select col3, min(col5) over(order by col4 desc, col2, col1 asc) minimum from min_collection;
--TEST: order by 1 position
select col4, min(all col5) over(order by 1) minimum from min_collection;
--TEST: order by 3 positions
select col3, col2, min(col5) over(order by 1, 2 desc, 3 asc) minimum from min_collection;
--TEST: order by more than 3 positions
select col1, col2, col3, col4, min(unique col5) over(order by 3, 2 asc, 1 desc, 4) minimum from min_collection;
--TEST: order by positions not existed
select col2, min(col5) over(order by 2) minimum from min_collection;
select col1, min(col5) over(order by 3, 4, 1 desc) minimum, col2, col4 from min_collection;
...
--TEST: partition by set
select col1, col2, col3, min(unique col5) over(partition by col1 order by col1, col2, col3) from min_collection;
--TEST: partition by multiset
select col2, col4, col1, min(col5) over(partition by col2 order by col2, col4, col1 desc) from min_collection;
--TEST: partition by list
select col3, min(all col5) over(partition by col3 order by col1, col2, col3) minimum, col2 from min_collection;
--TEST: partition by sequence
select col4, col1, min(distinct col5) over(partition by col4 order by col1, col2 desc) minimum from min_collection;
--TEST: syntax error
select col1, col2, min(col5) over(order by col1, col2 partition by col2) from min_collection;
