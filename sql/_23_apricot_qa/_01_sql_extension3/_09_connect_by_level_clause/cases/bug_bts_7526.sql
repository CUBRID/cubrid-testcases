...
select *,level from t where level % 2 = 1 start with a=1 connect by prior b=a and level < 4 order by 1,2;
...
-- error cases
select * from t start with a=1 connect by prior b=a and connect_by_isleaf=1 order by 1,2;
select * from t start with a=1 connect by prior b=a and connect_by_iscycle=0 order by 1,2;

truncate t;
insert into t values (1,1);
-- check ignore cycles
select *,level from t connect by level < 10 order by 1,2,3;
select *,level from t connect by nocycle level < 10 order by 1,2,3;
select *,level from t connect by level > -1 order by 1,2,3;
select *,level from t connect by level < 10 or level > -1 order by 1,2,3;
select *,level from t connect by level < 10 or prior a = a order by 1,2,3;
