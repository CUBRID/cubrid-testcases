...
select translate (s1,'伽','가') from t1 order by 1;
select translate (s1,'가','伽') from t2 order by 1;
...
prepare s from 'select translate(?, s1 , ?) from t1 order by 1';
execute s using 'aa가b伽abb','XX';
DEALLOCATE PREPARE s;

prepare s from 'select translate(?, s1 , ?) from t2 order by 1';
execute s using 'aa伽ba가bb','XX';
DEALLOCATE PREPARE s;

select translate(t1.s1, t2.s1, 'x') from t1,t2 order by 1;

select translate(t1.s1, cast (t2.s1 as string collate utf8_ko_cs_uca),'X') from t1,t2 order by 1;
