...
CREATE TABLE test_kr ( 
  id     int    NOT NULL, 
  name      CHAR(20) collate utf8_ko_cs_uca);
...
select id,name from test_kr where name < '개객갱갹' order by 1;

select id,name from test_kr where name <= '보병별변벽법 ' order by 1;

select id,name from test_kr where name <= ' ' order by 1;

select id,name from test_kr where name < '집징 ' order by 1;

select id,name from test_kr where name < '_ ' order by 1;
