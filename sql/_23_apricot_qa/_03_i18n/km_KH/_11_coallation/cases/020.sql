...
INSERT INTO t1 VALUES (null,'ឯកកតឰឰហថ','ឯៀៀតឰឰហថ啊你','2011-01-02', '17:10:32', ' 2011-01-02 17:10:32');
INSERT INTO t1 VALUES ('ឯកកតឰឰហថ',null,null,'2011-01-02', '17:10:32', ' 2011-01-02 17:10:32');

create Reverse unique index t1_index on t1(col2) where col1>='ហឡឡតឰឿហថ';
--test
SELECT /*+ recompile */* FROM t1 where col1>='ហឡឡតឰឿហថ' order by 1,2;
--test
SELECT /*+ recompile */col2 FROM t1 where col1>='ហឡឡតឰឿហថ' order by col2;

UPDATE t1 SET col2='ឯៀៀតឰឰហថ啊你' WHERE col2='ហឡកឮ';

DELETE  FROM t1 WHERE col2='ឯកកតឰឰហថ';
--test
SELECT /*+ recompile */index_name, is_unique, is_reverse, class_name, owner_name, key_count, is_primary_key, is_foreign_key, filter_expression, have_function, status, index_type, deduplicate_key_level, comment FROM db_index WHERE class_name='t1' where col1>='ហឡឡតឰឿហថ' order by 1,2;
--test
SELECT /*+ recompile */col2 FROM t1 where col1>='ហឡឡតឰឿហថ' order by col2;
