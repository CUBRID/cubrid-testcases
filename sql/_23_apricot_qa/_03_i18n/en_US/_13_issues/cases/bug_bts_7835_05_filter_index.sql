...
create index i_t1_f_s1 on t1(s1) where locate('İ',s1) > 1;
insert into t1 values ('İacb');
insert into t1 values ('aİbc');
insert into t1 values ('accİ');

-- index scan
select /* +recompile */ s1 from t1 where locate('İ',s1) > 1 using index i_t1_f_s1(+) order by 1;

set names iso88591;
-- no index scan
select /* +recompile */ s1 from t1 where locate('İ',s1) > 1 using index i_t1_f_s1(+) order by 1;
-- force index scan
select /* +recompile */ s1 from t1 where locate(_utf8'İ',s1) > 1 using index i_t1_f_s1(+) order by 1;
