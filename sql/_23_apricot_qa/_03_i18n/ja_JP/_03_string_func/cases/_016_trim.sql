...
select id, name, trim ('午' from name) from test_jp order by 1;

select id, name, trim ('.' from name) from test_jp order by 1;

select id, name, trim (TRAILING '語' from name) from test_jp order by 1;

select id, name, rtrim (name,'紀') from test_jp order by 1;

select id, name, trim (LEADING '暦' from name) from test_jp order by 1;

select id, name, ltrim (name,'？') from test_jp order by 1;
select id, name, ltrim (name,'?') from test_jp order by 1;
