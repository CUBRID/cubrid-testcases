...
select id,name from test_tr where name = SOME {'İzgÖ', ' '} order by 1;


select id,name from test_tr where name >= ALL{'Şükran', 'İzğü'} order by 1;

select id,name from test_tr where name <> ALL {'Gülen', 'İzzettin', 'İzgi', 'SevINÇ'} order by 1;
