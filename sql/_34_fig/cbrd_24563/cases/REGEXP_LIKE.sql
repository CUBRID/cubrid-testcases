...
SELECT name FROM athlete WHERE REGEXP_LIKE(name, '^F[[:alpha:]]+ A.+')<>0 order by 1;

DROP TABLE IF EXISTS many_word_name;
CREATE TABLE many_word_name ( name VARCHAR ) AS SELECT name FROM athlete WHERE REGEXP_LIKE(name, '^F[[:alpha:]]+ A.+')<>0;
SELECT * FROM many_word_name order by 1;

DROP TABLE IF EXISTS many_word_name;
CREATE TABLE many_word_name ( name VARCHAR ); 
INSERT INTO many_word_name SELECT name FROM athlete WHERE REGEXP_LIKE(name, '^F[[:alpha:]]+ A.+')<>0;
SELECT * FROM many_word_name order by 1;
