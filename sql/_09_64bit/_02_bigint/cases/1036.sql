...
select char_col, STDDEV(bigint_col) from test_class group by char_col order by 1;
select char_col, STDDEV(DISTINCT bigint_col) from test_class group by char_col order by 1;
select char_col, STDDEV(UNIQUE bigint_col) from test_class group by char_col order by 1;
select char_col, STDDEV(ALL bigint_col) from test_class group by char_col order by 1;
