...
create table t(id int primary key auto_increment not null, a json);
show create table t;
alter table t modify a json not null;
show create table t;
create trigger tr_t  BEFORE STATEMENT INSERT ON t execute insert into t(a) values(json_array('a'));
insert into t(a) values(json_object('a', 'b'));
...
replace into t(a) values(json_merge(default(a), json_array('a')));
select * from t order by id ;
