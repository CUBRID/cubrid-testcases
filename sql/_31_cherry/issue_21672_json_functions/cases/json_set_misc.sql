...
select * from t10 where json_array_append(json_set(name, '$', '{"bar": "baz", "balance": 7.77, "active":"false"}'), '$.bar', '1') is not null order by id;
drop table if exists t;
create table t(id int, name json);
insert into t values(1, @j);
select * from t;
update t set name = json_set(name, '$.b', @j); 
update t set name = json_set(name, '$.c', @j);  
select * from t;
