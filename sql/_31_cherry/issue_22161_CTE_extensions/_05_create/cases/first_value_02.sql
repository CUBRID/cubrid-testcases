...
SELECT groupid, itemno, FIRST_VALUE(itemno) IGNORE NULLS OVER(PARTITION BY groupid ORDER BY itemno) AS ret_val
FROM test_tbl
) delete from foo2 using foo2,
(
SELECT groupid, itemno, FIRST_VALUE(itemno) OVER(PARTITION BY groupid ORDER BY itemno) AS ret_val
FROM cte1
 ) as x( groupid, itemno,ret_val),(SELECT groupid, itemno, FIRST_VALUE(itemno) IGNORE NULLS OVER(PARTITION BY groupid ORDER BY itemno) AS ret_val
FROM cte2 ) as y( groupid, itemno,ret_val)
where foo2.groupid=x.groupid and x.groupid=y.groupid
and
foo2.ret_val=y.ret_val;
