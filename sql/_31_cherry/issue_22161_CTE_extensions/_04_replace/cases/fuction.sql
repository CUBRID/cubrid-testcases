...
with cte(c) as
(
select greatest(e, e, e, e ) from t1 order by 1
)select * from cte ;
...
with cte(c,d) as
(
select least(e, e, e, 'a'), least(e, e, e, 'x') from t1 order by 1, 2
)select c from cte ;

replace into foo
with cte(c) as
(
 select least(e, e, e, 'x') from t1 order by 1
)select c from cte ;
...
--with cte(c) as
--(
-- select least(e, e, e, 2) from t1 order by 1
--)select c from cte ;
...
with cte(c) as
(
select least(e, e, e, cast(e as varchar)) from t1 order by 1
)select * from cte ;
...
(
select nvl2(e, e, e), nvl2(null, e, e) from t1 order by 1, 2
) delete from foo where c in (select c from cte) ;
select * from foo  order by 1;
...
(
select e, md5(e), md5(cast(e as varchar)) from t1 order by 1, 2, 3
) delete from foo where c in (select f from cte);
select * from foo  order by 1;
drop table if exists t1,foo;
