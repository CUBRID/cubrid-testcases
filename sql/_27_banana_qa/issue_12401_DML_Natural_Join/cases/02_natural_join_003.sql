...
select * from (select * from t1 order by 1,2) t1 natural join (select * from t2 order by 1,2) t2 group by d1 order by 1;

select f1,i1+2 from (select * from t1 order by 1,2)  t1 natural join (select * from t2 order by 1,2)  t2 group by d1 order by 1;
prepare st from 'select f1,i1+? from t1 natural join t2 group by d1 order by 1';
execute st using 2;
...
select /*+ RECOMPILE*/ * from (select * from t1 order by 1,2) t1 natural left join (select * from t2 order by 1) t2 order by 1;
...
select /*+ RECOMPILE*/ * from (select i1+1 from t1 order by 1) t1 natural join (select i2 as x from t2 order by 1) t2 order by 1;
...
select /*+ RECOMPILE*/ * from (select i1+1 as i2 from t1 order by 1) t1 natural join (select i2 as i1 from t2 order by 1) t2 order by 1;
