...
set timezone '+9:00';
--test: union, t+dttz
select id, t from (select id,t,to_char(t) from t1 order by 1,3 desc) union all select id, dttz as t from t2 order by 1, 2;
...
select id, dtltz as t from (select id,dtltz,to_char(dtltz) from t1 order by 1,3 desc) union all select id, dttz as t from t2 order by 1, 2;
