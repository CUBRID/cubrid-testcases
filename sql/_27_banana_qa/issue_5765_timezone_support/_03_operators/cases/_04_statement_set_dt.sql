...
set timezone 'Asia/Seoul';
--test: union, ts+dttz
select id,to_char(ts,'YYYY-MM-DD HH24:MI:SS.FF TZR TZD') from (select id, ts from t1 union all select id, dttz as ts from t2 order by 1, 2);
select id,to_char(ts,'YYYY-MM-DD HH24:MI:SS.FF TZR TZD') from (select id, dttz as ts from t2 union select id, ts from t1 order by 1, 2);
--test: union, dtltz+dttz
select id,to_char(ts,'YYYY-MM-DD HH24:MI:SS.FF TZR TZD') from (select id, dtltz as ts from t1 union all select id, dttz as ts from t2 order by 1, 2);
select id,to_char(ts,'YYYY-MM-DD HH24:MI:SS.FF TZR TZD') from (select id, dttz as ts from t2 union select id, dtltz as ts from t1 order by 1, 2);
--test: union, ts+dtltz
