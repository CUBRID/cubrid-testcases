-- create table of hash partition having size 4 on date field that using extract function get day and query all partitions
...
create table hash_test(id int not null,	
	...
	PARTITION BY HASH (extract(day from test_date))
PARTITIONS 4;
...
update hash_test set test_date = '2008-01-10' where test_date < '2000-01-01';
select * from hash_test__p__p0 order by id;
select * from hash_test__p__p1 order by id;
select * from hash_test__p__p2 order by id;
select * from hash_test__p__p3 order by id;
