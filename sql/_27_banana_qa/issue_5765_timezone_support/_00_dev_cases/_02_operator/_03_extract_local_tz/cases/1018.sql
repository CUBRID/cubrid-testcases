-- insert data  whith create range partition table on a date field witch using function of extract month and then query all partitions and drop table
set timezone '+9:00';
...
create table range_test(id int not null,	
	...
	PARTITION BY RANGE (extract(month from test_date)) (
...
update range_test set test_date = '2008-12-01' where test_date < '2000-01-21';
select * from range_test__p__p0 order by id;
select * from range_test__p__p1 order by id;
select * from range_test__p__p2 order by id;
select * from range_test__p__p3 order by id;
