...
alter table ltz add c_tsltz1 datetime with local time zone default '2003-01-01 2:00:00.789 -8:00';
desc ltz;
select * from ltz order by a;

--test: [er] add column with not null, without default
alter table ltz add c_tsltz2 datetime with local time zone not null;

--test: add column with unique
alter table ltz add c_tsltz3 datetime with local time zone unique;
show columns in ltz;
select * from ltz order by a;

--test: [er] add column with unique and not null
alter table ltz add c_tsltz4 datetime with local time zone unique not null;
show columns in ltz;
select * from ltz order by a;

--test: [er] add column with unique and default
alter table ltz add c_tsltz5 datetime with local time zone default '2003-01-01 2:00:00.789 -8:00' unique;
show columns in ltz;
select * from ltz order by a;

--test: [er] add column with primary key
alter table ltz add c_tsltz4 datetime with local time zone primary key;
show columns in ltz;
select * from ltz order by a;
