...
select * from tz_test1 t1, tz_test2 t2 where t1.ts=t2.ts order by t1.id;
--test: inner join, tsltz & tsltz
select t2.id, t1.tsltz, t2.tsltz, t1.tstz from tz_test1 t1 inner join tz_test2 t2 on t1.tsltz=t2.tsltz order by 1;
--test: inner join, tstz & tstz
select t1.id, t2.tstz, t1.tstz, t1.ts aa, t2.tsltz bb from tz_test1 t1, tz_test2 t2 where t1.tstz=t2.tstz and t2.tstz < timestamptz'2000-12-1 00:00:00';

--test: left outer join, ts & tsltz
select * from tz_test1 t1 left outer join tz_test2 t2 on t1.ts=t2.tsltz order by 1;
--test: left outer join, tstz & tsltz
select t1.id, t1.ts, t2.tstz, t1.tsltz, t2.tsltz from tz_test2 t2 left outer join tz_test1 t1 on t1.tsltz=t2.tstz where t2.tsltz > timestamptz'1990-05-01 13:14:15 -10:00' order by t2.tstz, t1.id;
...
select t2.id, t1.tstz, t2.ts, t1.ts, t2.tsltz from tz_test1 t1 right outer join tz_test2 t2 on t1.tsltz=t2.tsltz where year(t1.tstz) < 2010;

--test: mix
select t1.id, t1.tsltz, t2.tstz, t3.tstz, t1.ts, t2.tsltz, t3.tsltz from tz_test1 t1 right outer join tz_test2 t2 on t1.tsltz=t2.tstz inner join tz_test1 t3 on t2.tstz=t3.tstz order by 1, 2, 3, 4, 5, 6;
