--test triggers with DATETIMELTZ column

autocommit off;

drop table if exists tz_test;
create table tz_test(id int, dtltz datetime with local time zone primary key, str varchar(20)) partition by hash(dtltz) partitions 5;

--test: create trigger action table
create table trigger_actions(id int auto_increment, dtltz datetime with local time zone unique, str varchar(20)) partition by hash(dtltz) partitions 5;
--test: create triggers
create trigger bef_del1 before delete on tz_test 
	execute	insert into trigger_actions(dtltz, str) values((select max(dtltz) from trigger_actions)+1*1000, 'before delete');
...
create trigger aft_del1 after delete on tz_test 
        execute insert into trigger_actions(dtltz, str) values((select max(dtltz) from trigger_actions)+1*1000, 'after delete');
...
create trigger bef_ins1 before insert on tz_test
	execute insert into trigger_actions(dtltz, str) values((select max(dtltz) from trigger_actions)+1*1000, 'before insert');
...
create trigger aft_ins1 before insert on tz_test
        execute insert into trigger_actions(dtltz, str) values((select max(dtltz) from trigger_actions)+1*1000, 'after insert');
...
create trigger bef_upd1 before update on tz_test
	execute insert into trigger_actions(dtltz, str) values((select max(dtltz) from trigger_actions)+1*1000, 'before update');
...
create trigger aft_upd1 before update on tz_test
	execute insert into trigger_actions(dtltz, str) values((select max(dtltz) from trigger_actions)+1*1000, 'after update');
...
create trigger aft_upd3 before update on tz_test
	if new.dtltz < obj.dtltz execute reject;

insert into trigger_actions(dtltz, str) values(datetimeltz'2000-12-12 21:00:00.999 +5:00', 'initialization');

commit;

set timezone '+9:00';
--test: insert
insert into tz_test values(1, datetimeltz'2025-12-31 14:00:00.999', 'inserted');
...
update tz_test set dtltz=dtltz-1;
select if(count(*)=0, 'ok', 'nok') from trigger_actions where str in ('before update', 'after update');

--test: replace
replace into tz_test values(2, datetimeltz'2025-12-31 14:00:00.999', 'replaced');
select * from tz_test order by 1;
...
insert into tz_test values(3, datetimeltz'2025-12-31 14:00:00.999', 'odku') on duplicate key update dtltz=dtltz-1, str='odku';
select * from tz_test order by 1;
select if(count(*)=7, 'ok', 'nok') from trigger_actions;
select if(count(*)=0, 'ok', 'nok') from trigger_actions where str in ('before update', 'after update');

--test: odku
insert into tz_test values(3, datetimeltz'2025-12-31 14:00:00.999', 'odku') on duplicate key update dtltz=dtltz+1*1000, str='odku';
select if(count(*)=2, 'ok', 'nok') from trigger_actions where str in ('before update', 'after update');

--test: merge
create table source(id int, dtltz datetime with local time zone, str varchar(100));
insert into source values(11, datetimeltz'2025-12-31 14:00:01.999', 'from source updated');
insert into source values(22, datetimeltz'2025-12-31 14:01:01.999', 'from source inserted');
...
insert values (s.id, s.dtltz, s.str);
