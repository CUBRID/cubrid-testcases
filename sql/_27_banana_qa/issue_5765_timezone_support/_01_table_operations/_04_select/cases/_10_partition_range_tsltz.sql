--test range partition on TIMESTAMPLTZ colum

drop table if exists tz_test;

set timezone '+00:00';
create table tz_test(id int, tsltz timestamp with local time zone, str varchar(50)) 
partition by range(tsltz)
...
create index idx on tz_test__p__p1(tsltz);
--test: create index on partition
create index idx on tz_test(tsltz);
...
update tz_test set tsltz=timestamptz'1990-06-01 10:30:21', str='moved from p2 to p3', id=333 where id=222;
select * from tz_test__p__p2 order by 1, 2, 3;
select * from tz_test__p__p3 order by 1, 2, 3;

update tz_test set id=44, tsltz=timestamptz'1990-06-01 13:59:59 Europe/Riga', str='moved from p1 to p4' where tsltz=timestamptz'1990-05-31 3:00:00 +2:00';
select * from tz_test__p__p1 order by 1, 2, 3;
select * from tz_test__p__p4 order by 1, 2, 3;

update tz_test set tsltz=tsltz-10801, str='moved from p2 to p1' where id in (2, 22);
select * from tz_test__p__p1 order by 1, 2, 3;
select * from tz_test__p__p2 order by 1, 2, 3;

update tz_test set tsltz=tsltz+1, str='added 1 sec' where str in ('p2', 'p3');
select * from tz_test order by 1, 2, 3;
select * from tz_test__p__p1 order by 1, 2, 3;
select * from tz_test__p__p2 order by 1, 2, 3;
select * from tz_test__p__p3 order by 1, 2, 3;
...
update tz_test set tsltz=tsltz-1, str='substract 1 sec from p1' where id=11;
select * from tz_test__p__p1 order by 1, 2, 3;
