--test triggers with TIMESTAMPTZ column

autocommit off;

drop table if exists tz_test;
create table tz_test(id int, tstz timestamp with time zone primary key, str varchar(20)) partition by hash(tstz) partitions 5;

--test: create trigger action table
create table trigger_actions(id int auto_increment, tstz timestamp with time zone unique, str varchar(20)) partition by hash(tstz) partitions 5;
--test: create triggers
create trigger bef_del1 before delete on tz_test 
	execute	insert into trigger_actions(tstz, str) values((select max(tstz) from trigger_actions)+1, 'before delete');
...
create trigger aft_del1 after delete on tz_test 
        execute insert into trigger_actions(tstz, str) values((select max(tstz) from trigger_actions)+1, 'after delete');
...
create trigger bef_ins1 before insert on tz_test
	execute insert into trigger_actions(tstz, str) values((select max(tstz) from trigger_actions)+1, 'before insert');
...
create trigger aft_ins1 before insert on tz_test
        execute insert into trigger_actions(tstz, str) values((select max(tstz) from trigger_actions)+1, 'after insert');
...
create trigger bef_upd1 before update on tz_test
	execute insert into trigger_actions(tstz, str) values((select max(tstz) from trigger_actions)+1, 'before update');
...
create trigger aft_upd1 before update on tz_test
	execute insert into trigger_actions(tstz, str) values((select max(tstz) from trigger_actions)+1, 'after update');
...
create trigger aft_upd3 before update on tz_test
	if new.tstz < obj.tstz execute reject;

insert into trigger_actions(tstz, str) values(timestamptz'2000-12-12 21:00:00 +5:00', 'initialization');

commit;

set timezone '+9:00';
--test: insert
insert into tz_test values(1, timestamptz'2025-12-31 14:00:00', 'inserted');
...
update tz_test set tstz=tstz-1;
select if(count(*)=0, 'ok', 'nok') from trigger_actions where str in ('before update', 'after update');

--test: replace
replace into tz_test values(2, timestamptz'2025-12-31 14:00:00', 'replaced');
select * from tz_test order by 1;
...
insert into tz_test values(3, timestamptz'2025-12-31 14:00:00', 'odku') on duplicate key update tstz=tstz-1, str='odku';
select * from tz_test order by 1;
...
--test: merge
create table source(id int, tstz timestamp with time zone, str varchar(100));
insert into source values(11, timestamptz'2025-12-31 14:00:01', 'from source updated');
insert into source values(22, timestamptz'2025-12-31 14:01:01', 'from source inserted');
...
insert values (s.id, s.tstz, s.str);
