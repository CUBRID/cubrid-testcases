...
create table t(i int auto_increment,j timestamp ,k varchar(20)) partition  by range(to_timestamp_tz(j))
(
...
set timezone '+3:00';
drop table if exists t;
create table t(i int auto_increment,j timestamp with local time zone,k varchar(20)) partition  by range(to_timestamp_tz(j))
(
...
create table t(i int auto_increment,j timestamp with local time zone,k varchar(20)) partition  by range(to_timestamp_tz(j))
(
partition p0 values less than (datetimetz'2015-1-1 00:00:00 +08:00'),
partition p1 values less than (datetimetz'2015-1-1 3:00:00 +08:00'),
partition p2 values less than (datetimetz'2015-1-2 12:00:00 am -08:00'),
partition p3 values less than maxvalue
);
set timezone '+07:00';
insert into t(j)values(subdate(datetimeltz'2014-12-31 23:59:59',interval 3 hour));
