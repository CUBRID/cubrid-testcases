...
create table t(i int auto_increment,t datetime with time zone,dtltz datetime with local time zone,tstz timestamp with time zone,tsltz timestamp with local time zone,ttz time, tltz timestamp with local time zone) partition by range(dayofweek(t))(
...
create table t(i int auto_increment,t datetime with time zone,dtltz datetime with local time zone,tstz timestamp with time zone,tsltz timestamp with local time zone,ttz time,
 tltz timestamp with local time zone) partition by range(dayofweek(dtltz))(
partition p0 values less than (6),
partition p1 values less than maxvalue
);
...
drop table if exists t;
create table t(i int auto_increment,t datetime with time zone,dtltz datetime with local time zone,tstz timestamp with time zone,tsltz timestamp with local time zone,ttz time,
 tltz timestamp with local time zone) partition by range(dayofweek(tstz))(
partition p0 values less than (6),
partition p1 values less than maxvalue
);
...
drop table if exists t;
create table t(i int auto_increment,t datetime with time zone,dtltz datetime with local time zone,tstz timestamp with time zone,tsltz timestamp with local time zone,ttz time,
 tltz timestamp with local time zone) partition by range(dayofweek(tsltz))(
partition p0 values less than (6),
partition p1 values less than maxvalue
);
