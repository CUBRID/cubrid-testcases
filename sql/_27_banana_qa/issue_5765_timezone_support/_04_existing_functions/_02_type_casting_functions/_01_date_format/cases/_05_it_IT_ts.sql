--test date_format function with TIMESTAMP(L)TZ columns, with it_IT language
--+ holdcas on;
...
prepare st from 'select id, date_format(ts, ?), date_format(tsltz, ?), date_format(tstz, ?) from tz_test order by 1';
