--test date_format function with DATETIME(L)TZ columns, with vi_VN language
--+ holdcas on;
...
prepare st from 'select id, date_format(ts, ?), date_format(dtltz, ?), date_format(dttz, ?) from tz_test order by 1';
...
prepare st from 'select id, date_format(ts+1, ?), date_format(dtltz+1, ?), date_format(dttz+1, ?) from tz_test order by 1';
set time zone 'Europe/London';
-- 0:00, DST
