--test to_char function with DATETIME(L)TZ columns, with it_IT language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''it_IT''), to_char(dtltz, ?, ''it_IT''), to_char(dttz, ?, ''it_IT'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''it_IT''), to_char(dtltz+1, ?, ''it_IT''), to_char(dttz+1, ?, ''it_IT'') from tz_test order by 1';
