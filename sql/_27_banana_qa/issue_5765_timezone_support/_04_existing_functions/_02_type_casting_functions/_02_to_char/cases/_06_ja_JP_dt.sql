--test to_char function with DATETIME(L)TZ columns, with ja_JP language
--+ holdcas on;
...
-- -6:00, DST
insert into tz_test values(2, datetime'2014-06-30 7:30:21.999 PM', datetimetz'2014-06-30 7:30:21.999 PM +10:30', datetimetz'2014-06-30 7:30:21.999 PM America/New_York');
...
prepare st from 'select id, to_char(ts, ?, ''ja_JP''), to_char(dtltz, ?, ''ja_JP''), to_char(dttz, ?, ''ja_JP'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''ja_JP''), to_char(dtltz+1, ?, ''ja_JP''), to_char(dttz+1, ?, ''ja_JP'') from tz_test order by 1';
