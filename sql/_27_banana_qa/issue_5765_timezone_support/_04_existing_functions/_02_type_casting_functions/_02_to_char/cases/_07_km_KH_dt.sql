--test to_char function with DATETIME(L)TZ columns, with km_KH language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''km_KH''), to_char(dtltz, ?, ''km_KH''), to_char(dttz, ?, ''km_KH'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''km_KH''), to_char(dtltz+1, ?, ''km_KH''), to_char(dttz+1, ?, ''km_KH'') from tz_test order by 1';
