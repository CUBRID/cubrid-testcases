--test to_char function with TIMESTAMP(L)TZ columns, with fr_FR language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''fr_FR''), to_char(tsltz, ?, ''fr_FR''), to_char(tstz, ?, ''fr_FR'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''fr_FR''), to_char(tsltz+1, ?, ''fr_FR''), to_char(tstz+1, ?, ''fr_FR'') from tz_test order by 1';
