--test to_char function with DATETIME(L)TZ columns, with de_DE language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''de_DE''), to_char(dtltz, ?, ''de_DE''), to_char(dttz, ?, ''de_DE'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''de_DE''), to_char(dtltz+1, ?, ''de_DE''), to_char(dttz+1, ?, ''de_DE'') from tz_test order by 1';
