--test to_char function with TIMESTAMP(L)TZ columns, with km_KH language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''km_KH''), to_char(tsltz, ?, ''km_KH''), to_char(tstz, ?, ''km_KH'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''km_KH''), to_char(tsltz+1, ?, ''km_KH''), to_char(tstz+1, ?, ''km_KH'') from tz_test order by 1';
...
execute st using 'MM/DD/YYYY HH24:MI:SSF TZD', 'MM/DD/YYYY HH24:MI:SS TZD', 'MM/DD/YYYY HH24:MI:SS TZD';
