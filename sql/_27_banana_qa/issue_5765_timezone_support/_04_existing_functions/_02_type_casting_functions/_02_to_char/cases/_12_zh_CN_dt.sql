--test to_char function with DATETIME(L)TZ columns, with zh_CN language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''zh_CN''), to_char(dtltz, ?, ''zh_CN''), to_char(dttz, ?, ''zh_CN'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''zh_CN''), to_char(dtltz+1, ?, ''zh_CN''), to_char(dttz+1, ?, ''zh_CN'') from tz_test order by 1';
