--test to_char function with TIMESTAMP(L)TZ columns, with ja_JP language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''ja_JP''), to_char(tsltz, ?, ''ja_JP''), to_char(tstz, ?, ''ja_JP'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''ja_JP''), to_char(tsltz+1, ?, ''ja_JP''), to_char(tstz+1, ?, ''ja_JP'') from tz_test order by 1';
