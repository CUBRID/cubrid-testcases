--test to_char function with DATETIME(L)TZ columns, with ro_RO language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''ro_RO''), to_char(dtltz, ?, ''ro_RO''), to_char(dttz, ?, ''ro_RO'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''ro_RO''), to_char(dtltz+1, ?, ''ro_RO''), to_char(dttz+1, ?, ''ro_RO'') from tz_test order by 1';
