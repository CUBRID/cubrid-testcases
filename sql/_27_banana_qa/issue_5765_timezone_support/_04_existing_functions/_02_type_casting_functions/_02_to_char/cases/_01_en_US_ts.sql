--test to_char function with TIMESTAMP(L)TZ columns, with en_US language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''en_US''), to_char(tsltz, ?, ''en_US''), to_char(tstz, ?, ''en_US'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''en_US''), to_char(tsltz+1, ?, ''en_US''), to_char(tstz+1, ?, ''en_US'') from tz_test order by 1';
