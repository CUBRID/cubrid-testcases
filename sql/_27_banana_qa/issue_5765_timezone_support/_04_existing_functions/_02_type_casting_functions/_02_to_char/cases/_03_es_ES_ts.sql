--test to_char function with TIMESTAMP(L)TZ columns, with es_ES language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''es_ES''), to_char(tsltz, ?, ''es_ES''), to_char(tstz, ?, ''es_ES'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''es_ES''), to_char(tsltz+1, ?, ''es_ES''), to_char(tstz+1, ?, ''es_ES'') from tz_test order by 1';
