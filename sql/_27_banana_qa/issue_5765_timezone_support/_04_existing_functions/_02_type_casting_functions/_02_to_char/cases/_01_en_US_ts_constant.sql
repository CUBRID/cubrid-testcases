--test to_char function with TIMESTAMP(L)TZ constant values, with en_US language
--+ holdcas on;
...
prepare st from 'select to_char(?, ?, ''en_US'')';
...
--test: leap seconds
prepare st from 'select to_char(?+1, ?, ''en_US'')';
