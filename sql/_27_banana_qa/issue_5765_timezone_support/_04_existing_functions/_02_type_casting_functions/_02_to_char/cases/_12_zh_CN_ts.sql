--test to_char function with TIMESTAMP(L)TZ columns, with zh_CN language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''zh_CN''), to_char(tsltz, ?, ''zh_CN''), to_char(tstz, ?, ''zh_CN'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''zh_CN''), to_char(tsltz+1, ?, ''zh_CN''), to_char(tstz+1, ?, ''zh_CN'') from tz_test order by 1';
