--test to_char function with DATETIME(L)TZ columns, with vi_VN language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''vi_VN''), to_char(dtltz, ?, ''vi_VN''), to_char(dttz, ?, ''vi_VN'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''vi_VN''), to_char(dtltz+1, ?, ''vi_VN''), to_char(dttz+1, ?, ''vi_VN'') from tz_test order by 1';
