--test to_char function with TIMESTAMP(L)TZ columns, with ro_RO language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''ro_RO''), to_char(tsltz, ?, ''ro_RO''), to_char(tstz, ?, ''ro_RO'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''ro_RO''), to_char(tsltz+1, ?, ''ro_RO''), to_char(tstz+1, ?, ''ro_RO'') from tz_test order by 1';
