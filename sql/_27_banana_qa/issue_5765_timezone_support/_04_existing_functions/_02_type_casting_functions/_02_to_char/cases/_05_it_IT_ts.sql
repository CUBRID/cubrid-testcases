--test to_char function with TIMESTAMP(L)TZ columns, with it_IT language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''it_IT''), to_char(tsltz, ?, ''it_IT''), to_char(tstz, ?, ''it_IT'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''it_IT''), to_char(tsltz+1, ?, ''it_IT''), to_char(tstz+1, ?, ''it_IT'') from tz_test order by 1';
