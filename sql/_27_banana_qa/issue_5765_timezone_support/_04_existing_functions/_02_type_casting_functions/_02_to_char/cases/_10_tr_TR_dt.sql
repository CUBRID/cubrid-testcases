--test to_char function with DATETIME(L)TZ columns, with tr_TR language
--+ holdcas on;
...
prepare st from 'select id, to_char(ts, ?, ''tr_TR''), to_char(dtltz, ?, ''tr_TR''), to_char(dttz, ?, ''tr_TR'') from tz_test order by 1';
...
set time zone 'Europe/London';
-- 0:00, DST
prepare st from 'select id, to_char(ts+1, ?, ''tr_TR''), to_char(dtltz+1, ?, ''tr_TR''), to_char(dttz+1, ?, ''tr_TR'') from tz_test order by 1';
