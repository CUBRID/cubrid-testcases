...
select percentile_cont(0.1) within group (order by col1) over() from p_cont_date_time order by 1;
select percentile_cont(0.3) within group (order by col2) over() from p_cont_date_time order by 1;
select percentile_cont(0.5) within group (order by col3) over() from p_cont_date_time order by 1;
select percentile_cont(0.7) within group (order by col4) over() from p_cont_date_time order by 1;
select percentile_cont(0.7) within group (order by 4) over() from p_cont_date_time order by 1;



--TEST: OVER(PARTITION BY) clause
--TEST: partition by date
select col1, percentile_cont(0) within group (order by col5) over(partition by col1) p_cont from p_cont_date_time order by 1, 2; 
--TEST: partition by time
select col2, col3, percentile_cont(0.2) within group (order by col1) over(partition by col2, col3) p_cont from p_cont_date_time order by 1, 2, 3; 
--TEST: partition by timestamp
select col3, percentile_cont(0.4) within group (order by col2) over(partition by col3) p_cont from p_cont_date_time order by 1, 2; 
--TEST: partition by datetime
select col4, col2, percentile_cont(1) within group (order by col3 desc) over(partition by col4, col2) p_cont from p_cont_date_time order by 1, 2, 3; 
--TEST: partition by position
select col1, col3, percentile_cont(0.5) within group (order by col5 asc) over(partition by 1, 2) from p_cont_date_time order by 1, 2, 3; 
--TEST: partition by multiple columns
select col1, col2, col3, col4, col5, percentile_cont(0) within group (order by col5) over(partition by col3, col4) p_cont from p_cont_date_time order by 1, 2, 3; 
