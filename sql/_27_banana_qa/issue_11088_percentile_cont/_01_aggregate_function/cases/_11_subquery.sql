--TEST: test the aggregate function within a subquery 
...
select * from p_cont where col1 = (select percentile_cont(1) within group (order by col1) p_cont from p_cont) order by 1;
--test: [er]
select * from p_cont where col1 = (select percentile_cont(0.5) within group (order by col2) p_cont from p_cont group by col5) order by 1;
...
update p_cont set col1 = (select percentile_cont(0.8) within group (order by col2) from p_cont);
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
update p_cont set col2 = (select percentile_cont(0.8) within group (order by col1) from p_cont);
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
update p_cont set col2 = (select percentile_cont(0.8) within group (order by col1) from p_cont group by col2 order by 1 desc limit 1);
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
update p_cont t1 set col2 = (select percentile_cont(0.8) within group (order by t1.col1) from p_cont where col1 < 20);
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
update p_cont t1 set col2 = (select percentile_cont(0.8) within group (order by t1.col1) from p_cont where col1 < 20 group by col2 order by 1 limit 1);
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;

--test: subquery in odku statement
insert into p_cont(col1, col2, col3, col4, col5) values(20, 999, 'inserted', '2013-12-12 12:12:12.222', B'1111') on duplicate key update col1=(select percentile_cont(0.1) within group (order by col1) from p_cont), col3='updated';
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
insert into p_cont(col1, col2, col3, col4, col5) values(20, 503, 'inserted', '1993-11-10 11:23:34.123', B'1111') on duplicate key update col1=(select percentile_cont(0.66) within group (order by col1) from p_cont), col3='updated';
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
...
on (t.col1=s.col1)
when matched then update set t.col4=(select percentile_cont(0.88) within group(order by col4) from p_cont), col3='updated'
...
using (select * from p_cont where col1 > 10) s
on (t.col1=s.col1)
when matched then update set t.col2=(select percentile_cont(0.88) within group(order by s.col2)), col3='updated'
when not matched then insert(col2, col3) values(100, 'inserted');
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
