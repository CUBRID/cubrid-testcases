...
update p_cont set col2 = (select percentile_cont(0.8) within group (order by col2) from p_cont);
...
select * from p_cont__p__p3 order by 1, 2, 3, 4, 5;
rollback;
update p_cont set col2 = (select percentile_cont(0.8) within group (order by col1) from p_cont group by col2 order by 1 limit 1);
select * from p_cont__p__p1 order by 1, 2, 3, 4, 5;
rollback;
update p_cont t1 set col2 = (select percentile_cont(0.8) within group (order by t1.col1) from p_cont where col1 < 20);
select * from p_cont__p__p1 order by 1, 2, 3, 4, 5;
select * from p_cont__p__p2 order by 1, 2, 3, 4, 5;
select * from p_cont__p__p3 order by 1, 2, 3, 4, 5;
rollback;
update p_cont t1 set col2 = (select percentile_cont(0.8) within group (order by t1.col1) from p_cont where col1 < 20 group by col2 order by 1 desc limit 1);
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;

--test: subquery in odku statement
alter table p_cont add constraint pk primary key(col1, col2);
--BUG: CUBRIDSUS-16002 (resolved)
insert into p_cont(col1, col2, col3, col4, col5) values(20, 999, 'inserted', '2013-12-12 12:12:12.222', B'1111') on duplicate key update col1=(select percentile_cont(0.5) within group (order by col1) from p_cont);
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
alter table p_cont add constraint pk primary key(col1, col2);
insert into p_cont(col1, col2, col3, col4, col5) values(20, 503, 'inserted', '2013-12-12 12:12:12.222', B'1111') on duplicate key update col2=(select percentile_cont(0.89) within group (order by col2) from p_cont);
select * from p_cont order by 1, 2, 3, 4, 5;
rollback;
