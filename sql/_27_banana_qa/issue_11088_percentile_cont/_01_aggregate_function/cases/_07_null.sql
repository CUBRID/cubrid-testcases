...
select percentile_cont(0.5) within group (order by col1 asc) p_cont from p_cont order by 1;
select col2, col3, percentile_cont(0.1) within group (order by col1 desc) p_cont from p_cont group by col2, col3 order by 1, 2, 3;
select col4, percentile_cont(0.8) within group (order by col2 desc) p_cont from p_cont group by col4 order by 1, 2;
select clob_to_char(col5), percentile_cont(0.4) within group (order by col3 asc) p_cont from p_cont group by clob_to_char(col5) order by 1, 2;
select col3, percentile_cont(0.77) within group (order by col4 desc) p_cont from p_cont group by col3 order by 1, 2;
select col2, col3, col4, percentile_cont(0.999) within group (order by clob_to_char(col5) asc) from p_cont where length(clob_to_char(col5))> 0 group by col2, col3, col4 order by 1, 2, 3, 4;

--test: [er] pass 2 arguments to both the first and the second param
select percentile_cont(0.5, 0.5) within group (order by col1, col2 desc) p_cont from p_cont;
select percentile_cont(0.5, 0.5) within group (order by col2, clob_to_char(col4) desc) p_cont from p_cont;
