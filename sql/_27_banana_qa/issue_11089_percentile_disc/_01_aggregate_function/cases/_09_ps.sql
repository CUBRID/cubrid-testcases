...
--test: 
prepare st from 'select percentile_disc(?) within group (order by col1 asc) p_disc from p_disc order by 1';
execute st using 0;
...
prepare st from 'select col2, col3, percentile_disc(?) within group (order by col1 desc) p_disc from p_disc where col1 < ? group by col2, col3 order by 1, 2, 3';
...
prepare st from 'select col4, percentile_disc(?) within group (order by col2 desc) p_disc from p_disc group by col4 order by 1, 2 limit ?';
execute st using 0.8, 10;
execute st using 1, 1;
deallocate prepare st;

prepare st from 'select clob_to_char(col5), percentile_disc(?) within group (order by col3 asc) p_disc from p_disc group by clob_to_char(col5) having year(col4) > ? order by 1, 2';
execute st using 0.3, 1993;
execute st using 0, 1991;
deallocate prepare st;

prepare st from 'select col3, percentile_disc(?) within group (order by col4 desc) p_disc from p_disc group by col3 order by 1, 2 limit ?';
execute st using 0.77, 100;
deallocate prepare st;

prepare st from 'select col2, col3, col4, percentile_disc(round(?, ?)) within group (order by ? asc) p_disc from p_disc group by col2, col3, col4 order by 1, 2, 3, 4';
execute st using 0.999, 2, 2;
execute st using 1.333, 0, 2;
deallocate prepare st;

--test: [er] pass 2 arguments to both the first and the second param
prepare st from 'select percentile_disc(?, ?) within group (order by col1, col2 desc) p_disc from p_disc;';
