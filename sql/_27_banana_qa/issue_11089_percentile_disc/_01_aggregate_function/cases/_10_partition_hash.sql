...
select clob_to_char(col5), percentile_disc(0.4) within group (order by col1 asc) p_disc from p_disc group by clob_to_char(col5) order by 1, 2;
select col3, percentile_disc(0.77) within group (order by col4 desc) p_disc from p_disc group by col3 order by 1, 2;
select col2, col3, col4, percentile_disc(0.999) within group (order by col5 asc) from p_disc group by col2, col3, col4 order by 1, 2, 3, 4;
