-- alert table that insert data whith create to hash partition on smallint field and it's size 4 and then query and drop on this table
...
ALTER TABLE hash_test	PARTITION BY HASH(test_smallint)
        PARTITIONS 4;
...
select * from hash_test where test_smallint = 2.0;
