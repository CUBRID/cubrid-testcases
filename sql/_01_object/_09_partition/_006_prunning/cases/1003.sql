--test the operators(in, not in, not null, not between, <>) with select queries and varchar type
create table list_test(id int not null ,
	...
	PARTITION BY LIST (test_varchar) (
	    ...
	    PARTITION p1 VALUES IN ('fff','ggg',null,'hhh'),
...
select * from list_test where test_varchar in ('aaa','ddd','lll') order by 1,2;
select * from list_test where test_varchar not in ('bbb','eee','hhh') order by 1,2;
select * from list_test where  test_varchar is not null order by 1,2;

select * from list_test where test_varchar not between 'bbb' and 'ggg' order by 1,2;
select * from list_test where test_varchar <> 'ccc' order by 1,2;
select * from list_test where test_varchar <> 'hhh' order by 1,2;
