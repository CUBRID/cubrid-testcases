-- insert data  whith create hash partition class on a int field with size 8 and query data with keyword "in","=" or nesting and trunc and then drop class
...
select * from hash_test where test_int = (select 11 from db_root);
select * from hash_test where test_int in (select test_int from hash_test where id =11);
select * from hash_test A where A.test_int in (select to_number(B.test_char) from hash_test B where B.id =A.id) order by id;
