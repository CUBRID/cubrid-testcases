--create range partition table with char data type,insert data to this table and select data in where clause with keyword 'in', 'not in', 'not null', 'not between','<>'
...
select * from range_test where test_char in ('aaa','ddd','iii') order by 1,2;

select * from range_test where test_char not in ('bbb','eee','hhh') order by 1,2;
select * from range_test where  test_char is not null order by 1,2;
select * from range_test where test_char not between 'bbb' and 'ggg' order by 1,2;
select * from range_test where test_char <> 'ccc' order by 1,2;
