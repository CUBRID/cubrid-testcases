--create range partition table with int data type,insert data to this table and select data in where clause with other select clause
...
select * from range_test where test_int = (select 11 from db_root) order by 1;
select * from range_test where test_int in (select test_int from range_test where id =10) order by 1;
select * from range_test A where A.test_int in (select to_number(B.test_char) from range_test B where B.id =A.id) order by id;
