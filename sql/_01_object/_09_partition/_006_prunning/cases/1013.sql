--create range partition table with int data type,insert data to this table and select data in where clause with function round,ceil,floor,position
...
select * from range_test where test_int = ceil(11.01) order by 1;
select * from range_test where test_int = floor(11.99) order by 1;
select * from range_test where test_int = position('aa' in 'swedfrtgqcaasd') order by 1;
