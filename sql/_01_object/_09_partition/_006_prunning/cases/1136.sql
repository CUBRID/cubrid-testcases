-- insert data  whith create hash partition class on a int field with size 8 and query data with function of abs and trunc and then drop class
...
select * from hash_test where test_int = abs(-11);
select * from hash_test where test_int = trunc(1.1);
select * from hash_test where test_int = trunc(11.10);
select * from hash_test where test_int = trunc(cast('11.10' as numeric));
drop hash_test;