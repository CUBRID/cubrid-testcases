--create list partiition table with a extract(day from date data field) function 

create table list_test(id int,	
				   test_time time,
				   test_date date,
				   test_timestamp timestamp, primary key(id,test_date))
	PARTITION BY LIST (extract(day from test_date)) (
...
update list_test set test_date = '2008-01-10' where test_date < '2000-01-01';
select * from list_test order by id;
select * from list_test__p__p0 order by id;
select * from list_test__p__p1 order by id;
select * from list_test__p__p2 order by id;
