--create list partiition table with a extract(hour  from timestamp  date field) function 

create table list_test(id int,	
				   test_time time,
				   test_date date,
				   test_timestamp timestamp, primary key(id,test_timestamp))
	PARTITION BY LIST (extract(hour from test_timestamp)) (
...
create index test_timestamp_idx on list_test (test_timestamp);
