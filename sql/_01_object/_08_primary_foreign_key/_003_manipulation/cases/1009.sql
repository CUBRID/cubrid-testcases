--+ holdcas on;
--Test the constraint of the foreign key using update (update no action and cache object)2
--it tests a constraint of the foreign key syntax using update no action and cache object
...
CREATE TABLE track(
  ...
  posn INTEGER,
  song VARCHAR(255),
  FOREIGN KEY (album) REFERENCES album(id) ON UPDATE NO ACTION 
);
