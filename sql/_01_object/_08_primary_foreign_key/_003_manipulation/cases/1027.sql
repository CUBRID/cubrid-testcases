--+ holdcas on;
--Test the constraint of the foreign key using delete cascade and update no action(update PK table,delete PK table)
--it tests a constraint of the foreign key syntax using delete cascade and update no action,update PK table, delete from PK table
...
CREATE TABLE track(
  ...
  posn INTEGER,
  song VARCHAR(255),
  FOREIGN KEY (album) REFERENCES album(id) ON DELETE CASCADE ON UPDATE NO ACTION 
);
