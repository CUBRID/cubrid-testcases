--create a table with three columns, define seven methods:get_2000() that its return type is int , get_success() that its return type is string ,  get_null() that its return type is string,get_int(int) that its return type is int, get_string(string) that its return type is string, get_string2(string,string) that its return type is string, get_set() that its return type is SET, declare the implemented functions TEST_getInt ,TEST_getString,TEST_getNull, TEST_int_input,TEST_string_input2,TEST_getSet contained in the file '$HOME/method_test/myyang', drop table at last
...
METHOD get_2000() int FUNCTION TEST_getInt,
       ...
       get_set() SET FUNCTION TEST_getSet
