...
select class_of, IF (is_system_class = 0, CONCAT (LOWER (owner.name), '.', class_name), class_name) as unique_name, class_name, owner.name from _db_class where (is_system_class | flags) % 8 = 0 order by unique_name;
select CONCAT (LOWER (owner.name), '.', name) as unique_name, name, owner.name from _db_serial;
call change_serial_owner ('dba.t1_ai_c1', 'u1') on class _db_serial;
select CONCAT (LOWER (owner.name), '.', name) as unique_name, name, owner.name, target_class, action_definition from _db_trigger order by 1;
