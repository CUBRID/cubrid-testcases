...
create or replace procedure t_DATETIME_BIT(sql_type string, procedure_type string, param BIT ) as begin
...
create or replace procedure t_DATETIMELTZ_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_DATETIMETZ_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_DATE_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_TIME_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_TIMESTAMP_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_TIMESTAMPLTZ_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_TIMESTAMPTZ_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_DOUBLE_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_FLOAT_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
call t_FLOAT_BIT('FLOAT', 'BIT', cast( 16777.217 as float ) ) ;
drop procedure t_FLOAT_BIT ;


call print_message('t_NUMERIC(8,4)_BIT. This scenario is a failure.');
create or replace procedure t_NUMERIC_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_BIGINT_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
call t_BIGINT_BIT('BIGINT', 'BIT', cast( 34589012 as bigint ) ) ;
drop procedure t_BIGINT_BIT ;


call print_message('t_INT_BIT. This scenario is a failure.');
create or replace procedure t_INT_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
call t_INT_BIT('INT', 'BIT', cast( 782346 as int ) ) ;
drop procedure t_INT_BIT ;


call print_message('t_SHORT_BIT. This scenario is a failure.');
create or replace procedure t_SHORT_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
call t_SHORT_BIT('SHORT', 'BIT', cast( 8934 as short ) ) ;
drop procedure t_SHORT_BIT ;


call print_message('t_BIT(8)_BIT. This scenario is a failure.');
create or replace procedure t_BIT_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_BITVARYING_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_CHAR_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_VARCHAR_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_SET_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_MULTISET_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_LIST_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_ENUM_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_BLOB_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_CLOB_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_JSON_BIT(sql_type string, procedure_type string, param BIT ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
