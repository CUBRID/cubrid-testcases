...
create or replace procedure t_DATETIME_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
...
create or replace procedure t_DATETIMELTZ_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_DATETIMETZ_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_DATE_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_TIME_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_TIMESTAMP_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_TIMESTAMPLTZ_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_TIMESTAMPTZ_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_DOUBLE_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_FLOAT_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
call t_FLOAT_CLOB('FLOAT', 'CLOB', cast( 16777.217 as float ) ) ;
drop procedure t_FLOAT_CLOB ;


call print_message('t_NUMERIC(8,4)_CLOB. This scenario is a failure.');
create or replace procedure t_NUMERIC_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_BIGINT_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
call t_BIGINT_CLOB('BIGINT', 'CLOB', cast( 34589012 as bigint ) ) ;
drop procedure t_BIGINT_CLOB ;


call print_message('t_INT_CLOB. This scenario is a failure.');
create or replace procedure t_INT_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
call t_INT_CLOB('INT', 'CLOB', cast( 782346 as int ) ) ;
drop procedure t_INT_CLOB ;


call print_message('t_SHORT_CLOB. This scenario is a failure.');
create or replace procedure t_SHORT_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
call t_SHORT_CLOB('SHORT', 'CLOB', cast( 8934 as short ) ) ;
drop procedure t_SHORT_CLOB ;


call print_message('t_BIT(8)_CLOB. This scenario is a failure.');
create or replace procedure t_BIT_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_BITVARYING_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_CHAR_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_VARCHAR_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_SET_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_MULTISET_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_LIST_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_ENUM_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_BLOB_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_CLOB_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
...
create or replace procedure t_JSON_CLOB(sql_type string, procedure_type string, param CLOB ) as begin
    dbms_output.put_line('sql_type = ' ||sql_type ||', procedure_type = '||procedure_type||', current_value = '|| param ); 
end;
