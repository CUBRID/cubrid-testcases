...
select sp_name, pkg_name, is_system_generated from _db_stored_procedure order by CONCAT (LOWER (owner.name), '.', CASE WHEN pkg_name IS NULL THEN '' ELSE CONCAT (pkg_name, '.') END, sp_name);
