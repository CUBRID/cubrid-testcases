...
create index i_tree_id_mgrid on tree(id,mgrid);

--result is not correct.
SELECT /*+ recompile */ id, mgrid FROM tree  START WITH  id>=0 and  mgrid IS NULL CONNECT BY prior id=mgrid order by id asc;

--result is not correct.
SELECT /*+ recompile */ id, mgrid FROM tree  START WITH  id>=0 and  mgrid IS NULL CONNECT BY prior id=mgrid order by id asc limit 5;
