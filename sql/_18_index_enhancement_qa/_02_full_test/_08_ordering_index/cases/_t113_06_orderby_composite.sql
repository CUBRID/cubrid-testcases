...
create index i_t_abc on t(a desc,b,c);
insert into t values (1,1,1,1,1);
insert into t values (2,2,2,2,2);
insert into t values (5,5,5,5,5);
insert into t values (null,null,null,null,null);

select /*+ recompile */ * from t where a>0 order by a desc,b,c;

select /*+ recompile */ * from t where a>0 order by a desc,b;

select /*+ recompile */ * from t where a>0 order by a desc,c;

select /*+ recompile */ * from t where a>0 order by a;

select /*+ recompile */ * from t where a>0 order by 1 desc,2,3;

select /*+ recompile */ * from t where a>0 order by 1 desc,2;
