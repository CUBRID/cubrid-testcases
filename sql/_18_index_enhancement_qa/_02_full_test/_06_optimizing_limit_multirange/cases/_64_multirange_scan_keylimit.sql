...
select /*+ recompile */ * from t where i1=0 and i2 in (0,1,2,3,4,5) AND i3 BETWEEN 0 AND 23 using index i_t_123 keylimit 1 order by i3;

--result is the same with above
select /*+ recompile */ * from t where i1=0 and i2 in (0,1,2,3,4,5) AND i3 BETWEEN 0 AND 23 using index i_t_123 keylimit 1 order by i3 for orderby_num()<=3;

--result is the same with above
select /*+ recompile */ * from t where i1=0 and i2 in (0,1,2,3,4,5) AND i3 BETWEEN 0 AND 23 using index i_t_123 keylimit 1 order by i3 limit 3;
