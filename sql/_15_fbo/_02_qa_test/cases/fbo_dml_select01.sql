...
select * from select_t1 where b like bit_to_blob(X'10111%');
select nr, str from select_t1 where c not like char_to_clob('aaa');
select nr, str from select_t1 where c not like char_to_clob('aaa%');
...
select distinct str from select_t1;
select distinct blob_to_bit(b) from select_t1;
select distinct clob_to_char(c) from select_t1;

select DISTINCTROW str from select_t1;
select DISTINCTROW blob_to_bit(b) from select_t1;
select DISTINCTROW clob_to_char(c) from select_t1;
-- 5 test GROUP BY on blob/clob column;
select str from select_t1 group by str;
select str,count(*) from select_t1 group by str;
