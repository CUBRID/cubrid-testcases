...
set @a = 0;
set @b = '';
select a, b, @a := @a + 1, @b := @b + cast(a as varchar(100)) + b from t order by a desc;

-- rewrite with subexpressions
set @a = 0;
select /*+ recompile */ a, @a := (@a + (a + a) / (a * 2)) * (c * 10) + 1 from t order by a desc;

-- rewrite with functions
set @a = 0;
select /*+ recompile */ a, @a := @a + if(@a < 2, 1, (a * 2) / a) from t order by a desc;

-- rewrite with functions 2
set @a = 0;
select /*+ recompile */ a, @a := @a + coalesce(if(@a < 2, NULL, a), a * 10, a / 2) from t order by a desc;

-- rewrite (dependent variables)
set @a = 0;
set @b = 0;
select /*+ recompile */ a, @a := @a + 1, @b := @a * (a - 1) from t order by a desc;
...
-- rewrite (name clash)
set @a = 0;
select /*+ recompile */ a as ex_0, a as sx_0, b + 'xx' as sx_1, @a := @a + a*2 from t order by a desc;

-- rewrite (name clash 2)
set @a = 0;
select /*+ recompile */ dt_sort.a, @a := @a + dt_sort.a from t dt_sort, t t2 order by dt_sort.a desc;
...
set @a = 0;
select /*+ recompile */ a, @a := @a + 1 from u group by a order by a desc;

set @a = 0;
select /*+ recompile */ b, @a := @a + 1 from u group by b order by b desc;

-- rewrite (derived table)
set @a = 0;
select /*+ recompile */ a, b, @a := @a + 1 from (select t1.a + 2 as a, t1.a + 4 as b from t t1) x order by b desc;
