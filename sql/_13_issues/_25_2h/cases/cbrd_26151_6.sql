 ...
 *
 * Scenario 6: A grantee group member can view authorizations where the
 * group is grantee (AC #4: grantee / grantee-group members see grants
 * they received).
 */
...
create user u2;
create user u3;
create user grantee_member;

alter user u3 add members grantee_member;
...
evaluate('6-1. login as grantee_member: authorizations where u3 is grantee');
call login('grantee_member','') on class db_user;
...
evaluate('6-2. login as dba: all authorizations visible (reference)');
