...
select class_name, owner_name, partition_name, partition_class_name, partition_type, partition_expr, partition_values, class_partition_type, comment from db_partition order by owner_name, partition_class_name;
select vclass_name, owner_name, vclass_def, comment from db_vclass where vclass_name in ('v1') order by owner_name, vclass_name;
...
select grantor_name, grantee_name, object_type, object_name, owner_name, auth_type, is_grantable from db_auth where object_name in (select class_name from db_class where is_system_class = 'NO') order by owner_name, grantor_name, grantee_name, object_name, auth_type;
