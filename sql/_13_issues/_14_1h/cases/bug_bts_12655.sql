...
select round(a),row_number() over() rn from (select * from xoo order by 1,2) order by 1;
drop xoo;
...
select blob_to_bit(col1), clob_to_char(col2), row_number() over() row_num from (select * from xoo order by blob_to_bit(col1)) order by 1;
drop xoo;
