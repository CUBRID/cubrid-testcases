...
select '----11 test view--------';
------------------------------------------------------------------------
create view v11 as select class_of, IF (is_system_class = 0, CONCAT (LOWER (owner.name), '.', class_name), class_name) as unique_name, class_name, class_type, is_system_class, flags, owner, inst_attr_count, class_attr_count, shared_attr_count, inst_meth_count, class_meth_count, collation_id, tde_algorithm, sub_classes, super_classes, inst_attrs, class_attrs, shared_attrs, inst_meths, class_meths, meth_files, query_specs, indexes, comment, [partition] from _db_class;
