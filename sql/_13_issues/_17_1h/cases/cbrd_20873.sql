...
WITH cte_s0lAS8PSg AS (select /*+ recompile */ a from tb where a between 3 and 1 group by a desc having groupby_num() between 2 and 4) select
 * from cte_s0lAS8PSg;

WITH cte_s0lAS8PSg AS (select /*+ recompile */ a from tb where a between 1 and 3 group by a desc having groupby_num() between 2 and 4) select
 * from cte_s0lAS8PSg;
...
WITH abcd 
    AS (  
        ...
        SELECT  t.id, t.Name, t.ParentID, timezone_checksum
         FROM    tbl AS t , db_root
               JOIN abcd AS a 
                   ON t.ParentId = a.id 
        ) SELECT * FROM abcd;
WITH abcd                                                                                                                                                                                  AS (                                                                                                                                                                                              SELECT  id, Name, ParentID, lang                                                                                                                                                            FROM    tbl, db_root                                                                                                                                                                          WHERE   ParentId IS NULL                                                                                                                                                                     UNION ALL                                                                                                                                                                                   SELECT  t.id, t.Name, t.ParentID, timezone_checksum                                                                                                                                           FROM    tbl AS t , db_root                                                                                                                                                                         JOIN abcd AS a                                                                                                                                                                                   ON db_root.charset = a.id                                                                                                                                                         ) SELECT * FROM abcd order by id, name;
...
WITH abcd                                                                                                                                                                                  AS (                                                                                                                                                                                              SELECT  id, Name, ParentID, lang                                                                                                                                                            FROM    tbl, db_root                                                                                                                                                                          WHERE   ParentId IS NULL                                                                                                                                                                     UNION ALL                                                                                                                                                                                   SELECT  t.id, t.Name, t.ParentID, timezone_checksum                                                                                                                                           FROM    tbl AS t , db_root                                                                                                                                                                         JOIN abcd AS a                                                                                                                                                                                   ON db_root.charset = a.id                                                                                                                                                         ), ctw as (SELECT * FROM db_root where charset in (with cte as (select abcd.id+2 from abcd)select * from cte limit 1)) select abcd.id+ctw.charset from abcd, ctw;
