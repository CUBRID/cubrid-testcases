 ...
 * 11. Control: With autocommit ON, login() executes normally (no pending work to roll back).
 * 12. Pending privilege changes (GRANT) are force-rolled-back.
 * 13. State Preservation: login() does not accidentally alter the session's autocommit setting.
 */
...
GRANT SELECT, INSERT ON tbl1 TO user2;
COMMIT;
...
SELECT * FROM tbl2;
-- Expected: ERROR - tbl2 does not exist
...
evaluate 'Case 11. Control: With autocommit ON, login() executes normally';
;autocommit on
...
evaluate 'Case 13. State Preservation: login() does not accidentally turn autocommit ON';
CALL login('user2') ON CLASS db_user;
INSERT INTO tbl1 VALUES (300);
ROLLBACK;
SELECT COUNT(*) FROM tbl1;
-- Expected: COUNT(*) = 0
