...
SELECT 
    ...
    CASE 
        ...
        WHEN p_next_pageid = -1 THEN 'p_next_pageid=-1: PASS'
        ELSE 'p_cur_pageid+1!=p_next_pageid && p_next_pageid!=-1: FAIL' 
    END AS assertion_2,
    CASE 
        WHEN p_cur_pageid - 1 = p_prev_pageid THEN 'p_cur_pageid-1=p_prev_pageid: PASS' 
        WHEN p_prev_pageid IS NULL THEN 'p_prev_pageid=NULL: PASS'
        ELSE 'p_cur_pageid-1!=p_prev_pageid && p_prev_pageid!=NULL: FAIL' 
    END AS assertion_3
FROM tbl_data;
