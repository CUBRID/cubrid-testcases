...
SELECT u.name, x.name as groups FROM _db_user AS u, TABLE(u.direct_groups) AS g(x) order by 1;
...
SELECT u.name, x.name as groups, u.comment FROM _db_user AS u, TABLE(u.direct_groups) AS g(x) order by 1;
