...
SELECT u.name, g.name as groups FROM db_user AS u, TABLE(u.direct_groups) AS g(name) order by 1;
...
SELECT u.name, g.name as groups, u.comment FROM db_user AS u, TABLE(u.direct_groups) AS g(name) order by 1;
