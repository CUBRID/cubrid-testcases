...
evaluate 'Case 9: conflicting LEADING in outer+subquery; subquery LEADING must be dropped after view merge';
...
evaluate 'Case 10: inline-view LEADING targets a non-renamed alias (tb b); ensure partial order is preserved after merge';
select /*+ recompile */ count(*)
from tc a,
...
evaluate 'Case 13: outer LEADING partial order + subquery LEADING exists (subquery LEADING should be removed due to conflict after view merge)';
...
evaluate 'Case 15: subquery ORDERED is removed on view merge; main query must not inherit ORDERED';
...
evaluate 'Case 16: ORDERED inline-view + NO_MERGE (keep ORDERED by preventing view merge)';
select /*+ recompile */ count(*)
from tc a,
...
evaluate 'Case 17: triangle join predicates + LEADING (partial order: b before c)';
...
evaluate 'Case 18: correlated EXISTS inline-view; no outer LEADING -> subquery LEADING should be copied on view merge';
select /*+ recompile */ count(*)
from tc x,
...
evaluate 'Case 19: correlated EXISTS inline-view; outer LEADING exists -> subquery LEADING should be removed due to conflict after view merge';
