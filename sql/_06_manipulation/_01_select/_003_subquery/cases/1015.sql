--test update using result of select query or a arithmetic expression
...
update DML_0001 set int_col = (select max(int_col) + 1 from dml_0002) + 6 where int_col = 2;

update DML_0002 set ref_col = (select dml_0001 from dml_0001 where int_col=3), set_col = {1,2,3,4,5} where int_col = 3;

update DML_0002 set ref_col = (select dml_0001 from dml_0001 where int_col=4), 
		set_col = set (select int_col,var_col from DML_0002 group by int_col,var_col) where int_col = 4;
