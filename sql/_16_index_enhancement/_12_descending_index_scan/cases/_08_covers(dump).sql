-- test for covering vs. order by skipping via descending index (with plan dump)
...
SELECT i, j, k FROM T WHERE j > 0 ORDER BY j desc, k desc;
DROP INDEX i_t_i_j_k on t;
SELECT i, j, k FROM T WHERE j > 0 ORDER BY j desc, k desc;
