--+ holdcas on;
-- test for ORDER BY skipping (with plan dump)
...
SELECT /*+ use_desc_idx */ i, s FROM t WHERE s >= 'a' ORDER BY s;
SELECT /*+ use_desc_idx */ i, s FROM t WHERE s >= 'a' ORDER BY s desc;
SELECT j, i, s FROM t WHERE j > 0 ORDER BY j, i DESC;
SELECT j, i, s FROM t WHERE j > 0 ORDER BY j DESC, i;
