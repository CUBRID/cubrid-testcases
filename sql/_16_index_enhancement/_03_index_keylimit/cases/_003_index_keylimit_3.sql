...
create index i_test_table_col_smallint on test_table(col_smallint);
create index i_test_table_col_smallint_col_float_col_string on test_table(col_smallint, col_float, col_string);
create index i_test_table_col_timestamp_col_datetime on test_table(col_timestamp, col_datetime);

select col_timestamp, col_datetime from test_table where col_timestamp > '06:59:12 PM 05/03/2008' using index i_test_table_col_timestamp_col_datetime keylimit 2;
select col_timestamp, col_datetime from test_table where col_timestamp > '11:59:59 PM 12/31/2005' using index i_test_table_col_timestamp_col_datetime keylimit 1,3;
select col_smallint, col_float, col_string from test_table where col_smallint > 0 and col_string like('%ab%') using index i_test_table_col_smallint_col_float_col_string(+) keylimit 12,9;
select col_timestamp, col_datetime, col_varchar_92 from test_table where col_varchar_92 like('%ab%') and col_timestamp > '06:59:12 PM 05/03/2002' using index i_test_table_col_timestamp_col_datetime(+) keylimit 5;
