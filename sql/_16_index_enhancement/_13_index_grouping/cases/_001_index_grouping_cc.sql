...
select treeid from t1 where treeid is null group by treeid;
select treeid from t1 where id > treeid group by treeid;
select treeid from t1 where treeid - 1 > 0 group by treeid;
select treeid from t1 where id > 0 or treeid < 100 and id < 10 or text > 'A' group by treeid;
select treeid from t1 where id > treeid or text < 'C' group by treeid;
