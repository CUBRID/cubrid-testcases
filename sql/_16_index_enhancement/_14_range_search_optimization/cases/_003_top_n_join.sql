...
create index idx3 on t(j, i);
create index idx4 on u(i, j, k desc);
...
create index idx5 on t(i, k, p desc);
...
create index idx6 on users(name, id);
create index idx7 on posts(uid, category, entry_date desc);
...
select posts.title, posts.category, posts.entry_date from	posts, users where	users.name='Ben' and posts.uid=users.id and	posts.category in ('friends','books','games','pets') order by entry_date desc limit 5;
