...
select a, avg(max(c2)) over(partition by a), median(max(c1)) over(partition by a) from median_2 group by a order by 1;
select a, avg(max(all c2)) over(partition by a), median(max(distinct c1)) over(partition by a) from median_2 group by a order by 1;
select a, avg(max(distinct c2)) over(partition by a), median(max(c1)) over(partition by a) from median_2 group by a order by 1;
