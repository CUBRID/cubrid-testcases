 ...
 * 3 - a NOLOGIN user still appears in db_authorization, same as a
 *     loginable one -- every user gets one row regardless of ownership
...
evaluate 'Case 3: a NOLOGIN user still appears in db_authorization, same as a loginable one';
SELECT owner FROM db_authorization WHERE owner IN ('USER_CAT1', 'USER_CAT2') ORDER BY owner;
