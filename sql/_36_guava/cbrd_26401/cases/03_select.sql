...
create index tA_cA_cB_idx on t1 (c1,c2);


insert into t1(id, c1, c2, c3, c4) values (1, 10, 100, 1000, 'a');
insert into t1(id, c1, c2, c3, c4) values (2, 20, 20, 2000, 'b');
...
show trace;
select /*+recompile*/ c2 from t1 order by c1;
show trace;
select /*+recompile*/ c2 from t1 order by 1;
show trace;
...
evaluate 'CASE expression with invisible column';
select id, c2, c4,
...
evaluate 'Nested CASE with invisible columns';
select id, c2,
...
evaluate 'ORDER BY with expression on invisible column';
select * from t1 order by c1 + c3;
select c2, c4 from t1 order by c1 * 2;

-- ORDER BY invisible in subquery
evaluate 'ORDER BY invisible in subquery';
...
evaluate 'LIMIT with invisible column in WHERE';
...
select count(distinct c1) from t1 order by 1;
...
evaluate 'SELECT * should not show invisible collections';
select * from t1 order by id;

-- SELECT with invisible collections
evaluate 'SELECT with invisible collections';
select id, c1, c2, c3, c4 from t1 order by id;

-- WHERE clause with invisible collection
evaluate 'WHERE clause with invisible collection';
select * from t1 where 10 in c1;
select id, c2, c4 from t1 where {10, 20} subseteq c1 order by 1;

-- Collection operators with invisible columns
evaluate 'Collection operators with invisible columns';
select id, c4 from t1 where c1 superset {10};
select id, c4 from t1 where c3 seteq {100, 200, 300} order by 1;

-- Update invisible collection
evaluate 'Update invisible collection';
update t1 set c1 = c1 + {40} where id = 1;
select id, c1, c2, c3, c4 from t1 where id = 1 order by 1;
