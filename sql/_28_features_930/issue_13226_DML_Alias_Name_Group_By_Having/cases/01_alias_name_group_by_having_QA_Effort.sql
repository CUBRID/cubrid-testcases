...
select a, ifnull(b,3) as b from t1 group by b order by 1,2;
...
select a, max(b) b, count(*) from t1  group by if(b is null, 2, b) order by b;
select a, max(b) b, count(*) from t1  group by if(b is null, 2, b) order by b nulls first;
select a, max(b) b, count(*) from t1  group by if(b is null, 2, b) order by b nulls last;
...
select a, ifnull(b,3) as b, count(*) from t1 group by if(b is null, 2, b) order by 1,2,3;

select a, if(b is null, 2, b) as b, count(*) from t1 group by if(b is null, 2, b) order by 1,2,3;
select a, if(b is null, 10, b) as b, count(*) from t1 group by if(b is null, 10, b) order by 1,2,3;
select a, if(b is null, null, b) as b, count(*) from t1 group by if(b is null, 10, b) order by 1,2,3;
...
select a, ifnull(b,3) as b, count(*) from t1 group by if(b is null, power(a,2), power(b,2)) order by 1,2,3;

select a as axx, ifnull(b,3) as bxx, count(*) from t1 group by if(bxx is null, power(axx,2), power(bxx,2)) having power(axx, 2) > 1;

select * from t1 where b in (select ifnull(b,2) as b from t1 group by b) order by 1,2,3;
select ifnull(b,4) as b from t1 where b in (select ifnull(b,null) as b from t1 group by b) order by 1;

select a, b as bxx from t1 order by 1,2;
select a, b as bxx from t1 group by bxx = 1 order by 1 , 2;
select a, b as bxx from t1 group by b = 1 order by 1 , 2;
select a, ifnull(b,1) as bxx from t1 group by b = 1 order by 1 , 2;
select min(a), ifnull(b,1) as bxx from t1 group by bxx = 1 order by 1, 2;
...
-- pt_name --
select a , count(*) from t1 group by b order by 1,2;
...
select a as bxx, count(*) from t1 group by bxx order by 1,2;
select a as bxx, count(*) from t1 group by b order by 1,2;
select a as bxx, ifnull(b,3) as b,count(*) from t1 group by b order by 1,2,3;
select t1.a as bxx, count(*) from t1 group by t1.bxx;

-- pt_expr --
select a, ifnull(b, 2) as b from t1 group by b order by 1,2;
select a, ifnull(b, 2) as b from t1 group by t1.b order by 1,2;
select a, ifnull(t1.b, 2) as b from t1 group by b order by 1,2;
select a, ifnull(b, 2) as b from t1 group by power(b, 2) order by 1,2;
select a, ifnull(b, 2) as txx from t1 group by power(txx,2) order by 1,2;
select a, ifnull(b, 2) as b from t1 group by power(txx,2);

-- pt_sort_spec --
select a, ifnull(b, 2) as b from t1 group by 2 order by 1,2;
