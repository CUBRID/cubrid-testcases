...
show trace;
select a, max(b) from (select a, b, d from t1 order by a,b,d) group by d;
show trace;
select a, avg(a) from (select a, b, d from t1 order by a,b,d) group by b;
show trace;
select a, min(b) from (select * from t1 order by 1,2,3,4) group by b,c,d;
show trace;
