...
prepare st from 'select /*+ recompile */* from t where ''a'' in (?, ?) order by 1, 2';
execute st using 'A', 'b';
set names iso88591 collate iso88591_en_ci;
execute st using 'A', 'b';
--BUG CUBRIDSUS-13650 resolved
set names utf8 collate utf8_en_cs;
execute st using 'a', 'b';

execute st using _iso88591'a', 'b';
execute st using 'a', _euckr'b';


prepare st1 from 'select * from t where ? in (?, ?) order by 1, 2';
...
--compare with s1
prepare st2 from 'select * from t where s1 in (?, ?) order by 1, 2';
...
--compare with s2
prepare st3 from 'select * from t where s2 in (?, ?) order by 1, 2';
