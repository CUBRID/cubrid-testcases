...
prepare st1 from 'select translate(?, ?, ?);';
...
--compare with s1
prepare st2 from 'select translate(?, trim(s1), ?), collation(translate(?, trim(s1), ?)) from t order by 1, 2';
...
--compare with s2
prepare st3 from 'select translate(?, s2, ?), collation(translate(?, s2, ?)) from t order by 1, 2';
