...
prepare b1 from 'select id, max(b) from t3 group by d order by 1,2';
execute b1;

prepare b2 from 'select id, max(b)-min(b) from t3 group by c, d having min(d)>? order by 1,2';
execute b2 using 1;
execute b2 using 'yes';
execute b2 using 'no';

prepare b3 from 'select id, avg(id), max(b) from (select id, b, c, d from t3 where b>?) group by d order by 1,2,3';
