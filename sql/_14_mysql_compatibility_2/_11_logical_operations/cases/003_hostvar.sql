...
select (1<>0 and ?<>0), (1<>0 and ?<>0), (?<>0 and 0<>0), (0<>0 and ?<>0) from db_root;

$int, $1, $int, $0, $int, $1, $int, $0
select (1<>0 or ?<>0), (1<>0 or ?<>0), (?<>0 or 0<>0), (0<>0 or ?<>0) from db_root;
...
$varchar, $testing123
select 'ok' as verdict from db_root where ?<>0;

$varchar, $1
select 'ok' as verdict from db_root where ?<>0;

$varchar, $000
select 'not-ok' as verdict_empty from db_root where ?<>0;
...
$numeric, $0.01
select 'huh?-not-ok' as verdict_empty from db_root where ?<>0;
