...
select /*+ recompile */ * from t where i1=0 and i2 in (0,1,2,3,4,5) using index i_t_123 keylimit 10 order by i3 for orderby_num()<=3;

select /*+ recompile */ * from t where i1=0 and i2 not in (0) using index i_t_123 keylimit 10 order by i3 desc for orderby_num()<=3;

select /*+ recompile */ * from t where i1=1 and i2 not in (0) using index i_t_123 keylimit 10 order by i2 desc for orderby_num() between 1 and 3;
