...
select /*+ recompile */ * from t
	where a in (1,3)
	order by b, c desc, d, e desc limit 5;
-- reversed index:
select /*+ recompile */ * from t
	where a in (1,3)
	order by b desc, c, d desc, e limit 5;
-- orders do not match:
select /*+ recompile */ * from t
	where a in (1,3)
	order by b,c,d,e limit 5;
-- columns do not match
select /*+ recompile */ * from t
	where a in (1,3)
	order by c desc, b, d, e desc limit 5;
select /*+ recompile */ * from t
	where a in (1,3)
	order by b, d, e desc limit 5;
...
-- all attributes that are in index before the first column must be in eq/range terms
select /*+ recompile */ * from t
	where a in (1,3) and b = 1
	order by d, e desc limit 5;
-- cannot have more than one range term
select /*+ recompile */ * from t
	where a in (1,3) and b in (1,2) and c = 1
	order by d, e desc limit 5;
-- can have only eq terms
select /*+ recompile */ * from t
	where a = 1 and b = 1
