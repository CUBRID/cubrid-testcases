...
--TEST: using idx(-)
--TEST: i1
select /*+ recompile */ * from ui where i1 < 5 using index pk_ui_i1(-);
select /*+ recompile */ * from ui where i1 < 5 using index pk_ui_i1(-), idx_i1_i2(-) order by 1;
select /*+ recompile */ * from ui where i1 < 5 using index pk_ui_i1(-) order by 1 desc;

--TEST: i2
select /*+ recompile */ * from ui where i2 > 9 using index idx_i2(-);
select /*+ recompile */ * from ui where i2 > 10 or i2 is null using index idx_i2_i3(-);
select /*+ recompile */ * from ui where i2 < 10 using index idx_i2(-), idx_i2_i3(-), idx_all(-) order by 1;
...
select /*+ recompile */ * from ui where to_date(i4) < '1995-12-31' using index idx_i4_i3(-);
select /*+ recompile */ * from ui where to_date(i4) < '1995-12-31' using index idx_i4_i3(-), idx_i4(-) order by i4, i1;

--TEST: mix
select /*+ recompile */ * from ui where i1 > 5 and i2 > 12 using index pk_ui_i1(-), idx_i2(-), idx_i1_i2(-);
select /*+ recompile */ * from ui where i3 in ('aaa', 'ddd') and i4 = '1998-1-1' using index idx_i3(-), idx_i3_i4_i2(-), idx_i4(-), idx_i4_i3(-) order by 1;
select /*+ recompile */ * from ui where i1 < 4 and i2 > 5 and i3 is not null and to_date(i4) > '1995-1-1' using index pk_ui_i1(-), idx_i2(-), idx_i1_i2(-), idx_i3(-), idx_i3_i4_i2(-), idx_i4(-), idx_i4_i3(-), idx_all(-) order by i2;
