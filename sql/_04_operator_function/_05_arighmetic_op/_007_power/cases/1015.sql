--+ holdcas on;
--[er]test power( real, x) with x={numeric}
...
select power(test_real, test_numeric) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_real, test_numeric),'9.99999999EEEE') from test_number order by 1;
