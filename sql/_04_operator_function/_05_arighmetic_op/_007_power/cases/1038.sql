--+ holdcas on;
--[er]test power( float, x) with x={float}
...
select power(test_float, test_float) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_float, test_float),'9.9999999EEEE') from test_number order by 1;
