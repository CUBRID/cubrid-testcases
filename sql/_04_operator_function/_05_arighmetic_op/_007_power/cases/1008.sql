--+ holdcas on;
--[er]test power( double, x) with x={numeric}
...
select power(test_double, test_numeric) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_double, test_numeric),'9.999999999EEEE') from test_number order by 1;
