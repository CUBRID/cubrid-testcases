--test power( numeric, x) with x={smallint, int}
...
select to_char(power(test_numeric, test_smallint),'9.9999999EEEE') from test_number order by 1;
select to_char(power(test_numeric, test_int),'9.9999999EEEE') from test_number order by 1;
