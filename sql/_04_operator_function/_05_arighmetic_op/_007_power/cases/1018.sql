--+ holdcas on;
--[er]test power( numeric, x) with x={float}
...
select power(test_numeric, test_float) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_numeric, test_float),'9.99999999EEEE') from test_number order by 1;
