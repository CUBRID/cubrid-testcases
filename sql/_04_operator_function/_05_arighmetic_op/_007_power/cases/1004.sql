--+ holdcas on;
--[er]test power( float, x) with x={monetary}
...
select power(test_float, test_monetary) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_float, test_monetary),'9.99999999999EEEE') from test_number order by 1;
