--+ holdcas on;
--[er]test power( integer, x) with x={double}
...
select power(test_int, test_double) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_int, test_double),'9.99999999EEEE') from test_number order by 1;
