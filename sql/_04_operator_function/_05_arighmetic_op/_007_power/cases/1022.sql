--+ holdcas on;
--[er]test power( numeric, x) with x={monetary}
...
select power(test_numeric, test_monetary) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_numeric, test_monetary),'9.99999999EEEE') from test_number order by 1;
