--+ holdcas on;
--[er]test power( numeric, x) with x={real}
...
select power(test_numeric, test_real) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_numeric, test_real),'9.99999999EEEE') from test_number order by 1;
