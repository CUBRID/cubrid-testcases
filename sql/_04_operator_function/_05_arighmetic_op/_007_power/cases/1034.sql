--+ holdcas on;
--[er]test power( integer, x) with x={real}
...
select power(test_int, test_real) from test_number order by 1;

set system parameters 'compat_mode=mysql';
select to_char(power(test_int, test_real),'9.9999999EEEE') from test_number order by 1;
