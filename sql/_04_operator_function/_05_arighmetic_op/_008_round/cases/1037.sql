--test round( float, x) with x={smallint, int}
...
select round(test_float, test_smallint) from test_number order by 1;
select round(test_float, test_int) from test_number order by 1;
