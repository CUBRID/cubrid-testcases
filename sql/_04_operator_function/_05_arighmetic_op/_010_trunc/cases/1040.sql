--test trunc(float, x) with x={smallint, int}
...
select trunc(test_float, test_smallint) from test_number order by 1;
select trunc(test_float, test_int) from test_number order by 1;
