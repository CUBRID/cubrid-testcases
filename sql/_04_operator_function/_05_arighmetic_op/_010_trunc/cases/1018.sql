--test trunc(numeric, x) with x={smallint, int}
...
select trunc(test_numeric, test_smallint) from test_number order by 1;
select trunc(test_numeric, test_int) from test_number order by 1;
