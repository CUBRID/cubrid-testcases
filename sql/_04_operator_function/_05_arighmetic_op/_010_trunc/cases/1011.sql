--test trunc(real, x) with x={smallint, int}
...
select trunc(test_real, test_smallint) from test_number order by 1;
select trunc(test_real, test_int) from test_number order by 1;
