--test trunc(integer, x) with x={smallint, int}
...
select trunc(test_int, test_smallint) from test_number order by 1;
select trunc(test_int, test_int) from test_number order by 1;
