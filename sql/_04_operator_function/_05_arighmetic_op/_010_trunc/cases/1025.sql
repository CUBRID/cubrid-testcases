--test trunc(monetary, x) with x={smallint, int}
...
select trunc(test_monetary, test_smallint) from test_number order by 1;
select trunc(test_monetary, test_int) from test_number order by 1;
