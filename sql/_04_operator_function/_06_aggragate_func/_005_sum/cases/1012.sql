--[er]test sum function with 'unique' operator and the data type(like varchar, char, nchar, nchar varying, string)
...
select sum(unique varchartest),sum(unique chartest),sum(unique nchartest), sum(unique ncharvartest) , sum(unique stringtest) from func_09 order by 1;
