--[er]test sum function with 'distinct' operator and the data type(like date, time, timestamp)
...
select sum(distinct datetest), sum(distinct timetest), sum(distinct timestamptest)   from func_09 order by 1;
