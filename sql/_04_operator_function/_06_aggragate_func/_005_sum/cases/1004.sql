--[er]test sum function with 'all' operator and the data type(like varchar, char, nchar, nchar varying, string)
...
select sum(all varchartest),sum(all chartest),sum(all nchartest), sum(all ncharvartest) , sum(all stringtest) from func_09 order by 1;
