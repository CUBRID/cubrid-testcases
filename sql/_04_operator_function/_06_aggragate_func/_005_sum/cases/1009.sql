--[er]test sum function with the data type(like varchar, nchar, char, string)
...
select sum(varchartest),sum(chartest),sum(nchartest), sum(ncharvartest) , sum(stringtest) from func_09 order by 1;
