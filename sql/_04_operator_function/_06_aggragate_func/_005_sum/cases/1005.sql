--[er]test sum function with 'all' operator and the data type(like date, time, timestramp)
...
select sum(all datetest), sum(all timetest), sum(all timestamptest)   from func_09 order by 1;
