--[er]test sum function with 'unique' operator and the data type(like date, time, timestramp)
...
select sum(unique datetest), sum(unique timetest), sum(unique timestamptest)   from func_09 order by 1;
