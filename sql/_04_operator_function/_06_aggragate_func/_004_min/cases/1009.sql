--test min function with 'all' keyword and the data type(like varchar, char, nchar, nchar varying, string, date, time, timestamp)
...
select min(all varchartest),min(all chartest),min(all nchartest), min(all ncharvartest) , min(all stringtest) from func_09;	
