--test max function with 'unique' keyword and the data type(like date, time, timestamp, bit, bit varying)
...
select max(unique datetest), max(unique timetest), max(unique timestamptest)   from func_09 order by 1;

select max(unique bittest), max(unique bitvartest) from func_09 order by 1;
