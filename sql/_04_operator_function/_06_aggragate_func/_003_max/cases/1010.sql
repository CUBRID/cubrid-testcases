--test max function with 'distinct' keyword and the data type(like varchar, char, nchar, nchar varying
...
select max(distinct varchartest),max(distinct chartest),max(distinct nchartest), max(distinct ncharvartest) , max(distinct stringtest) from func_09 order by 1;
