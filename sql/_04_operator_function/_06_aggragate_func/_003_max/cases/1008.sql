--test max function with 'unique' keyword and the data type(like varchar, char, nchar, string)
...
select max(unique varchartest),max(unique chartest),max(unique nchartest), max(unique ncharvartest) , max(unique stringtest) from func_09 order by 1;
