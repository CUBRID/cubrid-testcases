--test max function with varchar,char,nchar types
...
select max(varchartest),max(chartest),max(nchartest), max(ncharvartest) , max(stringtest) from func_09 order by 1;

select max(datetest), max(timetest), max(timestamptest)   from func_09 order by 1;

select max(bittest), max(bitvartest) from func_09 order by 1;
