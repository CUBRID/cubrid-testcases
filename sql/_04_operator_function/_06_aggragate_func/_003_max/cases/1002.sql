--test max function with 'all' keyword and the data type(like vchar, char, nchar varying, string, date, time, timestamp)
...
select max(all varchartest),max(all chartest),max(all nchartest), max(all ncharvartest) , max(all stringtest) from func_09 order by 1;

select max(all datetest), max(all timetest), max(all timestamptest)   from func_09 order by 1;
