--test max function with 'distinct' keyword and the data type(like date, time, timestamp)
...
select max(distinct datetest), max(distinct timetest), max(distinct timestamptest)   from func_09 order by 1;
