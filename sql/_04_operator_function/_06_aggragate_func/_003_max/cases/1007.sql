--test max function with unicque keyword and the data type(like smallint, int, numeric, float, real, double, monetary)
...
select max(unique sinttest), max(unique inttest), max(unique numerictest), max(unique floattest), max(unique realtest), max(unique doublepretest), max(unique moneytest) from func_09 order by 1;
