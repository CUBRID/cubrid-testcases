-- retrieve by function of to_char using parameter of field and using format_argument of "CC" ,'cc' or 'YY'
...
select a,to_char(a),b, to_char(b), c,to_char(c) from  func_04 order by 1,3,5	;
select a,to_char(a),b, to_char(b), c,to_char(c) from  func_04 order by 1,3,5	;
select a,to_char(a,'CC'),b, to_char(b,'CC') from  func_04 order by 1,3	;
select a,to_char(a,'cc'),b, to_char(b,'cc') from  func_04 order by 1,3	;
select a,to_char(a,'YY'),b, to_char(b,'YY') from  func_04 order by 1,3;
