-- create a class,create a class with an object attribute,insert data to the latter class,create index on classes,select data using format  'decr(obj.attr)' and 'decr(attr)' 
...
select b, decr(b) from xoo where c = '1';
select * from xoo order by 1,2;

select d.b , decr(d.b) from xoo  where a = 100;
