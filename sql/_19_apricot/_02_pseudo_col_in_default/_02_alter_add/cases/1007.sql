...
ALTER TABLE tst ADD COLUMN timestamp_col timestamp default unix_timestamp();
...
select if (abs(timestamp_col - UNIX_TIMESTAMP()) <= 86400, 'ok', 'nok') from tst where int_col <> 1 and int_col <> 4;
