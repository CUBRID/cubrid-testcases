...
select /*+ RECOMPILE */ s1,coalesce(s1,'AA') from t1 order by 2,1;

select /*+ RECOMPILE */ s1,coalesce(s1,'AA') from t2 order by 2,1;

select /*+ RECOMPILE */ s1,coalesce(s1,'AA') from t3 order by 2,1;
...
select /*+ RECOMPILE */ * from t2 where 'aa' > coalesce(s1,'0')  order by 1;

select /*+ RECOMPILE */ * from t3 where 'aa' > coalesce(s1,'0')  order by 1;
