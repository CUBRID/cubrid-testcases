...
create unique index u_idx on t(id);
create index idx on t(textlabel, description) where least(id,1000)>char_length(textlabel);
...
drop index idx on t(textlabel, description);
-- try alter table add index --
alter table t add index idx_t_id(id) where id > 10;
alter table t add index idx_t_id_text(textlabel, id) where id > 10;
create index t_textlabel on t(textlabel) where char_length(textlabel) > 3;
...
drop index t_textlabel on t(textlabel);
...
-- all the following queries should return an error --
...
create reverse unique index idx_rev_un on t(id, textlabel) where id > 12;
drop reverse unique index idx_rev_un on t(id, textlabel);
