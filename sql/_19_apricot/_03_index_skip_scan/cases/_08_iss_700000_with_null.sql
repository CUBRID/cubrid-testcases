...
select /*+ recompile  */ count(*) from (select /*+ recompile index_ss NO_MERGE */ i,j,k from t where j between 151 and 199) tt;
...
select /*+ recompile  */ count(*) from (select /*+ recompile index_ss NO_MERGE */ i,j,k from t where j between 2 and 50) tt;
