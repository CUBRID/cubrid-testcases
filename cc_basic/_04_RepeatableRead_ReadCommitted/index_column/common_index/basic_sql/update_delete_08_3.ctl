...
C1's updating the instances intersect with the C2's deleting instances (case 1)
...
/* test case */
C1: UPDATE t1 SET tag = 'X', id = 0 WHERE id >= 5;
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE (tag BETWEEN 'D' AND 'F') or col = 'def';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 5,6,7 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 rows (id=2,4) deleted message should generated once C2 ready, C2 select - id = 2,4 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 5,6,7 are updated, id = 2,4 are deleted */
