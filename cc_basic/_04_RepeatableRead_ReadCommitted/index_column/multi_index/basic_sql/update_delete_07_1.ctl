...
C1 commit, C2 commit
Metrics: schema = single table, index = multiple indexes(PK + simple index), data size = small, where clause = simple
...
/* test case */
C1: UPDATE t1 SET tag = 'X' WHERE id IN (1,3) or col = 'mno';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE tag IN ('D','X');
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 1,3,5 are updated */
...
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 1,3,5 are updated, id = 4 is deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 1,3,5 are updated, id = 4 is deleted */
