...
C1,C2's where clauses are not on index (heap index)
...
2) C1 instances won't be updated since C1 rollback, C2 instances should be deleted
...
/* test case */
C1: UPDATE t1 SET tag = 'X', id = 0 WHERE tag IN ('A','C','E');
MC: wait until C1 ready;
...
MC: wait until C1 ready;
/* expect: C2 select - id = 6 is deleted */
...
C2: commit;
MC: wait until C2 ready;
/* expect: no instance is updated, id = 6 is deleted */
