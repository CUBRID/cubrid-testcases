...
C2: insert into t select t1.* from (select t.* from t order by id) t1, (select sleep(3)=0) x where id%3=0;

C3: select sum(id) from t, (select sleep(10)=0) x group by col order by 1;
