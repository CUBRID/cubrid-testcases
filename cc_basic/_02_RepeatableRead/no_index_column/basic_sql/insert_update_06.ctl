...
NUM_CLIENTS = 2
prepare (1,2,7)
C1: insert into t select rownum,'b' from t where sleep(1,id)=1 limit 3;
...
/* test case */
C1: insert into t select rownum,'b' from (select sleep(1)) x, t limit 3;
MC: wait until C1 ready; 
C2: update t set col='c' where (select sleep(1))=0 and id>0;
MC: sleep 4;
