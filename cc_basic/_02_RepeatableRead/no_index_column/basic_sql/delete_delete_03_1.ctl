...
Priority: 2
Reference case: cc_basic/_02_RepeatableRead/no_index_column/basic_sql/delete_delete_03.ctl
...
C2 rollback before C1 rollback
...
3) All the data affected from C1 and C2 shouldn't be deleted (Visibility Test)
...
MC: wait until C1 ready;
/* expect: 1 row (id=2) deleted message, C2 select - id = 2 is deleted, but id = 3-5 are still existed */
C2: SELECT * FROM t1 order by 1,2;
C2: rollback;
/* expect: C1 select - id = 3-5 are deleted, id = 2 is still existed */
C1: SELECT * FROM t1 order by 1,2;
C1: rollback;
