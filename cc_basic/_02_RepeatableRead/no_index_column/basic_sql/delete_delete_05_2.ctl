...
C2: DELETE FROM t1 WHERE id BETWEEN 1 AND 2;
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 2 is deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 row (id=1) deleted message should generated once C2 ready, C2 select - id = 1 is deleted, id = 2 is still existed */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 1,2 are deleted */
