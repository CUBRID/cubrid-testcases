...
NUM_CLIENTS = 4 
C1: update on table t1: 1, 2, 3. C1 block C2 and C3 on the same object   
C2: update on table t1: 2, 3, 4. (2 is in the second partition t1_p1. Transaction C2 is blocked, waiting for lock on 3 not 2 since t1_p0 is scanned before t1_p1.)
C3: update on table t1: 3, 4, 5.
...
MC: wait until C1 ready;
C2: update t1 set col='bb' where id<5 and id>1;
...
C3: commit;
MC: wait until C3 ready;
/* we can only see C2's update results */
