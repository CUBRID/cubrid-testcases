...
C1 C2 where clauses are not on index (i.e. sequence scan) - (case 2, without using index syntax)
C1 commit, C2 commit
...
C1: commit;
/* expect: 1 row(id=5) updated message, C2 select - id = 5 is updated, but id = 4 is still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 4 is deleted, id = 5 is updated */
