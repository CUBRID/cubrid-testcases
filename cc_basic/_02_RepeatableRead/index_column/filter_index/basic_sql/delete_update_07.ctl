...
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE id >= 4 AND id <= 5 USING INDEX idx_id;
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 3,4 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 row updated message should generated once C2 ready, C2 select - id = 5 is not updated, but id = 3,4 are still visible */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3,4 are deleted, id = 5 is not updated */
