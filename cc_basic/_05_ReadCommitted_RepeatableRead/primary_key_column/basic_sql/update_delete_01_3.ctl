...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id IN (1,2,3) AND col != 'def';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id >= 5 AND id <= 6 AND col != 'mno';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 1,3 are updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: C2 select - id = 6 is deleted, and id = 1,3 are not updated */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 1,3 are updated & id = 6 is deleted */
