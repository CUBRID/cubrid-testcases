...
/* test case */
C1: update t1 set id=4, col='iii' where id=2 and col='def';
C1: select * from t1 order by 1,2;
MC: wait until C1 ready;
C2: update t1 set id=id+1 where id=4 and col like 'i%';
MC: wait until C2 ready;
C1: commit;
MC: wait until C1 ready;
/* we cannot see the update result of C1, can see C2' update result */
C2: select * from t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* we can C1 and C2 update result */
