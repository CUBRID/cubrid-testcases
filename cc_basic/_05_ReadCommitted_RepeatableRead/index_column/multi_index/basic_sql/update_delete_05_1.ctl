...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id = 3 OR id = 4;
MC: wait until C1 ready;
...
C1: commit;
/* expect: 0 row deleted message should generated once C2 ready, C2 select - id = 5 is not deleted, but id = 3,4 are unchanged */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3,4 are updated, id = 5 is not deleted */
