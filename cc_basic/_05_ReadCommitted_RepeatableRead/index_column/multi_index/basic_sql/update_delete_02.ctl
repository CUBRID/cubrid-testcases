...
C1, C2 where clause are on both index column (i.e. index range scan) 
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id >= 1 and id <= 2 AND col != 'def';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id BETWEEN 4 AND 5 AND col = 'jkl';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 1 is updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: C2 select - id = 4 is deleted, but id = 1 is unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 1 is updated & id = 4 is deleted */
