...
C1: CREATE INDEX idx_id on tb1(job);
C1: commit work;
...
C1: INSERT INTO tb1 VALUES(6,'Smith','dev',240);INSERT INTO tb1 VALUES(7,'Brown','account',NULL);
C1: commit;
/* select then delete */
C1: SELECT tb1.* FROM tb1 WHERE job IN {'dev','sal'} ORDER BY id;
MC: wait until C1 ready;
