...
C1 where clause is on index (index scan), C2 where clause is not on index (sequence scan)
...
C2: DELETE FROM t1 WHERE col IN ('abc','mno');
...
/* expect: 0 row deleted message should be generated once C2 ready, C2 select - id = 1,5 are still existed */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 5 are deleted */
