...
/* expect: C1 select - id = 1 is updated, id = 5 is changed */
...
C1: SELECT title, INCR(read_count) FROM t1 WHERE id = 1 order by title; 
C1: commit;
MC: wait until C1 ready;

/* expect: C2 select - id = 5 is updated, id = 1 is unchanged */
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
MC: wait until C2 ready;
