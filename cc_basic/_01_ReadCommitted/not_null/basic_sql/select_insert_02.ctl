...
Test Scenario:
C1 select, C2 insert
C2 completed before C1 finishes execution since C1 takes a long time
...
2) C2 instances should be inserted 
   C1 instances should be selected from its original snapshot (Visibility Test)
...
/* expect: no transactions need to wait, C2 completed before C1 finished */
MC: wait until C2 ready;

/* expect: 1 row inserted message, C2 select - id = 8 is inserted  */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: C1 completed after C2 commit, C1 select - 7 rows are returned */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;
