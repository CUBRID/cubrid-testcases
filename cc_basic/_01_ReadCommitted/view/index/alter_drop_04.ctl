...
C1: ALTER VIEW t1_view ADD QUERY SELECT t1.id, t1.phone, t2.address FROM t1, t2 WHERE t1.id IN (4);
MC: wait until C1 ready;
C2: ALTER VIEW t1_view ADD QUERY SELECT t1.id, t1.phone, t2.address FROM t1, t2 WHERE phone IS NULL and t1.id=t2.id;
MC: wait until C2 blocked;
C1: ALTER VIEW t1_view DROP QUERY 2,3;
C1: SELECT * FROM t1_view order by 1,2,3;
C1: COMMIT;
MC: wait until C2 ready;
C2: ALTER VIEW t1_view DROP QUERY 2,3;
C2: SELECT * FROM t1_view order by 1,2,3;
C2: ALTER VIEW t1_view DROP QUERY 2;
C2: SELECT * FROM t1_view order by 1,2,3;
C2: COMMIT;
MC: wait until C2 ready;
C2: SELECT * FROM t1_view order by 1,2,3;
