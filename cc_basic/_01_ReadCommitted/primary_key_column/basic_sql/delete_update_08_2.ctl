...
C1 where clause is not on pk column (heap scan), C2 where clause is on pk column (index scan)
C2 commit, C1 commit
...
/* test case */
C1: DELETE FROM t1 WHERE tag = 'X' or id = sleep(1); 
C2: UPDATE t1 SET tag = 'X' WHERE id BETWEEN 3 AND 5;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C2 select - id = 3,4,5 are updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: C1 finished execution after C2 commit, 0 row deleted message, C1 select - no instance is deleted */
MC: wait until C2 ready;
MC: wait until C1 ready;
...
/* expect: the instances of id = 3,4,5 are updated, id = no instance is deleted */
