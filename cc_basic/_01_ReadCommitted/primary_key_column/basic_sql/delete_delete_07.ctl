...
Priority: 2
Reference case: cc_basic/_01_ReadCommitted/primary_key_column/basic_sql/delete_delete_06.ctl
Author: Ray
...
2) The instances of C1 will be deleted, the instances of C2 won't be deleted again
...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id > 1 and id < 4;
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 2,3 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 0 row deleted message should generated once C2 ready, C2 select - id = 2,3 are deleted */
