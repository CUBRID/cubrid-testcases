...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE col = 'abc' or col = 'ghi' or col = 'pqr';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE tag = 'A' or tag = 'F';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,3,6,7 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 rows deleted message should generated once C2 ready, C2 select - id = 1,6 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 3,7 are updated, id = 1,6 are deleted */
