...
/* test case */
C1: UPDATE t1 SET col = 'abc' WHERE tag = 'B' or id = 3;
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE col = 'abc' or id = 4;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 2,3 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: C2 select - id = 2,3 are updated, id = 1,4,7 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: DELETE FROM t1 WHERE col = 'abc' or id = 4;
/* expect: C2 select - id = 2,3 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 1,2,3,4,7 are deleted */
