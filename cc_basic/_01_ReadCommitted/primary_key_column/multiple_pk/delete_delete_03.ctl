...
Priority: 2
Reference case: cc_basic/_01_ReadCommitted/primary_key_column/multiple_pk/delete_delete_02.ctl
Author: Ray
...
/* test case */
C1: DELETE FROM t1 WHERE col = 'ghi';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id > 5 or col = 'def' or col = 'abc';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 3 is deleted */
C1: SELECT * FROM t1 order by 1,2;
MC: wait until C1 ready;
/* expect: C2 select - id = 1,2,6,7 are deleted */
C2: SELECT * FROM t1 order by 1,2;
MC: wait until C2 ready;
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 1,2,3,6,7 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: all the instances except id = 4,5 are deleted */
