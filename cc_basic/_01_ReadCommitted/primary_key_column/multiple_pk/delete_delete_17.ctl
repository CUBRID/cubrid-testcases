...
/* test case */
C1: DELETE FROM t1 WHERE id = 1 or id = 4 or col = 'mno';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id >= 5;
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,4,5 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 rows deleted message should generated once C2 ready, C2 select - id = 1,4,5,6,7 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: all the instances except id = 1,4,5,6,7 are deleted */
