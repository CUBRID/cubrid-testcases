...
C1 where clauses is not on index (heap scan), C2 where clause is on index (index scan)
C1 where clause uses single index
...
C1: INSERT INTO t1 VALUES(1,'abc','A'),(2,'def','B'),(3,'ghi','C'),(4,'jkl','D'),(5,'mno','E'),(6,'pqr','F'),(7,'abcd','G');
C1: COMMIT WORK;
...
C1: commit;
MC: wait until C1 ready;
/* expect: 1 row (id=5)deleted message should generated once C2 ready, C2 select - id = 2,3,4,5 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 2,3,4,5 are deleted */
