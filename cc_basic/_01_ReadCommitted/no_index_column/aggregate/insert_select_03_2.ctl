...
C1: set @newincr=0;
C1: insert into t select (@newincr:=@newincr+1),(@newincr)%10 from db_class a,db_class b,db_class c,db_class d limit 10;
C1: commit;
...
C2: insert into t select t.* from (select sleep(5))x,t where id%2=1;
/*MC: wait until C2 ready;*/
MC: sleep 1;

C3: select id=(select max(y.id)>min(y.id) from t y),max(col) from t group by col order by 1,2;
C3: commit work;
