...
MC: wait until C1 ready;
C2: UPDATE t1 SET id = 6, col = 'xyz' WHERE col IN ('def','ghi') and tag != 'B';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 4,5,6 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 row (id=3) updated message should generated once C2 ready, C2 select - id = 3(C2) is updated, id = 4,5,6(C1) are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: id = 4,5,6 are deleted, id = 3 is updated */
