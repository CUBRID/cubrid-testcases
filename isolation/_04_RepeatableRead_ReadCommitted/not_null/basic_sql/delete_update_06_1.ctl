...
Priority: 2
Reference case: cc_basic/_01_ReadCommitted/not_null/basic_sql/delete_update_06.ctl
Author: Ray
...
2) All the C1 instances won't be deleted since C1 rollback
   All the C2 instances will be updated after reevaluation (Visibility/Reevaluation Test)
...
C1: DELETE FROM t1;
MC: wait until C1 ready;
C2: UPDATE t1 SET tag = 'X';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - all instances are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: rollback;
/* expect: 7 rows updated message should generated once C2 ready, C2 select - all instances(C2) are updated */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: all instances are updated, no instance is deleted */
