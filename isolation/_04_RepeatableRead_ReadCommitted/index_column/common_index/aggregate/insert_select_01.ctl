...
C1: commit;
C1: insert into t select rownum,rownum%10 from db_class a,db_class b,db_class c where rownum <= 10000;
C1: commit;
...
/* expected 10 group */
C6: select col,sum(id) from t where (id between 1 and 9000) and (select sleep(3)) = 0 group by col order by col;
...
C6: commit;
C6: select col,sum(id) from t where (id between 1 and 9000) group by col order by col;
C6: commit;
