...
C1: insert into t1 values(1,'abc');insert into t1 values(2,'def');insert into t1 values(3,'abc');insert into t1 values(3,'lmn');insert into t1 values(4,'lmn');
C1: create index idx on t1(id);
C1: commit;
MC: wait until C1 ready;

/* test case */
C1: update t1 set id=2 where id=4;
/* C1 get 3 where col=lmn; C2 get 4 where col=lmn*/
C1: select max(id), col from t1 where id>0 group by col order by 1,2;
MC: wait until C1 ready;
C2: select max(id), col from t1 where id>0 group by col order by 1,2;
