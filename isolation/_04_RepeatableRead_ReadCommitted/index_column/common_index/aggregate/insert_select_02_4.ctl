...
C1: insert into t select rownum,rownum%10 from db_class a,db_class b,db_class c,db_class d where rownum <= 5000;
...
MC: wait until C3 ready;
C6: select distinct col from t where id between 1 and 5000 and (select sleep(2)) = 0 order by 1;
