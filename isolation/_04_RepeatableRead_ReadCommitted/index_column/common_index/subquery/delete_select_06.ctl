...
C2: SELECT * FROM ( SELECT * FROM tb1 WHERE age=(SELECT MAX(age) FROM tb1) ORDER BY id) where rownum <= 1;
MC: wait until C2 ready;

C1: SELECT * FROM ( SELECT * FROM tb1 WHERE age=(SELECT MAX(age) FROM tb1) ORDER BY id) where rownum <= 1;
C1: rollback;
MC: wait until C1 ready; 
