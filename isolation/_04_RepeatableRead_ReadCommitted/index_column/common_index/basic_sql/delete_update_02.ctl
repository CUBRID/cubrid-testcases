...
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'abc' WHERE id = 2 or col = 'mno';
...
MC: wait until C1 ready;
/* expect: C2 select - id = 2,5 are updated */
C2: SELECT * FROM t1 order by 1,2;
MC: wait until C2 ready;
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 1,7 are deleted, id = 2,5 are updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 1,7 are deleted, id = 2,5 are updated */
