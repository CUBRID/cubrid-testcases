...
/* test case */
C1: DELETE FROM t1 WHERE ((id BETWEEN 2 AND 3) or col = 'mno') and sleep(2)=0; 
C2: UPDATE t1 SET col = 'mno' WHERE col IN ('jkl','pqr') ;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C2 select - id = 4,6 are updated */
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
/* expect: C1 finished execution after C2 commit, 3 rows (id=2,3,5)deleted message, C1 select - id = 2,3,5 are deleted */
MC: wait until C1 ready;
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
MC: wait until C1 ready;

/* expect: the instances of id = 4,6 are updated, id = 2,3,5 are deleted */
