...
/* test case */
C1: UPDATE t1 SET id = 7 WHERE id = 5;
C2: DELETE FROM t1 WHERE id = 7 and 0 = (select sleep(2));
/* expect: no transactions need to wait */
MC: wait until C1 ready;
/* expect: C1 select - id = 5 is updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
/* expect: C2 finished execution after C1 commit, 1 row (id=7)deleted message, C2 select - id = (7,'abc','G') is deleted */
