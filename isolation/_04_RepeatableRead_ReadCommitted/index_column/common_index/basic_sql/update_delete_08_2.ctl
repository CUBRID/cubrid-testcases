...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE col = 'def' or tag BETWEEN 'E' AND 'F';
MC: wait until C1 ready;
...
C1: commit;
/* expect: 1 row (id=7) deleted message should generated once C2 ready, C2 select - id = 7 is deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 2,5,6 are updated, id = 7 is deleted */
