...
/* test case */
C1: DELETE FROM t1 WHERE col = 'def' or (tag BETWEEN 'C' AND 'E');
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'def' WHERE tag >= 'E';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 2,3,4,5 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 rows (id = 6,7)updated message should generated once C2 ready, C2 select - id = 6,7 are updated */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 2,3,4,5 are deleted, id = 6,7 are updated */
