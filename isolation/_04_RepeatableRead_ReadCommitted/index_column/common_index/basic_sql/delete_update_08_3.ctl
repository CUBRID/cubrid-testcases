...
/* test case */
C1: DELETE FROM t1 WHERE tag IN ('B','C') and sleep(2)=0; 
...
C2: commit;
/* expect: C1 finished execution after C2 commit, 2 rows (id=2,3)deleted message, C1 select - id = 2,3 are deleted */
MC: wait until C1 ready;
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
MC: wait until C1 ready;

/* expect: the instances of id = 1,7 are updated, id = 2,3 are deleted */
