...
/* test case */
C1: UPDATE t1 SET col = 'ghi' WHERE col = 'def' or tag IN ('D','E');
MC: wait until C1 ready;

C2: DELETE FROM t1 WHERE col = 'ghi' and 0 = (select sleep(2));
...
/* expect: C2 finished execution after C1 commit, 1 row (id=3)deleted message, C2 select - id = 3 is deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 2,4,5 are updated, id = 3 are deleted */
