...
/* test case */
C1: DELETE FROM t1 WHERE id >= 6 or tag = 'B';
MC: wait until C1 ready;

C2: UPDATE t1 SET tag = 'B' WHERE col = 'ghi' or tag = 'E';
/* expect: no transactions need to wait */
MC: wait until C2 ready;

/* expect: C1 select - id = 2,6,7 are deleted */
...
/* expect: C1 select - id = 2,6,7 are deleted, id = 3,5 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;

/* expect: the instances of id = 2,6,7 are deleted, id = 3,5 are updated */
