...
C2: select t.* from (select sleep(1)) x, t where col=1 order by 1,2; --expected can not select C1 inserted data
MC: sleep 1;
*/
...
/* test case */
C1: insert into t values(1,1);
C2: select t.* from t where col=1 and (select sleep(2)) = 0 order by 1,2;
...
C2: select t.* from t where col=1 order by 1,2;
MC: wait until C2 ready;
