...
/* test case */
C1: UPDATE t1 SET id = 8 WHERE tag = 'A';
C2: DELETE FROM t1 WHERE id >= 6 and 0 = (select sleep(4));
/* expect: no transactions need to wait */
MC: wait until C1 ready;
/* expect: C1 select - id = 1 is updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
/* expect: C2 finished execution after C1 commit, 2 rows (id=6,7)deleted message, C2 select - id = 6,7 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 1 is updated, id = 6,7 are deleted */
