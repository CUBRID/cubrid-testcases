...
/* test case */
C1: UPDATE t1 SET col = 'abc', tag = 'A' WHERE id IN (3,6) ;
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE tag IN ('A','B') and 0 = (select sleep(4));
...
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 3,6 are updated, id = 1,2 are deleted */
