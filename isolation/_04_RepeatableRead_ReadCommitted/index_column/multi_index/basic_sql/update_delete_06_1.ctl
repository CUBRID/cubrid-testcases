...
C1 where clause is on index (i.e. index scan), C2 where clause is not no index (heap scan)
C1 commit, C2 commit
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id IN (1,5,6);
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE tag IN ('A','E');
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,5,6 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;
/* expect: 2 rows deleted message should generated once C2 ready, C2 select - id = 1,5 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: id = 6 is updated, id = 1,5 are deleted */
