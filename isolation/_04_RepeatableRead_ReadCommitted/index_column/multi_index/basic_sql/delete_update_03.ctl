...
Metrics: data size = small, index = multiple index(2 unique indexes), where clause = simple, schema = single table
...
/* test case */
C1: DELETE FROM t1 WHERE tag LIKE '%A%' or tag IN ('D','E');
MC: wait until C1 ready;

C2: UPDATE t1 SET tag = 'D' WHERE id = 2 or col = 'stu' ;
...
/* expect: C1 select - id = 1,4,5 are deleted, id = 2,7 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: rollback;
MC: wait until C1 ready;

/* expect: the instances of id = 2,7 are deleted, no instance is deleted */
