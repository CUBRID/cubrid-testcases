...
2) C1 instances won't be deleted since C1 rollback, the C2 instances won't be updated (the duplicate key is not deleted)
   (i.e. the version will be deleted, C2's search condition is satisfied but with duplicate key constraint)
...
/* test case */
C1: DELETE FROM t1 WHERE tag IN ('B','C','E');
MC: wait until C1 ready;
C2: UPDATE t1 SET id = 2, col = 'ghi' WHERE tag = 'A' or tag = 'E';
