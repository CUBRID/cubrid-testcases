...
/* test case */
C1: DELETE FROM t1 WHERE (col IN ('abc','ghi') or tag = 'F') and 0 = (select sleep(6)); 
C2: UPDATE t1 SET tag = 'F' WHERE tag BETWEEN 'D' AND 'E';
...
C2: commit;
MC: wait until C2 ready;
/* expect: C1 finished execution after C2 commit, 3 rows (id=1,3,6)deleted message, C1 select - id = 1,3,6 are deleted */
MC: wait until C1 ready;
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
MC: wait until C1 ready;
/* expect: the instances of id = 4,5 are updated, id = 1,3,6 are deleted */
