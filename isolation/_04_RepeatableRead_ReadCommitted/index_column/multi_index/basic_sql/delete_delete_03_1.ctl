...
Priority: 2
Reference case: cc_basic/_01_ReadCommitted/index_column/multi_index/basic_sql/delete_delete_03.ctl
Author: Ray
...
C2 where clause uses single index
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE tag >= 'E' and tag <= 'F';
MC: wait until C1 ready;

C2: DELETE FROM t1 WHERE col = 'abc' or tag = 'B';
...
/* expect: C2 select - id = 1,2,7 are deleted */
...
/* expect: C2 select - id = 1,2,5,6,7 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 1,2,5,6,7 are deleted */
