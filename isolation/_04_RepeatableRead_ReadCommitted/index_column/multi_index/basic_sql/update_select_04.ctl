...
C1: insert into t1 values(1,'abc');insert into t1 values(2,'def');insert into t1 values(3,'abc');insert into t1 values(1,'def');insert into t1 values(4,'ijk');insert into t1 values(5,'lmn');
...
/* test case */
C1: update t1 set id=2, col='abc' where id=5 and col='lmn';
C1: select * from t1 where id>0 and col not like 'aa' order by 1,2;
MC: wait until C1 ready;
C2: select * from t1 where id>0 and col not like 'aa' order by 1,2;
