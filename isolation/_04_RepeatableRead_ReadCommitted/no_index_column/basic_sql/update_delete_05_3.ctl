...
C1's updating the instances are intersected withthe C2's deleting instances
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id >= 5 or col = 'abc';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE (id <= 5 and id >= 4) or col = 'abc';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,5,6,7 are updated */
...
C2: commit;
/* expect: the instances of id = 1,6,7 are updated, id = 4,5 are deleted */
