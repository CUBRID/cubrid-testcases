...
C1 where clause is on pk (i.e. index scan), whereas C2 where clause is not on pk (i.e. heap scan)
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE tag = 'D' or tag = 'E' or tag = 'B';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 4,5,6,7 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 3 rows deleted message should generated once C2 ready, C2 select - id = 2,4,5 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 6,7 are updated, id = 2,4,5 are deleted */
