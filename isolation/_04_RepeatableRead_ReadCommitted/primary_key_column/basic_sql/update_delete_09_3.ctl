...
C1,C2 where clauses are not on pk (i.e. heap scan)
C1 commit, C2 commit
...
/* test case */
C1: UPDATE t1 SET tag = 'A' WHERE tag BETWEEN 'C' AND 'E';
C2: DELETE FROM t1 WHERE (tag = 'A' or col IN ('def','pqr')) and 0 = (select sleep(6));
/* expect: no transactions need to wait */
MC: wait until C1 ready;
/* expect: C1 select - id = 3,4,5 are updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
/* expect: C2 finished execution after C1 commit, 3 rows (id=1,2,6)deleted message, C2 select - id = 1,2,6 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 3,4,5 are updated, id = 1,2,6 are deleted */
