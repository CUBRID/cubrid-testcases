...
Test Plan: 
Test DELETE/UPDATE locks (X_LOCK on instance) if the instances of the transactions are not overlapped (with pk index)
...
MC: wait until C1 ready;
C2: UPDATE t1 SET id = 7, tag = 'X' WHERE id >= 4 and id <= 5 and tag NOT IN ('E','F','G');
