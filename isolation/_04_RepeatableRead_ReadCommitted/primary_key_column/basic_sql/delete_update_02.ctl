...
C1's where clause is not on pk (i.e. heap scan), C2's where clause is on pk (index scan)
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'abcd' WHERE id = 6;
...
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 1,7 are deleted, id = 6 is updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 1,7 are deleted, id = 6 is updated */
