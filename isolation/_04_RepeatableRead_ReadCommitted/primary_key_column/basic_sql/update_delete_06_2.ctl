...
C1's where clause is on pk (i.e. index scan), C2's where clause is not on pk (i.e. heap scan)
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE tag = 'B' or tag = 'C' or tag = 'D';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 2,3 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 3 rows deleted message should generated once C2 ready, C2 select - id = 2,3,4 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 2,3,4 are deleted */
