...
C1 C2 where clauses are on both indexes (i.e. index range scan)
C1 C2 index scans are not overlapped (C2 at right)
C1 commit, C2 commit
...
C2: DELETE FROM t1 WHERE id BETWEEN 5 AND 6 AND col != 'mno';
...
/* expect: C2 select - id = 6 is deleted, but id = 1 is still visible */
