...
C1, C2 where clauses are on multiple indexes (i.e. index scan)
C1 C2 index scan are not overlapped 
C1 commit, C2 commit
Metrics: schema = single table, index = multiple indexes, data size = small, where clause = simple
...
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE id = 3 AND col = 'ghi';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 2 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: C2 select - id = 3 are updated, but id = 2 is still existed */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 2 is deleted, id = 3 is updated */
