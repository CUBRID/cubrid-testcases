...
C1: insert into t select rownum,rownum ||'a' from (select sleep(2)) x, t where rownum <= 3; --expected blocked
...
/* test case */
C1: insert into t select rownum, rownum ||'a' from t where rownum <= 3 and (select sleep(5)) = 0; 
MC: sleep 1;
