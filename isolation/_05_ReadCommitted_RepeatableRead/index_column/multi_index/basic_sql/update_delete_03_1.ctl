...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE col IN ('ghi','abcd');
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 1,2 are updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 rows (id=3) deleted message, C2 select - id = 3 is deleted (mysql behavior with select) */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3 is deleted, id = 1,2 are updated */
