...
C1 where clause is on index column (i.e. index range scan), C2 where clause is on index column (i.e. index all scan)
i.e. C1 C2 index scans are overlapped
C1 commit, C2 commit
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id >= 2 AND id <= 3 AND col = 'ghi';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id = 4 OR col = 'mno';
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 3 is updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 2 rows (id=4,5) deleted message, C2 select - id = 4,5 are deleted, but id = 3 is unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3 is updated & id = 4,5 are deleted */
