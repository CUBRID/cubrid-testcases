...
/* test case */
C1: DELETE FROM t1 WHERE id >= 2 AND id <= 3 AND col != 'def';
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE id BETWEEN 6 AND 7 AND col != 'abc';
...
C2: commit;
/* expect: C1 select - only id = 3 is deleted, id = 6 is not updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: the instances of id = 3 is deleted, id = 6 is updated */
