...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id BETWEEN 5 AND 6 AND col != 'ghi';
MC: wait until C1 ready;
...
C2: DELETE FROM t1 WHERE id >= 1 AND id <= 2 AND col != 'abc';
...
/* expect: C2 select - id = 2 is deleted, id = 5.6 are unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2,3;
C2: commit;
MC: wait until C2 ready;
