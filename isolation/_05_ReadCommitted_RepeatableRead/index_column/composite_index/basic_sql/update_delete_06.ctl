...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id >= 1 AND id <= 3 AND col = 'def';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id BETWEEN 2 AND 3 AND col = 'abcd';
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 2 is updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 rows deleted message, C1 select - id = 2 is unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 2 are update, no instances is deleted */
