...
C1,C2 where clause are all on composite index column (i.e. index scan)
...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id >= 1 AND id < 3 AND col != 'abc';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 3 is updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 rows (id=2)deleted message, C2 select - id = 2 is deleted, id = 3 is unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3 is updated & id = 2 is deleted */
