...
C2 update do affect C1 delete index scan
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE id >= 1 AND id <= 2 AND col = 'def';
MC: wait until C1 ready;
C2: UPDATE t1 set id = 1 WHERE id = 5 AND col = 'mno';
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 2 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 row (id=5)updated message, C2 select - id = 5 is updated, but id = 2 is still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 2 is deleted, id = 5 is updated */
