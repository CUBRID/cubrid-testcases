...
C1 C2 index scans are not overlapped - case 2 
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE id >= 3 AND id <= 4 AND col = 'jkl';
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE id >= 1 AND id < 3 AND col != 'abc';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 4 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: C2 select - id = 2 is updated, but id = 4 is still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 4 is deleted, id = 2 is updated */
