...
C2 failed since update multiple rows
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: UPDATE t1 set id = 2 WHERE id >= 3 AND id <= 5 AND (col = 'jkl' or col = 'mno');
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 2 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: an unique key violation error message should generated once C2 ready, C2 select - no data is updated, id = 2 is still visible */
