...
C1, C2 where clause are all on index column (i.e. index scan), index scans are not overlapped
C2 commit, C1 commit
...
C2: commit;
/* expect: C1 select - id = 5 is updated, but id = 2,3 are still existed */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: the instances of id = 5 is updated & id = 2,3 are deleted */
