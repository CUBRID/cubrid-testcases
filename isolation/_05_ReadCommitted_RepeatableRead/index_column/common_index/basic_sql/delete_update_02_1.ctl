...
C1, C2 where clause are all on index (i.e. index scan), and index scans are not overlapped
C2 updates an index column 
...
MC: wait until C1 ready;
C2: UPDATE t1 set id = 0 WHERE id = 2;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 6 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: C2 select - id = 2 is updated, but id = 6 is still existed */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 6 is deleted, id = 2 is updated */
