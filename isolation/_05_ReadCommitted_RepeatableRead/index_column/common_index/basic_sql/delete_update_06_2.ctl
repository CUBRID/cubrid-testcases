...
C1 C2 where clauses are on index, C1 range-type search (i.e. mysql range search, C2 at C1 left) 
C1 commit, C2 commit
Metrics: schema = single table, index = single unique index, data size = small, where clause = simple

Test Point:
1) C1 and C2 will not be waiting (Locking Test) - mysql behavior
...
C1: commit;
/* expect: C2 select - id = 2 is updated, id = 3,4 are existed (mysql issue - range left scan) */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3,4 are deleted, id = 2 is updated */
