...
Priority: 1
Reference case: cc_basic/_02_RepeatableRead/index_column/common_index/basic_sql/delete_delete_07.ctl
...
C1,C2 where clauses are on an unique index, all-type search (i.e. index scan)
C1 commit, C2 commit
Metrics: data size = small, index = single unique index, where clause = simple, schema = single table
...
C2: DELETE FROM t1 WHERE id IN (1,3);
...
/* expect: 2 rows (id=1,3) deleted message, C2 select - id = 1,3 are deleted, but id = 4-6 are still existed */
