...
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE col IN ('bcd','pqr');
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 5 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 2 rows (id=1,6)updated message, C2 select - id = 1,6 are updated, but id = 5 is still existed */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 5 is deleted, id = 1,6 are updated */
