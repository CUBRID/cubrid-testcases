...
Priority: 1
Reference case: cc_basic/_02_RepeatableRead/index_column/common_index/basic_sql/delete_delete_01.ctl
...
C1 commit, C2 commit (C1 commits before C2)
Metrics: data size = small, index = single index, where clause = simple, schema = single table

Test Point:
1) C1 and C2 will not be waiting (Locking Test)
2) C1 anb C2 can only see the its own deletion but not other transactions deletion 
...
/* expect: C2 select - only id = 5 is deleted, id = 3 is still existed */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
