...
C1,C2 where clauses are on index, range-type search(i.e. index scan)
C1 commit, C2 commit
...
/* expect: C1 select - id = 5,6 is deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;

/* expect: C2 select - id = 2,3 are deleted, but id = 5,6 are still existed */
