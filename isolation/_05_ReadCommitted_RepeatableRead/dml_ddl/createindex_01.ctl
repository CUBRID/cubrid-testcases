...
C1: INSERT INTO tb_a SELECT rownum,mod(rownum,200) FROM db_class a,db_class b,db_class c,db_class d where rownum<= 50000;
C1: INSERT INTO tb_b SELECT rownum,mod(rownum,200) FROM db_class a,db_class b,db_class c,db_class d where rownum<= 50000;
...
C1: insert into tb_b values(5,'run');insert into tb_b values(100,'test');insert into tb_b values(25,'com');
C1: commit;
MC: wait until C1 ready;
...
MC: wait until C2 ready;
C1: SELECT * FROM db_index where class_name like 'tb_%';
C1: CREATE INDEX idx_all on tb_a(id,col);
