...
Test Scenario:
C1 select(decr), C2 delete, the affected rows are overlapped (based on where clause)
C1 commit, C2 commit
...
/* test case */
C1: SELECT id, title, DECR(read_count) FROM t1 WHERE id = 2;
MC: wait until C1 ready;
C2: DELETE FROM t1 where title = 'book2' or title = 'book6'; 
