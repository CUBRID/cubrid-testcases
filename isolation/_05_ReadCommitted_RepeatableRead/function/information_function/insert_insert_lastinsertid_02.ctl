...
/* test case */
C1: INSERT INTO t1 (col,tag) VALUES('xyz','H'),('abcd','I'),('xxx','J');
MC: wait until C1 ready;
C2: INSERT INTO t1 (col,tag) VALUES('yyy','K');
...
MC: wait until C1 ready;
/* expect: C2 select - last insert id = 11 is selected */
C2: SELECT LAST_INSERT_ID();
C2: SELECT * FROM t1 ORDER BY 1,2;
MC: wait until C2 ready;
C1: INSERT INTO t1 (col,tag) VALUES('zzz','L');
/* expect: C1 select - last insert id = 12 is selected */
C1: SELECT LAST_INSERT_ID();
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: C2 select - last insert id = 11 is selected */
