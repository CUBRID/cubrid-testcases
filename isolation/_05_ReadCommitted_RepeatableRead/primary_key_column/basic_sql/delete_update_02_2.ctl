...
/* test case */
C1: DELETE FROM t1 WHERE col IN ('ghi','pqr'); 
MC: wait until C1 ready;
...
C1: commit;
/* expect: 1 row (id=2) updated message, C2 select - id = 2 is updated, but id = 3,6 are still existed */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 2 is updated, whereas id = 3,6 are deleted */
