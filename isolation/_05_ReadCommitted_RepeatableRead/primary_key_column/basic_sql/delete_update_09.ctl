...
Test Scenario:
C1 delete, C2 update, the affected rows are overlapped (whole table manipulation)
C1 delete all the instances of the table, C2 update all the instances of the table
C1 commit, C2 commit
...
/* expect: C1 select - all the data are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;

/* expect: 0 row updated message should generated once C2 ready, C2 select - no instance is updated, all the data are still existed and unchanged  */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: all the data are deleted */
