...
Priority: 1
Reference case: cc_basic/_02_RepeatableRead/primary_key_column/basic_sql/delete_delete_01.ctl
...
C1,C2 where clauses are only on PK (i.e. a unique search condition on a unique index)
...
/* expect: C2 select - only id = 4 is deleted, id = 2 are still existed */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
