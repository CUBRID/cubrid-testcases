...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE col IN ('ghi','mno');
MC: wait until C1 ready;
...
C1: commit;
/* expect: 1 row (id=6) deleted message, C2 select - id = 6 is deleted, but id = 3,5 are not updated */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3,5 are updated & id = 6 is deleted */
