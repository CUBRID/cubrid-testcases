...
Priority: 2
Reference case: cc_basic/_02_RepeatableRead/primary_key_column/basic_sql/delete_delete_06.ctl
...
C1 C2 where clauses are on PK, range-type search - not overlapped (index range scan)
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE id IN (1,3);
MC: wait until C1 ready;
...
C1: commit;
/* expect: 2 rows (id=4,5) deleted message should generated once C2 ready, C2 select - id = 4,5 are deleted, but id = 1,3 are still existed */
