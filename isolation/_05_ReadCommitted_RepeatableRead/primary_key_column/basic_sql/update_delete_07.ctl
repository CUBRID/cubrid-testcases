...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id IN (3,5);
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 3 are updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 row (id=5) deleted message should generated once C2 ready, C2 select - id = 5 is deleted, id = 3 is still existed and unchanged*/
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 5 is deleted, id = 3 is updated */
