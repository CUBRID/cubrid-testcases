...
If the first transaction commits, the second transaction must be aborted. C1's update results decreases the record number to 0 in C2's where condition.

NUM_CLIENTS = 3
C1: update on table t1; commit.  Update results decreases the record number to 0 in C2's where condition.
C2: update on table t1; abort 
...
/* test case */
C1: update t1 set id=3 where id<3 and col<>'def';
C1: select * from t1 order by 1,2;
MC: wait until C1 ready;
C2: update t1 set col='bb' where id<2 and col='abc';
