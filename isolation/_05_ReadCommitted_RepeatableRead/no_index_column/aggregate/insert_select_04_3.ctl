...
NUM_CLIENTS = 3
C3: select count(t1.id),max(t1.col) from t t1 left outer join (select min(t.col) as col from t where id>5) as t2 on t1.id=t2.col group by t2.col;
*/
...
MC: sleep 1;
C3: select count(t1.id),max(t1.col) from t t1 left outer join (select min(t.col) as col from t where id>5) as t2 on t1.id=t2.col group by t2.col order by 1,2;
MC: sleep 1;
