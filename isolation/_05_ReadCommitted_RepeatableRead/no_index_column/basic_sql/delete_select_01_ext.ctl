...
C1: INSERT INTO tb1 SELECT rownum,mod(rownum,100) FROM db_class a,db_class b, db_class c, db_class d where rownum<=500;
C1: commit;
...
C1: commit;
MC: wait until C1 ready;
C2: SELECT * FROM tb1 where id < 100 ORDER BY id;
...
C1: commit;
MC: wait until C1 ready;
C2: SELECT * FROM tb1  where id between 210 and 300 ORDER BY id;
C2: commit;
MC: wait until C2 ready;
C1: select count(*) from tb1;
C1: commit;
