...
Test Point:
1) C2 C1 don't need to wait (Locking Test)
2) C1 and C2 can only see the its own deletion but not other transactions deletion (Visibility - Phantom Read Test)
...
C1: commit;
/* expect: 1 row (id=5) deleted message, C2 select - id = 5 is deleted, but id = 1 is existed */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 1,5 are deleted */
