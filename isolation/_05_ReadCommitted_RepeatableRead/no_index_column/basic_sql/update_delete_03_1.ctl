...
C1's updating the instances contains the C2's delete instances
C1 commit, C2 commit
...
/* test case */
C1: UPDATE t1 SET id = 0 WHERE id IN (1,4,6);
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE col = 'jkl';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,4,6 are updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 row deleted message should generated once C2 ready,C2 select - id = 4 is not deleted, id = 1,4,6 are unchanged */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of  id = 1,4,6 are updated */
