...
C2: update on table t1; blocked and abort (different with read committed isolation level) 
...
C1: insert into t1 values(1,'abc');insert into t1 values(2,'def');insert into t1 values(3,'gh');insert into t1 values(4,'ijk');insert into t1 values(1,'lmn');insert into t1 values(5,'abc');
C1: commit work;
...
C1: commit;
MC: wait until C1 ready;
/* we cannot see the update result of C1, can see C2's update result */
C2: select * from t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
