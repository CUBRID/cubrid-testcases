...
C1: create table t(id int,col char(10)) partition by range(id)(partition p1 values less than (501),partition p2 values less than (10000));
C1: set @newincr=0;
C1: insert into t select (@newincr:=@newincr+1),'a' from db_class a,db_class b,db_class c limit 5000;
C1: commit work;
C1: delete from t;
C1: commit work;
C1: set @newincr=0;
...
C2: select count(*) from t order by id;
C2: commit;
