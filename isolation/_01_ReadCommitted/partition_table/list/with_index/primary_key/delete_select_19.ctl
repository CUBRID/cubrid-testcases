...
C1: create table t1(t_date date primary key, client varchar(10)) partition by list (YEAR(t_date)) (partition p1 values in (2000,2004,2008), partition p2 values in (2001,2003,2005,2007,2009), partition p3 values in (2002, 2006));
C1: insert into t1 values('2000-01-01','abc'),('2002-02-28','def'),('2004-02-29','abc'),('2005-12-31','gh'),('2008-03-15','def');
C1: commit work;
MC: wait until C1 ready;

/* test case */
C1: delete from t1 where YEAR(t_date) in (2004,2001,2007,2006);
...
C1: select * from t1 partition(p3) where YEAR(t_date)<2010 order by 1,2;
MC: wait until C1 ready;
...
C2: select * from t1 partition(p3) where YEAR(t_date)<2010 order by 1,2;
