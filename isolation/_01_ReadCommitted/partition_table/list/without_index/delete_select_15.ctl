...
C1: create table t1(t_date date, client varchar(10)) partition by list (YEAR(t_date)) (partition p1 values in (2004,2008), partition p2 values in (2001,2002,2003,2005,2006,2007,2009));
...
MC: wait until C1 ready;
C2: select t1.* from (select sleep(10)) x, t1 where YEAR(t_date)>2000 order by 1,2;
MC: sleep 2;
C1: commit;
MC: wait until C1 ready;
