...
C1: insert into t3(valueid, str) SELECT a.id+9, a.col+'xx' FROM t1 a INNER JOIN t2 b ON a.id = b.id WHERE b.id >1 and b.id <7;
MC: wait until C1 ready;
...
C2: COMMIT;
MC: wait until C2 ready;
C1: insert into t3(valueid, str) SELECT a.id+100, a.col+'xx' FROM t1 a INNER JOIN t2 b ON a.id = b.id WHERE b.id >1 and b.id <7;
