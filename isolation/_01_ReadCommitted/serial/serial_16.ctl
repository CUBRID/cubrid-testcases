...
Test Point:
increase up to the maxvalue then other transaction use the serial.
MaxValue:1000, CYCLE:true
...
C1: CREATE SERIAL s1 START WITH 1000 INCREMENT BY 10 MAXVALUE 1500 CYCLE;
C1: commit work;
MC: wait until C1 ready;

/* test case */
C1: INSERT INTO tt1 VALUES(s1.NEXT_VALUE,'Park');
C1: SELECT SERIAL_NEXT_VALUE(s1,49);
C1: INSERT INTO tt1 VALUES(s1.NEXT_VALUE,'Museum');
MC: wait until C1 ready;
C2: INSERT INTO tt1 VALUES(s1.NEXT_VALUE,'Square');
MC: wait until C2 ready;
C1: INSERT INTO tt1 VALUES(s1.NEXT_VALUE,'Studio');
C1: commit work;
C2: commit work;
MC: wait until C1 ready;
MC: wait until C2 ready;
