...
C1: CREATE TABLE t2(id int primary key, address varchar(10)) DONT_REUSE_OID partition by list (id) (partition p0 values in (1,3,5,7), partition p1 values in (2,4,6,8));
...
C2: CREATE OR REPLACE VIEW t2_view_2 AS SELECT * FROM t1 ORDER BY id DESC;
MC: wait until C2 ready;
C1: update t1_view_1 set id=7 where id=5;
MC: wait until C1 ready;
C2: update t2_view_2 set id=6 where id=5;
MC: wait until C2 ready;
