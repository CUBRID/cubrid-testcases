...
C1: CREATE VIEW t1_view AS SELECT t1.id, t1.phone, t2.address FROM t1, t2 WHERE phone IS NOT NULL and t1.id=t2.id WITH CHECK OPTION;
...
C2: COMMIT;
MC: wait until C2 ready;
C2: update t1_view set address='bbbbb' where id=3;
MC: wait until C2 ready;
C1: SELECT * FROM t1_view order by 1,2;
C1: COMMIT;
MC: wait until C1 ready;
