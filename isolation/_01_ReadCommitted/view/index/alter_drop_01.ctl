...
C1: ALTER VIEW t1_view ADD QUERY SELECT t1.id, t1.phone, t2.address FROM t1, t2 WHERE t1.id IN (4,5,7) and phone IS NULL;
...
C1: show tables;
MC: wait until C1 ready;
C2: RENAME VIEW t1_view AS t1_view_temp;
MC: wait until C2 blocked;
C1: COMMIT;
MC: wait until C1 ready;
C2: show tables;
