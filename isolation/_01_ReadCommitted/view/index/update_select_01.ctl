...
C1: CREATE TABLE t1(id INT primary key, phone VARCHAR(10)) DONT_REUSE_OID;
C1: INSERT INTO t1 VALUES(1,'111-1111'), (2,'222-2222'), (3, '333-3333'), (4, NULL), (5, NULL);
C1: CREATE TABLE t2(id int primary key, address varchar(10)) DONT_REUSE_OID;
...
C1: CREATE VIEW t2_view AS SELECT t1.id, t2.address FROM t1, t2 WHERE phone IS NOT NULL and t1.id=t2.id;
C1: commit;


C1: SELECT * FROM t1_view order by 1,2;
C1: update t1_view set id=7 where id=1;
MC: wait until C1 ready;
C2: SELECT * FROM t2_view order by 1,2;
C2: update t2_view set id=7 where id=2;
