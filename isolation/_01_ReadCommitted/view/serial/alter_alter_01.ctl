...
C1: ALTER VIEW t1_view ADD QUERY SELECT t1.id, t1.phone, t2.name FROM t1, t2 WHERE t1.id IN (1,2);
MC: wait until C1 ready;
C2: ALTER VIEW t1_view ADD QUERY SELECT t1.id, t1.phone, t2.name FROM t1, t2 WHERE phone IS NOT NULL;
MC: wait until C2 blocked;
C1: insert into t1 VALUES(6, 'fff', '666-6666');
C1: SELECT * FROM t1_view order by 1,2;
C1: COMMIT;
MC: wait until C2 ready;
C2: insert into t2 VALUES(7, 'fff');
MC: wait until C2 ready;
