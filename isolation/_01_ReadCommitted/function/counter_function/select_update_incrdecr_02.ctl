...
2) C1 instances will be updated(increment) by using INCR function 
   C2 instances will be updated based on its original search condition(snapshot) 
...
/* test case */
C1: SELECT title, INCR(read_count) FROM t1 WHERE id = 4; 
MC: wait until C1 ready;

C2: UPDATE t1 SET read_count = 10 WHERE read_count >= 1 and read_count <= 2;
...
/* expect: C2 select - id = 3,4,6 are updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 3,4,6 are updated */
