...
C1 select does affect C2 select, C2 select does affect C1 select
...
2) C1 instances will be updated(increment) based on its original snapshot
   C2 instances will be updated(decrement) based on its original snapshot 
...
/* test case */
C1: SELECT title, INCR(read_count) FROM t1 WHERE read_count = 1; 
MC: wait until C1 ready;

C2: SELECT title, DECR(read_count) FROM t1 WHERE read_count = 3; 
...
/* expect: C2 select - id = 5,6 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;
