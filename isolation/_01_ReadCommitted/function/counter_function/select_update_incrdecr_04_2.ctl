...
/* test case */
C1: SELECT title, INCR(read_count) FROM t1 WHERE read_count = 2; 
MC: wait until C1 ready;
C2: UPDATE t1 SET read_count = 0 WHERE read_count >= 3 and read_count <= 4 or title = 'book6';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 6 is updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: rollback;
/* expect: 3 rows (id=1,5,6)updated message should be generated once C2 ready, C2 select - id = 1,5,6 are updated*/
MC: wait until C1 ready;
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 1,5,6 are updated */
