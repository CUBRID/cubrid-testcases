...
/* test case */
C1: UPDATE t1 SET read_count = 0 WHERE read_count = 3;
MC: wait until C1 ready;
C2: SELECT INCR(read_count) FROM t1 WHERE read_count = 3;
...
C1: commit;
MC: wait until C1 ready;
/* expect: no instance is updated once C2 ready, C2 select - id = 1 is updated by C1*/
