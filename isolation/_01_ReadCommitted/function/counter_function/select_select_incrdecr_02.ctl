...
2) C1 instances will be updated(decrement) by using INCR function (INCR/DECR is independent of commit/rollback)
   C2 instances will be updated(decrement) by using INCR function 
...
/* test case */
C1: SELECT title, DECR(read_count) FROM t1 WHERE id = 3; 
MC: wait until C1 ready;

C2: SELECT title, DECR(read_count) FROM t1 WHERE title = 'book6'; 
...
/* expect: C2 select - id = 3,6 is updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
