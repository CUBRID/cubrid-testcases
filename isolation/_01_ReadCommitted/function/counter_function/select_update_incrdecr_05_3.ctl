...
C1: CREATE INDEX idx_read on t1(read_count) WHERE read_count BETWEEN 1 and 3;
C1: COMMIT WORK;
MC: wait until C1 ready;

/* test case */
C1: SELECT title, INCR(read_count) FROM t1 WHERE id IN (4) and (select sleep(4)=0)<>0 order by 1; 
...
C2: commit;
/* expect: C1 finish the execution after C2 commit, 1 rows (id=4) updated message, C1 select - id = 4 is updated */
MC: wait until C1 ready;
