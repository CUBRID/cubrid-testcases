...
Metrics: data size = small, index = no index, where clause = complex, schema = single table

Test Point:
1) C2 needs to wait until C1 completed
2) C1 instances will be updated(increment) even if C1 rollback, C2 instances will be updated(increment) based on the reevaluation.
...
/* test case */
C1: SELECT title, INCR(read_count) FROM t1 WHERE id BETWEEN 2 AND 4 and read_count > 0 and title NOT IN ('book3','book6'); 
MC: wait until C1 ready;
C2: SELECT title, INCR(read_count) FROM t1 WHERE id IN (2,5,6) and (read_count BETWEEN 3 AND 10) and title IN ('book2','book4','book7'); 
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 2 is updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
/* expect: C2 select - id = 2 is updated based on C1 updated value (reevaluation, read_count = 7) */
MC: wait until C1 ready;
