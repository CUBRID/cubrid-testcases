...
C1 commit, C2 commit
Metrics: data size = small, index = multiple indexes(PK + Unqiue), where clause = simple, schema = single table
...
/* test case */
C1: SELECT title, DECR(read_count) FROM t1 WHERE read_count = 2; 
MC: wait until C1 ready;

C2: SELECT title, INCR(read_count) FROM t1 WHERE title IN ('book1','book2') and read_count < 4; 
...
/* expect: C2 select - id = 1,6 are updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
