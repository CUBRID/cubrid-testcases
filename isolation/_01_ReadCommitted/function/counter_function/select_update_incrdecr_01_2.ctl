...
2) C1 instances will be updated(increment) since icr/decr is independent with commit/rollback 
...
/* test case */
C1: SELECT title, INCR(read_count) FROM t1 WHERE read_count >= 5 order by 1,2; 
MC: wait until C1 ready;

C2: UPDATE t1 SET read_count = 1 WHERE id = 4 or title = 'book6';
...
C1: rollback;
/* expect: C2 select - id = 2,4,6 are updated */
MC: wait until C1 ready;
