...
MC: wait until C1 ready;
C2: UPDATE t1 SET read_count = 1 WHERE title = 'book4';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 4 is updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
/* expect: 1 row (id=4)updated message should be generated once C2 ready, C2 select - id = 4 is updated based on C1 updated value*/
MC: wait until C1 ready;
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 4 are updated */
