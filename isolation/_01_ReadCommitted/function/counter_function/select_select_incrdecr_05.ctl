...
C1,C2's where clauses are all on index (index scan)
C1 commit, C2 commit
Metrics: data size = small, index = single index(PK), where clause = simple, schema = single table

Test Point:
1) C2 needs to wait until C1 completed
2) C1 instances will be updated(increment), C2 instances will be updated(increment) based on the reevaluation.
...
MC: wait until C1 ready;
C2: SELECT title, INCR(read_count) FROM t1 WHERE id = 1; 
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 1 is updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
/* expect: C2 select - id = 1 is updated based on C1 updated value (reevaluation read_count = 5) */
MC: wait until C1 ready;
