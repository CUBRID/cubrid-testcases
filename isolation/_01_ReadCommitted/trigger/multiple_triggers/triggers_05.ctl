...
NUM_CLIENTS = 2
C1: update tt1 to fire a deferred trigger to insert a row into hi; 
...
C1: CREATE TRIGGER tt1_insert DEFERRED UPDATE ON tt1 EXECUTE INSERT INTO hi VALUES(obj.id,obj.col);
C1: commit work;
MC: wait until C1 ready;

/* test case */
C1: UPDATE tt1 SET col=NULL WHERE id=1; 
MC: wait until C1 ready;
C2: UPDATE tt1 SET col=NULL WHERE id=2;
C2: commit;
