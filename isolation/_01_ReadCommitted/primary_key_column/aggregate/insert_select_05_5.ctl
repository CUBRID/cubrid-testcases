...
/* expected 10 group */
C6: select col,avg(id) from t where id between 1 and 9000 and (select sleep(5)=0)<>0 group by col order by col;
C3: insert into t values(2,'cc');
MC: wait until C3 ready;
C2: commit;
C3: commit;
C4: insert into t select id-12,col from t where id between 4 and 5;
