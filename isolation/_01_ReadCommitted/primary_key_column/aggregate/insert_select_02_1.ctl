...
/* test case */
C1: update t set id=id+10 where id%2=0 and (select sleep(10)=0)<>0;
/*MC: wait until C1 ready;*/
MC: sleep 1;

C2: insert into t select id+10,col from t where id%2=1 and (select sleep(5)=0)<>0;
/*MC: wait until C2 ready;*/
MC: sleep 1;

/*C3: select * from (select distinct avg(id)>id-col+8 from t group by col) t order by 1;*/
C3: select * from (select distinct avg(id)>id from t group by col) t order by 1;
C3: commit work;
