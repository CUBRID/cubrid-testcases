...
/* test case */
C1: update t set id=id+10 where id%2=0;
/*MC: wait until C1 ready;*/
MC: sleep 1;
...
/*C3: select distinct count(id)>id-col+8,max(id)!=789 from (select sleep(5))x ,t group by col;*/
C3: select count(id),max(id) from t group by col order by 1,2;
C3: commit work;
