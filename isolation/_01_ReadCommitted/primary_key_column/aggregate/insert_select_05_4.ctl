...
C5: insert into t select id-9,col from t where id between 2 and 4 and sleep(4)=0;
C1: commit;
C2: insert into t values(1,'aa');

/* expected 10 group */
C6: select distinct col from t, (select sleep(3)) x where id between 1 and 1000 group by id;
MC: wait until C6 ready;
C3: insert into t values(2,'cc');
C2: commit;
C3: commit;
C4: insert into t select id-12,col from t where id between 4 and 5 and sleep(2)=0;
MC: wait until C6 ready;
C4: commit;
C5: commit;
