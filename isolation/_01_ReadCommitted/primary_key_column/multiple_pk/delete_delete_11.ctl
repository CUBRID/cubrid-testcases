...
C1,C2 where clauses are on single pk 
...
2) The instances of C1 won't be deleted since C1 rollback, whereas C2 instances will
   (i.e. the version won't be updated, and the C2 search condition is still satisfied)
...
/* test case */
C1: DELETE FROM t1 WHERE id <= 4 and id >= 2;
MC: wait until C1 ready;
...
C2: commit;
/* expect: all the instances except id = 3,4 are deleted */
