...
/* test case */
C1: DELETE FROM t1 WHERE id >= 6 or col = 'ghi';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id >= 5 or col = 'ghi';
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 3,6,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 row deleted message should generated once C2 ready, C2 select - id = 3,5,6,7 are deleted */
