...
/* test case */
C1: DELETE FROM t1 WHERE col = 'ghi' or col = 'pqr';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id >= 6 or col = 'ghi';
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 3,6 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: rollback;
/* expect: 3 rows deleted message should generated once C2 ready, C2 select - id = 3,6,7 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: all the instances except id = 3,6,7 are deleted */
