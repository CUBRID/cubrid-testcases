...
/* test case */
C1: DELETE FROM t1 WHERE id = 1 or id = 3;
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE col = 'abc' or col = 'ghi' or col = 'mno';
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,3 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: rollback;
/* expect: 1 row deleted message should generated once C2 ready, C2 select - id = 1,3,5 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: all the instances except id = 1,3,5 are deleted */
