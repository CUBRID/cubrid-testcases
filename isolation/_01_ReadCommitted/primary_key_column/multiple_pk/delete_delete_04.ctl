...
2) The instances of C1 will be deleted, whereas C2 instances will not
   (i.e. the version will be updated, the C2 search condition is not satisfied)
...
/* test case */
C1: DELETE FROM t1 WHERE id > 4 or col = 'jkl';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id <= 7 and id >= 3 and col != 'ghi';
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 4,5,6,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 0 row deleted message should generated once C2 ready, C2 select - id = 4,5,6,7 is deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: all the instances except id = 4,5,6,7 are deleted */
