...
Priority: 2
Reference case: cc_basic/_01_ReadCommitted/primary_key_column/multiple_pk/delete_delete_16.ctl
Author: Ray
...
C1 delete instances are partially included in the C2 delete instances
C1 where clause is on single pk, C2 where clause is on multiple pks  
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE col = 'abc' or col = 'jkl';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id <= 5 and col != 'ghi';
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,4,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 rows deleted message should generated once C2 ready, C2 select - id = 1,2,4,5,7 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: all the instances except id = 1,2,4,5,7 are deleted */
