...
C1 where clause is on multiple pks, whereas C2 where clause is on single pk 
...
2) The instances of C1 won't be deleted because of C1 rollback, whereas C2 instances will be deleted
...
/* test case */
C1: DELETE FROM t1 WHERE id > 4 and col != 'mno';
MC: wait until C1 ready;
...
C1: rollback;
/* expect: 1 row deleted message should generated once C2 ready, C2 select - id = 7 is deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: all the instances except id = 7 is deleted */
