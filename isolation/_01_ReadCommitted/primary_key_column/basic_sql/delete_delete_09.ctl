...
/* test case */
C1: DELETE FROM t1 WHERE col = 'abc' or col = 'def';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id = 1 or id = 7;
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,2,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 0 row deleted message should generated once C2 ready, C2 select - id = 1,2,7 are deleted */
