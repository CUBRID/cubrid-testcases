...
C1,C2's where clauses are not on pk (i.e. heap scan)
C1 commit, C2 commit
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE col = 'abc' or tag = 'B' or col = 'jkl';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE col = 'abc' or tag = 'B' or col = 'mno';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,2,4,7 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 rows (id=2,5)deleted message should generated once C2 ready, C2 select - id = 2,5 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 1,4,7 are updated, id = 2,5 are deleted */
