...
C2: DELETE FROM t1 WHERE (id = 8 or id = 2) and 0 = (select sleep(2));
/* expect: no transactions need to wait */
MC: wait until C1 ready;
/* expect: C1 select - id = 1 is updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
/* expect: C2 finished execution after C1 commit, no instance deleted message, C2 select - id = 2 is deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 1 is updated, id = 2 is deleted */
