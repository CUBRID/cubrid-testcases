...
C1's where clause is on pk (i.e. index scan), C2's where clause is not on pk (heap scan)
...
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'abcd', id = 0 WHERE tag = 'B';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 6,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
MC: wait until C1 ready;
/* expect: C2 select - id = 2 is updated */
C2: SELECT * FROM t1 order by 1,2;
MC: wait until C2 ready;
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 2 is updated, id = 6,7 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 6,7 are deleted, id = 2 is updated */
