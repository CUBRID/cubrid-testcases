...
sation is not blocked. And the search condition is not re-evaluate because the first transaction is not commited yet, check the s
econd update is executed correctly.

NUM_CLIENTS = 3
C1: update on table t1; commit
C2: update on table t1;  do not re-evaluate and update
C3: select on table t1, and C3 is used to check the update result. Expect Result: 2,3,5,6
*/
...
C1: insert into t1 values(1,'abc'),(2,'def'),(4,'ijk'),(5,'abc');
C1: commit work;
...
C2: update t1 set id=id+1, col='aa' where id>2;
