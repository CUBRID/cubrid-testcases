...
/* test case */
C1: DELETE FROM t1 WHERE col = 'def' or tag = 'D'; 
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'def' WHERE col = 'abc' or tag = 'E';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 2,4 are deleted */
C1: SELECT * FROM t1 order by 1,2;
MC: wait until C1 ready;
/* expect: C2 select - id = 1,5,7 are updated */
C2: SELECT * FROM t1 order by 1,2;
MC: wait until C2 ready;
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 2,4 are deleted, id = 1,5,7 are updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 2,4 are deleted, id = 1,5,7 are updated */
