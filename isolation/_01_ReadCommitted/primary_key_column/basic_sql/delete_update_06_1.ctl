...
2) All the C1 instances won't be deleted beacuse of C1 rollback, all the C2 instances will updated 
...
/* test case */
C1: DELETE FROM t1 WHERE id > 4 or tag = 'D';
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'abcd' WHERE col = 'mno' or tag = 'G';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 4,5,6,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: rollback;
/* expect: 2 rows updated message should generated once C2 ready, C2 select - id = 5,7 are updated */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: the instances of id = 5,7 are deleted */
