...
C1,C2's where clause are not on pk column (heap scan)
C2 commit, C1 commit
...
/* test case */
C1: DELETE FROM t1 WHERE (tag BETWEEN 'C' AND 'D') and 0 = (select sleep(4)); 
C2: UPDATE t1 SET tag = 'D' WHERE col IN ('abc','pqr') ;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C2 select - id = 1,6,7 are updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: C1 finished execution after C2 commit, 2 rows (id=3,4)deleted message, C1 select - id = 3,4 are deleted */
MC: wait until C1 ready;
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;

/* expect: the instances of id = 1,6,7 are updated, id = 3,4 are deleted */
