...
C1 delete instances are partially included (intersection) the instances from C2 delete 
C1 commit, C2 commit
...
/* test case */
C1: DELETE a,b FROM t1 a RIGHT OUTER JOIN t2 b ON a.id = b.id WHERE a.id <= 4 and a.id >= 3;
MC: wait until C1 ready;
C2: DELETE a,b FROM t1 a RIGHT OUTER JOIN t2 b ON a.id = b.id WHERE b.col = 'abc' or (b.id > 4 and b.id < 8);
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - t1 id = 3,4 are deleted, t2 id = 3,4 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: SELECT * FROM t2 order by 1,2;
C1: commit;
/* expect: 6 rows (id=5,6,7) deleted message should be generated once C2 ready, 
   C2 select - t1 id = 3,4,5,6,7 are deleted, t2 id = 3,4,5,6,7 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: SELECT * FROM t2 order by 1,2;
C2: commit;
/* expect: the instances of t1,t2 id = 3,4,5,6,7 are deleted */
