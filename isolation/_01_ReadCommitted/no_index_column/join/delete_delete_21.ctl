...
2) The data affected from C1 will be deleted,  the overlapped C2 instances won't be deleted but the remained instances will
...
/* test case */
C1: DELETE a,b FROM t1 a CROSS JOIN t2 b WHERE a.col = 'ghi' and b.col = 'abc';
MC: wait until C1 ready;
C2: DELETE a,b FROM t1 a CROSS JOIN t2 b WHERE a.id >= 3 and a.id <=5 and b.id >=4 and b.id <=5;
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - t1 id = 3 is deleted, t2 id = 4 is deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: SELECT * FROM t2 order by 1,2;
C1: commit;
/* expect: 3 rows (t1 id = 4,5; t2 id = 5) deleted message should be generated once C2 ready, 
   C2 select - t1 id = 3,4,5 are deleted, t2 id = 4,5 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: SELECT * FROM t2 order by 1,2;
C2: commit;
/* expect: the instances of t1 id = 3,4,5 are deleted, t2 id = 4,5 are deleted */
