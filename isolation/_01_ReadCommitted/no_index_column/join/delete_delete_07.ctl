...
C1 delete instances include(this case - contain) the instances from C2 delete 
C1 commit, C2 commit
...
/* test case */
C1: DELETE a,b FROM t1 a INNER JOIN t2 b ON a.id = b.id WHERE a.id > 3;
MC: wait until C1 ready;
C2: DELETE a,b FROM t1 a INNER JOIN t2 b ON a.id = b.id WHERE b.id > 5; 
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - t1 id = 4,5,6,7 are deleted, t2 id = 4,5,6,7 are deleted */
