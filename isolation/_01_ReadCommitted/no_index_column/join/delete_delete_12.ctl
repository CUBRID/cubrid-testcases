...
/* test case */
C1: DELETE a,b FROM t1 a LEFT OUTER JOIN t2 b ON a.id = b.id WHERE a.id = 5 or a.col = 'jkl';
MC: wait until C1 ready;
C2: DELETE a,b FROM t1 a LEFT OUTER JOIN t2 b ON a.id = b.id WHERE b.id >= 4 and b.id <= 5;
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - t1 id = 4,5 are deleted, t2 id = 4,5 is deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: SELECT * FROM t2 order by 1,2;
C1: commit;
/* expect: 0 row deleted message should be generated once C2 ready, 
   C2 select - t1 id = 4,5 are deleted, t2 id = 4,5 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: SELECT * FROM t2 order by 1,2;
C2: commit;
/* expect: the instances of t1 id = 4,5 are deleted, t2 id = 4,5 are deleted*/
