...
C1 delete instances intersect with the instances from C2 delete 
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: DELETE a,b FROM t1 a CROSS JOIN t2 b WHERE a.id = 3 and b.id = 5;
...
C1: commit;
/* expect: 0 rows deleted message should be generated once C2 ready, 
   C2 select - t1 id = no instance (C2) is deleted, t2 id no instance is deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: SELECT * FROM t2 order by 1,2;
C2: commit;
/* expect: the instances of t1 id = 3 is deleted, t2 id = 4 is deleted */
