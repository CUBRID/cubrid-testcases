...
/* test case */
C1: SELECT * FROM t1 WHERE id = 1 or col = 'ghi' order by 1,2 FOR UPDATE;
MC: wait until C1 ready;

C2: SELECT * FROM t1 WHERE id = 2 or col = 'jkl' order by 1,2 FOR UPDATE;
...
/* expect: the instances of id = 1,2,4 is update */
