...
Priority: 1
Reference case: /scenario/cc_basic/_01_ReadCommitted/no_index_column/basic_sql/select_select_for_update_01.ctl
...
/* test case */
C1: SELECT * FROM t1 WHERE id = 1 or id = 3 FOR UPDATE;
MC: wait until C1 ready;

C2: SELECT * FROM t1 WHERE id = 2 or id = 5 order by 1,2;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
C1: UPDATE t1 SET col = 'abcd' WHERE id = 1 or id = 3;
MC: wait until C1 ready;
C1: COMMIT;
...
/* expect: the instances of id = 1,3 are update */
