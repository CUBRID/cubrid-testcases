...
Priority: 2
Reference case: cc_basic/_01_ReadCommitted/no_index_column/basic_sql/delete_delete_08.ctl
...
2) All the C1 instances won't deleted, but the C2 instances will (Visibility Test)
   (i.e.the version won't be updated because of C1 rollback, the C2 search condition is satisfied) 
...
C1: rollback;
/* expect: 1 rows deleted message should generated once C2 ready, C2 select - id = 6 is deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: the instances of id = 6 is deleted */
