...
C1: insert into person values('Alicia Florrick',40,'female'), ('Diane Lockhart',52,'female'),('Cary Agos',40,'male'),('Peter Florrick',42,'male'),('Alicia Florrick',30,'female');
C1: commit work;

/* test case */
C1: select * from person where name='Diane Lockhart' order by 1,2,3;
...
C1: select * from person order by 1,2,3;
MC: wait until C1 ready;
C2: select * from person order by 1,2,3;
MC: wait until C2 ready;
C3: select * from person order by 1,2,3;
MC: wait until C3 ready;
C1: commit;
C2: commit;
C3: commit;
...
MC: wait until C2 ready;
C3: select * from person where age<60 order by 1,2,3;
MC: wait until C3 ready;
C1: commit;
C2: commit;
C3: commit;
...
C2: select * from person where name like '%Florrick' order by 1,2,3;
MC: wait until C2 ready;
C3: select * from person where gender='female' order by 1,2,3;
MC: wait until C3 ready;
C1: rollback;
C2: rollback;
C3: rollback;
