...
C1: insert into t1 values(1,'abc'),(2,'def'),(3,'abc'),(4,'def'),(5,'ijk'),(6,'lmn'),(1,'gh'),(1,'abc');
C1: commit;
...
MC: wait until C1 ready;
C2: update t1 set id=6 where id=5;
...
C1: select * from t1 where id>4 order by 1,2;
MC: wait until C1 ready;
C2: update t1 set col='ddd' where col='def';
MC: wait until C2 ready;
C1: commit;
C2: commit;
MC: wait until C1 ready;
