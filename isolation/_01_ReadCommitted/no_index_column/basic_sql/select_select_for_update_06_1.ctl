...
Test Scenario:
C1 select for update, C2 select, the affected rows are overlapped, but the tuple is not existed
C1 commit, C2 commit
...
/* test case */
C1: SELECT * FROM t1 WHERE id = 7 FOR UPDATE;
C1: UPDATE t1 SET id = 7 WHERE id = 1;
MC: wait until C1 ready;

C2: SELECT * FROM t1 WHERE id = 7 FOR UPDATE;
