...
C2: update t set id=id+1,col='c' where sleep(1)=0 and id>0;
*/
...
/* test case */
C1: set @newincr=0;
C1: insert into t select (@newincr:=@newincr+1),(@newincr)||'b' from t, (select sleep(1)) x limit 3; 
MC: wait until C1 ready;
C2: update t set id=id+1,col=col||'c' where sleep(1)=0 and id>0;
