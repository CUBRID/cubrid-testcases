...
2) C1 instances should be deleted, C2 instances shouldn't be updated (since the duplicate key hasn't been deleted)
...
/* test case */
C1: DELETE FROM t1 WHERE id IN (2,3,5,7) and col != 'stu';
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'def' WHERE id IN (6,7) and col != 'abc';
