...
Priority: 2
Reference case: cc_basic/_01_ReadCommitted/primary_key_column/basic_sql/update_update_05.ctl
Author: Mandy
...
C1's update result may affect C2's where condition, but it does not block C2.

NUM_CLIENTS = 3
C1: update on table t1; commit
C2: update on table t1; not block, commit
...
C1: insert into t1 values(1,'abc'),(2,'def'),(3,'abc'),(3,'def'),(4,'ijk'),(5,'abc');
...
MC: wait until C1 ready;
C2: update t1 set col='bb' where id<4 and col='aa';
