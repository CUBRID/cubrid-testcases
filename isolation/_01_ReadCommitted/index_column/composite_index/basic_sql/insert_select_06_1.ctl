...
C1: insert(21);
C1: insert(22);
C1: select * from t where id>0 order by 1; -- all data are visable
C1: rollback work;
C1: select * from t where id>0 order by 1; -- above data are invisable
*/
...
C1: insert into t values(22,'efg');
C1: select * from t where id>0 and col='abc' order by 1;
C1: rollback work;
C1: select * from t where id>0 and col='abc' order by 1;
C1: commit;
