...
C1 where clause uses function index (i.e. index scan), C2 doesn't use function index (heap scan)
C1 commit, C2 commit
Metrics: data size = small, index = single function/filtered index(ABS), where clause = simple, schema = single table
...
/* test case */
C1: UPDATE t1 SET id = 6 WHERE ABS(id) BETWEEN 2 AND 3 USING INDEX idx_id_abs;
MC: wait until C1 ready;
...
/* expect: C2 finishes execution after C1 commit, 1 row (id=6) deleted message should be generated, C2 select - id = 6 is deleted */
