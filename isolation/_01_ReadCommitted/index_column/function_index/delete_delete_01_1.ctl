...
C1 commit, C2 commit
Metrics: data size = small, index = single function index(LOWER) + PK index, where clause = simple, DELETE state = single table deletion
...
C1: INSERT INTO t1 VALUES(1,'a','A'),(2,'bb','b'),(3,'ccc','c'),(4,'dddd','D'),(5,'eeeee','E'),(6,'f','F'),(7,'gggg','g');
C1: CREATE INDEX idx_tag_lower on t1(LOWER(tag));
C1: COMMIT WORK;
MC: wait until C1 ready;

/* test case */
C1: DELETE FROM t1 WHERE LOWER(tag) IN ('a','c');
MC: wait until C1 ready;

C2: DELETE FROM t1 WHERE tag IN ('d','E','F');
...
/* expect: the instances of id = 1,3,5,6 are deleted */
