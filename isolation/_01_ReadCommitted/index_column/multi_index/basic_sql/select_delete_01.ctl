...
C1: set @newincr=0;
C1: INSERT INTO tb1 SELECT (@newincr:=@newincr+1)%2,(@newincr)%50,(@newincr)%100 FROM db_class a,db_class b LIMIT 500;
...
/* test case */
C1: SELECT * FROM tb1 WHERE acol=10 and bcol>50 order by 1,2;
MC: wait until C1 ready;
C2: DELETE FROM tb1 WHERE acol < 2 and bcol > 50;
C2: commit work;
MC: wait until C2 ready;
C1: SELECT * FROM tb1 WHERE acol=10 and bcol>50 order by 1,2;
C1: SELECT * FROM tb1 WHERE acol<2 and bcol>50 order by 1,2;
C1: commit work;
