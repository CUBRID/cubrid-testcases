...
/* test case */
C1: UPDATE t1 SET id = 1 WHERE id = 2 or col = 'ghi';
C2: DELETE FROM t1 WHERE id = 1 and 0 = (select sleep(2));
/* expect: no transactions need to wait */
MC: wait until C1 ready;
/* expect: C1 select - id = 2,3 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: C2 finished execution after C1 commit, 1 row (id=1)deleted message, C2 select - id = 1 is deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 2,3 are updated, id = 1 is deleted */
