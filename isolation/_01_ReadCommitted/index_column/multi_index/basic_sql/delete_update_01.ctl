...
C1 commit, C2 commit
Metrics: data size = small, index = multiple index(2 simple indexes), where clause = simple, schema = single table
...
/* test case */
C1: DELETE FROM t1 WHERE id IN (2,3);
MC: wait until C1 ready;
C2: UPDATE t1 SET tag = 'F', col = 'def' WHERE col = 'jkl';
...
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 2,3 are deleted, id = 4 is updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
