...
/* test case */
C1: UPDATE t1 SET col = 'abc' WHERE tag BETWEEN 'E' AND 'F';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE (col = 'abc' or id = 2) and 0 = (select sleep(3));
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id =  id = 5,6 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: C2 finished execution after C1 commit, 3 rows (id=1,2,7)deleted message, C2 select - id = 1,2,7 are deleted */
MC: wait until C1 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 5,6 are updated, id = 1,2,7 are deleted */
