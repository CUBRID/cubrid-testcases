...
C1 commit, C2 commit
Metrics: schema = single table, index = multiple indexes(2 Unique indexes), data size = small, where clause = simple
...
/* test case */
C1: UPDATE t1 SET id = 0, tag = 'G' WHERE tag = 'B';
C2: DELETE FROM t1 WHERE tag IN ('C','G') and sleep(2)=0;
/* expect: no transactions need to wait */
MC: wait until C1 ready;
/* expect: C1 select - id = 2 is updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: C2 finished execution after C1 commit, 2 rows (id=3,7)deleted message, C2 select - id = 3,7 are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: the instances of id = 3,7 are deleted, id = 2 is updated */
