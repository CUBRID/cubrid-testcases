...
C1: insert into t1 values(1,'abc'),(2,'def'),(3,'ghijk'),(4,'lmn'),(5,'opq'),(5,'rst'),(1,'def');
...
C1: commit;
MC: wait until C1 ready;
C3: select *, sleep(5) from t1 where id>0 and col not like 'cc' order by 1,2;
MC: sleep 6;
C2: update t1 set col='bbb' where id=5 and col='opq';
C2: select * from t1 where id>0 and col not like 'cc' order by 1,2;
C2: commit;
MC: wait until C2 ready;
MC: wait until C3 ready;

C1: select * from t1 where id>0 and col not like 'cc' order by 1,2;
MC: wait until C1 ready;
C2: select * from t1 where id>0 and col not like 'cc' order by 1,2;
MC: wait until C2 ready;
C3: select * from t1 where id>0 and col not like 'cc' order by 1,2;
MC: wait until C3 ready;
