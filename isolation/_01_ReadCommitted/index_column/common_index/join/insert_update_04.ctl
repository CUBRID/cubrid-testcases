...
/* test case */
C1: SELECT * FROM t1 a LEFT JOIN t2 b ON a.id = b.id WHERE b.id >1 and b.id <7 order by 1,2,3,4,5;
MC: wait until C1 ready;
...
C1: insert into t3(valueid, str) SELECT a.valueid, a.col+'xx' FROM t1 a LEFT JOIN t2 b ON a.id = b.id WHERE b.id >1 and b.id <7;
MC: wait until C1 ready;
