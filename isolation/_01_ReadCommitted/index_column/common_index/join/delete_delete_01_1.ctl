...
/* test case */
C1: DELETE a,b FROM t1 a, t2 b WHERE a.id = b.id AND a.id >= 1 AND a.id <= 2;
MC: wait until C1 ready;
C2: DELETE a,b FROM t1 a, t2 b WHERE a.id = b.id AND a.id > 2 AND a.id <= 3;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - t1 id = 1,2 is deleted, t2 id = 1,2 is deleted */
...
MC: wait until C2 ready;
C1: commit;
/* expect: C2 select - t1 id = 1-3 are deleted, t2 id = 1-3 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: SELECT * FROM t2 order by 1,2;
C2: commit;
