...
/* test case */
C1: DELETE a,b FROM tb1 a INNER JOIN tb2 b ON a.id = b.id WHERE a.id in (2,3,4);
MC: wait until C1 ready;
C2: SELECT *  FROM tb1 a INNER JOIN tb2 b ON a.id = b.id WHERE a.id in (2,3,4) order by 1,2;
