...
C1 commit, C2 commit
C1 index scan, C2 sequential scan
...
/* test case */
C1: DELETE a,b FROM t1 a, t2 b WHERE a.id = b.id AND a.id = 1;
MC: wait until C1 ready;

C2: DELETE a,b FROM t1 a, t2 b WHERE a.id = b.id AND (a.id = 2 OR b.id = 3);
