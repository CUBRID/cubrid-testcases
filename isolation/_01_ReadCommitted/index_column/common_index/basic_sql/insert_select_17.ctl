...
C1: set @newincr=0;
C1: insert into t select (@newincr:=@newincr+1),(@newincr) from db_class a,db_class b,db_class c,db_class d limit 100000;
C1: commit;
...
C6: select t.* from (select sleep(1)) x, t where id between 1 and 3 order by id;
