...
/* test case */
C1: DELETE FROM t1 WHERE col = 'jkl' or col = 'mno';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id <= 4 and id >= 2;
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 4,5 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 rows (id=2,3) deleted message should generated once C2 ready, C2 select - id = 2,3,4,5 are deleted */
