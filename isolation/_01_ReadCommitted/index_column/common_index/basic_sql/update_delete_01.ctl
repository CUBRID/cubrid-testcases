...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id = 5 or id = 7;
MC: wait until C1 ready;
...
/* expect: C1 select - id = 5,7 are updated */
C1: SELECT * FROM t1 order by 1,2,3;
MC: wait until C1 ready;

/* expect: C2 select - id = 2,3,4 are deleted */
...
/* expect: C2 select - id = 2,3,4 are deleted, 5,7 are updated */
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 5,7 are updated & id = 2,3,4 are deleted */
