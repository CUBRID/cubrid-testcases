...
C1's updated results do affect a portion of the C2's deleting instances (case 1)
...
/* test case */
C1: UPDATE t1 SET col = 'abcd', tag = 'X' WHERE col = 'abc' or (tag <= 'D' and tag >= 'C');
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE col = 'abcd' or tag > 'G' or id = 6;
MC: wait until C2 ready;
/* expect: no transactions need to wait */
/* expect: C1 select - id = 1,3,4,7 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 1,3,4,7 are updated, id = 6 is deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: rollback;
MC: wait until C2 ready;
/* expect: id = 1,3,4,7 are updated */
