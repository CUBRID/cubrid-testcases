...
Test Scenario:
C1 delete, C2 update, the affected rows are overlapped (whole table execution, no where clause)
C1 commit, C2 commit
Metrics: data size = small, index = single index, where clause = no, schema = single table

Test Point:
1) C2 needs to wait until C1 completed
2) C1 instances should be deleted, C2 instances won't be updated since the search condition is not satisfied
