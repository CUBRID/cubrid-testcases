...
/* test case */
C1: DELETE FROM t1 WHERE id IN (2,4);
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id <= 5 and id >= 4 or col = 'def';
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 2,4 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 row deleted message should generated once C2 ready, C2 select - id = 2,4(C1),5(C2) are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: the instances of id = 2,4,5 are deleted */
