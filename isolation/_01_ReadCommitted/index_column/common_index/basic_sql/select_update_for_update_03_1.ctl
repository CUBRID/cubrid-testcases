...
/* test case */
C1: SELECT * FROM t1 WHERE id >= 1 and id <= 3 FOR UPDATE;
MC: wait until C1 ready;

C2: UPDATE t1 SET col = 'abcd' WHERE id > 2 and id <= 4;
/* expect: C2 needs to wait */
MC: wait until C2 blocked;
C1: UPDATE t1 SET col = 'abcde' WHERE id >= 1 and id <= 3;
MC: wait until C1 ready;
C1: COMMIT;

/* expect: C2 select - id = 1-4 (3-4 abcd, 1-2 abcde) is updated only on id */
