...
/* test case */
C1: DELETE FROM t1 WHERE (col = 'abcd' or col = 'def') and 0 = (select sleep(2)); 
C2: UPDATE t1 SET col = 'abcd' WHERE id IN (1,3,5) ;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C2 select - id = 1,3,5 are updated */
C2: SELECT * FROM t1 order by 1,2,3;
C2: commit;
/* expect: C1 finished execution after C2 commit, 0 row deleted message, C1 select - id = 2 is deleted */
MC: wait until C1 ready;
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
MC: wait until C1 ready;

/* expect: the instances of id = 1,3,5 are updated, id = id = 2 is deleted */
