...
C2: DELETE FROM t1 WHERE (id BETWEEN 1 AND 3) and 0 = (select sleep(6));
/* expect: no transactions need to wait */
MC: wait until C1 ready;
/* expect: C1 select - id = 5,6,7 are updated */
C1: SELECT * FROM t1 order by 1,2,3;
C1: rollback;
/* expect: C2 finished execution after C1 commit, 3 rows (id=1,2,3)deleted message, C2 select - id = 1,2,3 are deleted */
