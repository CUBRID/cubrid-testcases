...
C1: insert into t1 values(1,'abc'),(2,'def'),(3,'abc'),(4,'ijk'),(1,'lmn');
...
MC: wait until C1 ready;
/*  get 1*/
C2: select col,min(id) from t1 where sleep(2)=0 and id>0 group by col;
MC: sleep 2;
C1: commit;
MC: wait until C1 ready;
/* get 2*/
C2: select col,min(id) from t1 where id>0 group by col;
C2: commit;
/* get 2*/
C2: select col,min(id) from t1 where id>0 group by col;
