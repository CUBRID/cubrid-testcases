...
NUM_CLIENTS = 2
prepare(1,3,7)
C1: select t.* from (select sleep(2)) x, t where id>0 using index idx(+) order by 1,2; --expected only can see 1,3,7
...
/* test case */
C1: select t.* from (select sleep(2)) x, t where id>0 using index idx(+) order by 1,2;
MC: wait until C1 ready;
C2: insert into t values(8,'abc');
