...
C1: CREATE INDEX idx_id on t1(id) WHERE id >= 3;
C1: COMMIT WORK;
MC: wait until C1 ready;

/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id = 5 USING INDEX idx_id;
MC: wait until C1 ready;
...
/* expect: C2 select - id = 6 is deleted, id = 5 is updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 6 is deleted, id = 5 is updated */
