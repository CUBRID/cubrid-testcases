...
C1 where clause uses filtered index(index scan - partially), C2 where clause doesn't uses filtered index(heap scan)
...
C1: CREATE INDEX idx_id on t1(id) WHERE id IN (3,5,7);
C1: COMMIT WORK;
MC: wait until C1 ready;

/* test case */
C1: DELETE FROM t1 WHERE id IN (2,3,5) USING INDEX idx_id;
MC: wait until C1 ready;

C2: DELETE FROM t1 WHERE tag ='A' or col ='jkl';
/* expect: no transactions need to wait */
MC: wait until C2 ready;

/* expect: C1 select - id = 3,5 are deleted, since id = 2 is not included in the filtered index */
...
/* expect: C2 select - id = 1,3,4,5 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
