...
C1,C2 where clause uses filtered index (i.e. index scan - partially)
a portion of C1, C2 instances are using index scan
C1 commit, C2 commit
...
C1: CREATE INDEX idx_id on t1(id) WHERE id IN (1,2,6);
C1: COMMIT WORK;
MC: wait until C1 ready;

/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id = 2 or id = 3 USING INDEX idx_id;
MC: wait until C1 ready;

C2: DELETE FROM t1 WHERE id = 1 or id = 4 USING INDEX idx_id;
/* expect: no transactions need to wait */
MC: wait until C2 ready;

/* expect: C1 select - id = 2 is updated, id = 3 is not updated */
C1: SELECT * FROM t1 order by 1,2;
MC: wait until C1 ready;

/* expect: C2 select - id = 1 is deleted, id = 4 is not deleted */
...
/* expect: C2 select - id = 2 is updated, id = 1 is deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: rollback;
MC: wait until C2 ready;

/* expect: the instances of id = 2 are updated, no instance is deleted */
