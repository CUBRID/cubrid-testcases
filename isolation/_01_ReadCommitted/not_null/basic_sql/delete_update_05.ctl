...
C1 commit, C2 commit
Metrics: data size = small, index = single index(PK), where clause = no, schema = single table

Test Point:
1) C2 needs to wait until C1 completed (Locking Test)
2) C1 instances will be deleted
   C2 instance will be updated since duplicate key has been deleted (Visibility/Reevaluation Test)
...
/* test case */
C1: DELETE FROM t1 WHERE id IN (1,4,7);
MC: wait until C1 ready;
C2: UPDATE t1 SET id = 7, col = 'xyz' WHERE tag BETWEEN 'C' AND 'D';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,4,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 row updated message should generated once C2 ready, C2 select - id = 3(C2) is updated, id = 1,4,7(C1) are deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: id = 1,4,7 are deleted, id = 3 is updated */
