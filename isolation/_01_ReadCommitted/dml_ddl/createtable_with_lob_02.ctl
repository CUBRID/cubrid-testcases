...
Test Scenario:
C1 create table, C2 create table, the table name are overlapped

Test Point:
1) C2 should be waiting (Locking Test)
...
MC: wait until C1 ready;
C2: CREATE TABLE t1 (doc_id VARCHAR(64) PRIMARY KEY, content CLOB);
...
/* expect: an error message, C2 show tables - t1 is created */
C2: SHOW TABLES;
C2: commit;
MC: wait until C2 ready;
