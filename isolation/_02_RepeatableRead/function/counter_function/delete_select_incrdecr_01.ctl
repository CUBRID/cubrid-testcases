...
Test Scenario:
C1 delete, C2 select(decr), the affected rows are not overlapped (based on where clause)
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: SELECT DECR(read_count) FROM t1 WHERE id = 3 order by 1;
/* expect: no transactions need to wait*/
MC: wait until C2 ready;
/* expect: C1 select - id = 4,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: C2 select - id = 3 is updated, id = 4,7 are still visible */
