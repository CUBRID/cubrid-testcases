...
C1 where clause is not on index (i.e. sequence scan), whereas C2 is on index (index scan)
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE id >= 3 AND id <= 4 AND col != 'jkl';
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 1,2 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 row (id=3)updated message, C2 select - id = 3 is updated, but id = 1,2 are still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 1,2 are deleted, id = 3 is updated */
