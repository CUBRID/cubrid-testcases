...
C1 where clause is on filter index column (i.e. index scan), C2 is not on index column (sequence scan)
C1 commit, C2 commit
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id = 3 USING INDEX idx_id;
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id = 1 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 3 is updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 row deleted message, C2 select - id = 1 is not deleted, id = 3 is unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3 is updated & id = 1 is not deleted */
