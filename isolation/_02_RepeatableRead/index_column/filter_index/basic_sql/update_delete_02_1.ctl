...
C1 where clause is not on filter index column (i.e. sequence scan), C2 is on index column (index scan)
C1 commit, C2 commit
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id >= 2 and id <= 3 USING INDEX idx_id;
MC: wait until C1 ready;
...
C1: commit;
/* expect: 1 row(id=6) deleted message, C2 select - id = 6 is deleted, id = 3 is unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3 is updated & id = 6 is deleted */
