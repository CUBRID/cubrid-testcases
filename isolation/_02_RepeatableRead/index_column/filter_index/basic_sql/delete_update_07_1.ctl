...
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE id IN (3,5,6) USING INDEX idx_id;
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 5 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 rows updated message should generated once C2 ready, C2 select - id = 3,6 are NOT updated, but id = 5 is still visible */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 5 is deleted, id = 3,6 are not updated */
