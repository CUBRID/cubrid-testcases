...
Priority: 2
Reference case: cc_basic/_02_RepeatableRead/index_column/filter_index/basic_sql/update_delete_01.ctl
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id >= 3 AND id <= 4 USING INDEX idx_id;
MC: wait until C1 ready;
...
MC: wait until C2 ready;
C1: commit;
/* expect: C2 select - id = 5,6 are deleted, id = 3,4 are unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3,4 are updated & id = 5,6 are deleted */
