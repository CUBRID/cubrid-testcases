...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id = 4 OR id = 2 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 4 is deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 0 row deleted message after C2 is ready, C2 select - id = id = 4 is visible */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: the instances of id = 4 are deleted */
