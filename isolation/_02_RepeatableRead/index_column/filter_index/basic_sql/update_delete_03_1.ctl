...
/* test case */
C1: UPDATE t1 SET id = 0 WHERE id >= 4 AND id <= 5 USING INDEX idx_id; 
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id >= 0 AND id <= 1 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 4,5 are updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 row deleted message, C2 select - no instance is deleted, id = 4,5 are unchanged */
