...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id >= 5 AND id <= 6 USING INDEX idx_id;
MC: wait until C1 ready;
...
C1: commit;
/* expect: 1 row(id=3) deleted message, C2 select - id = 3 is deleted, id = 5,6 are unchanged ? */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 5,6 are updated, id = 3 is deleted */
