...
C1 where clause is on index (i.e. index scan), C2 where clause is not on index (sequence scan)
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE id = 4 USING INDEX idx_id;
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE id = 1 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 4 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 row updated message, C2 select - id = 1 is not updated, and id = 4 is still visible */
