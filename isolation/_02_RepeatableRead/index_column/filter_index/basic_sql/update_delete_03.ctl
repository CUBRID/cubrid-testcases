...
/* test case */
C1: UPDATE t1 SET id = 4 WHERE id = 3 USING INDEX idx_id;
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id = 4 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 3 is updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 rows(id = 4) deleted message, C2 select - id = 4 is deleted, id = 3 is unchanged */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3 is updated, id = 4 is deleted */
