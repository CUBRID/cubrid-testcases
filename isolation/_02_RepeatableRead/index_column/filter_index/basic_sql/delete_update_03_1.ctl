...
C1 C2 index scan are overlapped
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'abcd' WHERE id > 5 AND id <= 7 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 4,5 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 2 row(id=6,7) updated message, C2 select - id = 6,7 are updated, id = 4,5 are still visible */
