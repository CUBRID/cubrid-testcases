...
C2 update affect with C1 delete
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE id = 6 USING INDEX idx_id;
MC: wait until C1 ready;
C2: UPDATE t1 set id = 6 WHERE id = 3 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 6 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 row(id=3) updated message, C2 select - id = 3 is updated, but id = 6 is still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 6 is deleted, id = 3 is updated */
