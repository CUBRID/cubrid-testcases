...
Priority: 2
Reference case: cc_basic/_02_RepeatableRead/index_column/filter_index/basic_sql/delete_update_01.ctl
...
C2: UPDATE t1 set col = 'abcd' WHERE id >= 6 USING INDEX idx_id;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 3,4 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit; 
/* expect: C2 select - id = 6,7 are updated, but id = 3,4 are still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3,4 are deleted, id = 6,7 are updated */
