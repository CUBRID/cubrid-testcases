...
C1 where clause is on index, C2 where clause is not on index 
...
MC: wait until C1 ready;
C2: UPDATE t1 set col = 'def' WHERE id = 1 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 2 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: no data updated message, C2 select - no data is updated, id = 2 is still visible */
