...
C1 C2 where clauses are not on index (i.e. index scan)
...
C2: DELETE FROM t1 WHERE id >= 2 AND id <= 3 USING INDEX idx_id;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;

/* expect: C1 select - no instances are deleted */
C1: SELECT * FROM t1 order by 1,2,3;
C1: commit;
MC: wait until C1 ready;

/* expect: 0 row deleted message, C2 select - no instances are deleted */
