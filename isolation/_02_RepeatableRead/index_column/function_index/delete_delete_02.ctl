...
C1 where clause uses index (i.e. index scan), C2 where clause doesn't use index (sequence index) 
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE TRIM(col) = 'a';
/* expect: C2 doesn't need to wait until C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 5 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 row (id=1)deleted message, C2 select - id = 1 is deleted, but id = 5 is still visible */
