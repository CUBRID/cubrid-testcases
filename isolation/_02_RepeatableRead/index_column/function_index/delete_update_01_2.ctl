...
C1 where clause is on function index (i.e. index scan), C2 where clause is on an index but not function index(index scan)
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: UPDATE t1 SET tag = 'X' WHERE id = 6;
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 2,3 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: C2 select - id = 6 is updated, id = 2,3 are still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 2,3 are deleted, id = 6 is updated */
