...
C1 C2 where clause doesn't uses index (sequence scan)
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE DAY(tag) >= 10 AND DAY(tag) <= 20;
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE DAY(tag) <= 2 AND DAY(tag) >= 1;
/* expect: C2 doesn't need to wait until C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 2,6 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 3 rows (id=1,3,5)deleted message, C2 select - id = 1,3,5 are deleted, but id = 2,6 are still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 1,2,3,5,6 are deleted */
