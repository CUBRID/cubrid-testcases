...
/* test case */
C1: DELETE FROM t1 WHERE LENGTH(col) >= 4 and LENGTH(col) <= 5;
MC: wait until C1 ready;
...
/* expect: C2 select - id = 3 is deleted, but id = 4,5,7 are still visible */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 3,4,5,7 are deleted */
