...
MC: wait until C1 ready;
C2: UPDATE t1 SET tag = 'X' WHERE LENGTH(col) >= 2 AND LENGTH(col) < 3;
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 3,4,7 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 row (id=2)updated message, C2 select - id = 2 is updated, id = 3,4,7 are still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3,4,7 are deleted, id = 2 is updated */
