...
C1 where clause uses index (i.e. index scan), C2 where clause doesn't use index (sequence scan) 
C1 commit, C2 commit
...
MC: wait until C1 ready;
C2: UPDATE t1 SET tag = 'X' WHERE col = 'bb';
/* expect: C2 doesn't need to wait until C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 5 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 row (id=2)updated message, C2 select - id = 2 is updated, but id = 5 is still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 5 is deleted, id = 2 is updated */
