...
/* test case */
C1: DELETE FROM t1 WHERE LENGTH(col) BETWEEN 4 AND 5;
MC: wait until C1 ready;
C2: UPDATE t1 set tag = 'X' WHERE LENGTH(col) >= 3 AND LENGTH(col) <= 4;
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 4,5,7 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 row updated message should generated once C2 ready, C2 select - id = 3 is NOT updated, but id = 4,5,7 are still visible */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 4,5,7 is deleted, id = 3 is not updated */
