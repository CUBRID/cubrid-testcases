...
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'aaa' WHERE LENGTH(col) = 1;
/* expect: C2 needs to wait until C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 3 is deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 row (id=1)updated message, C2 select - id = 1 is updated, id = 3 is still visible */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3 is deleted, id = 1 is updated */
