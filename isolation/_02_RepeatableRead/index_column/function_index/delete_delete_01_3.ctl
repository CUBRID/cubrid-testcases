...
C1 C2 where clauses are on function index (i.e. index scan)
C1 commit, C2 commit
Metrics: data size = small, index = function index(LENGTH + TRIM), where clause = simple, schema = single table
...
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE TRIM(col) = 'a' OR TRIM(col) = 'bb';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 4,7 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: C2 select - id = 1,2 are deleted, but id = 4,7 are still visible */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: the instances of id = 1,2,4,7 are deleted */
