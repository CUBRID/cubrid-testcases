...
Test Scenario:
C1 select for update, C2 delete, the affected rows are not overlapped
C1 commit, C2 commit
...
C1: UPDATE t2 SET col = 'abcd' WHERE id = 1;
MC: wait until C1 ready;
C1: COMMIT;

/* expect: C2 select - t1 id = 1 is updated, id = 2 is deleted, t2 id = 1 is not updated */
...
/* expect: the instances of t1, t2 id = 1 are updated, id = 2 is deleted */
