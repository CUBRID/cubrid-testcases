...
/* test case */
C1: SELECT *,sleep(1) FROM tb1 a RIGHT JOIN tb2 b ON a.id = b.id WHERE a.id <= 2 or b.col = 'yzab' order by 1,2 ;
MC: wait until C1 ready;
...
C1: SELECT * FROM tb1 a RIGHT JOIN tb2 b ON a.id = b.id WHERE a.id <= 2 or b.col = 'yzab'  order by 1,2;
C1: commit;
MC: wait until C1 ready;
