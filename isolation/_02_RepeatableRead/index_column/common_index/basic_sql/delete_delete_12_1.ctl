...
Priority: 1
Reference case: cc_basic/_02_RepeatableRead/index_column/common_index/basic_sql/delete_delete_12.ctl
...
Test Scenario:
C1 delete, C2 delete, the affected rows are empty (based on where clause)
C1 where clause is not on PK (sequence scan), C2 where clause is on PK (index scan)
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE col IN ('aaa','ccc');
MC: wait until C1 ready;

C2: DELETE FROM t1 WHERE col = 'bbb';
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;

/* expect: C1 select - all the data are selected */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;

/* expect: 0 row deleted message, C2 select - all the data are selected */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: no data is deleted */
