...
/* test case */
C1: SELECT * FROM t1 WHERE id >= 1 and id <= 2 and id != 2 and id != 3 FOR UPDATE;
MC: wait until C1 ready;
C2: SELECT * FROM t1 WHERE id >= 2 and id <= 3 FOR UPDATE;
...
C2: UPDATE t1 SET col = 'abcd' WHERE id >= 2 and id <= 3;
MC: wait until C2 ready;
C2: commit;
MC: wait until C2 ready;
