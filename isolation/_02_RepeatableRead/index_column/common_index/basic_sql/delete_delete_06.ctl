...
C1,C2 where clauses are, but not only on index (i.e. sequence scan)
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE id = 5 OR col = 'ghi';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id = 2 OR col = 'pqr';
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 3,5 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 rows (id=2,6) deleted message, C2 select - id = 2,6 are deleted, but id = 3,5 are still existed */
