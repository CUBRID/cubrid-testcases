...
/* test case */
C1: UPDATE t1 SET col = 'abc' WHERE id = 2;
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE col = 'abc';
/* expect: C2 deosn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 2 is updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 rows (id=1)deleted message, C2 select - id = 1 is deleted, but id = 2 is still unchanged */
