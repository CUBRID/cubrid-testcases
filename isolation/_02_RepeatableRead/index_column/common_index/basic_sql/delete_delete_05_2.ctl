...
C1,C2 where clause are on index,  range-type search  (i.e. index range scan)
...
C2: DELETE FROM t1 WHERE col IN ('ghi','jkl');
...
/* expect: 2 rows (id=3,4) deleted message, C2 select - id = 3,4 are deleted, but id = 1,7 are still existed */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 1,3,4,7 are deleted */
