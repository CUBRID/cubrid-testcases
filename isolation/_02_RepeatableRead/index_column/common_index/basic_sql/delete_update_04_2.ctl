...
MC: wait until C1 ready;
C2: UPDATE t1 set id = 6 WHERE id = 2;
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 5,6 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 1 row (id=2)updated message, C2 select - id = 2 is updated, but id = 5,6 are still existed */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 5,6 is deleted, id = 2 is updated */
