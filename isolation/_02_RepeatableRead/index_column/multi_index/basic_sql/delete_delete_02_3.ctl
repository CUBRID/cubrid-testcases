...
C1 C2 index scans are overlapped (C1 at right)
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE id BETWEEN 5 AND 6 AND col != 'mno';
C2: DELETE FROM t1 WHERE id >= 1 AND id <= 2 AND col != 'def';
...
C1: commit;
/* expect: C2 select - id = 1 is deleted, but id = 6 is still visible */
