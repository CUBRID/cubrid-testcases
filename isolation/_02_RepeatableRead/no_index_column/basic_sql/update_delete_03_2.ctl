...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id IN (2,3,5);
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE id IN (2,6);
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 2,3,5 are updated */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 0 rows deleted message should generated once C2 ready,C2 select - id = 2,6 are not deleted, id = 2,3,5 are unchanged */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 2,6 are not deleted, id = 2,3,5 are updated */
