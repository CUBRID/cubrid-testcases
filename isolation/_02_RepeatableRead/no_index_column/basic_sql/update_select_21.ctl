...
NUM_CLIENTS = 3
C1: update on table t1, commit before C3 start  
C2: update on table t1, commit/rollback after C3 start  
C3: Select on table t1. Check the select results   
*/
...
/* test case 2, C2 rollback */
C1: update t1 set col='def' where id<4;
C1: commit;
MC: wait until C1 ready;

C2: update t1 set col='ggg' where col='def';
MC: wait until C2 ready;
