...
C2: SELECT SUM(t1.id) FROM tb1 t1 LEFT OUTER JOIN (SELECT MIN(tb1.col) AS col FROM tb1 WHERE id>5 GROUP BY id%10 ) AS t2 ON t1.col=t2.col  GROUP BY t2.col;
*/
...
C1: commit work;
MC: wait until C1 ready;
C2: SELECT SUM(t1.id) FROM tb1 t1 LEFT OUTER JOIN (SELECT MIN(tb1.col) AS col FROM tb1 WHERE id>5 GROUP BY id%10 ) AS t2 ON t1.col=t2.col  GROUP BY t2.col order by 1;
C2: commit;
C2: SELECT SUM(t1.id) FROM tb1 t1 LEFT OUTER JOIN (SELECT MIN(tb1.col) AS col FROM tb1 WHERE id>5 GROUP BY id%10 ) AS t2 ON t1.col=t2.col  GROUP BY t2.col order by 1;
