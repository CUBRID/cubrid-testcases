/*
Test Case: update & delete db_partition 
Priority: 1
Reference case:
Author: Lily
...
C2: ALTER TABLE t1 DROP PARTITION p1;
MC: wait until C2 ready;
C3: select class_name, owner_name, partition_name, partition_class_name, partition_type, partition_expr, partition_values, comment from db_partition order by 1,2,3;
...
C1: select class_name, owner_name, partition_name, partition_class_name, partition_type, partition_expr, partition_values, comment from db_partition order by 1,2,3;
MC: wait until C1 ready;
C2: commit;
MC: wait until C2 ready;
C3: commit;
