...
In this case, where conditions are not overlapped. But the first update result will affect the the second update's where conditi

NUM_CLIENTS = 3
C1: update on table t1; commit. Update results increases the record number in C2's where condition.
...
/* test case */
C1: update t1 set col='abc' where id>2 and col<>'abc';
...
C2: commit;
MC: wait until C2 ready;
/* We cannot see the update result of C1 and C2 */
