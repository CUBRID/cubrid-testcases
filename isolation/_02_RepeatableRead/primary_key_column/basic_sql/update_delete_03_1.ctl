...
/* test case */
C1: UPDATE t1 SET col = 'jkl' WHERE id >= 2 AND id <= 3;
MC: wait until C1 ready;
...
C1: commit;
/* expect: 1 row (id=4) deleted message, C2 select - id = 4 is deleted, id = 2,3 are unchanged */
MC: wait until C2 ready;
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 4 is deleted, id = 2,3 are updated */
