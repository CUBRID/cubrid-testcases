...
C1 C2 where clauses are, but not only on PK (index scan)
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE id = 1 AND col = 'abc';
MC: wait until C1 ready;
...
C1: commit;
/* expect: 1 row (id=5) deleted message should generated once C2 ready, C2 select - id = 5 is deleted, but id = 1 is still existed */
