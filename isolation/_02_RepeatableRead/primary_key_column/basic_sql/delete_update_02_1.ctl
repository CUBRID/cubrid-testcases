...
C1 where clause is not on pk (i.e. sequence scan), C2 where clause is on pk (i.e. index scan)
...
C2: UPDATE t1 SET col = 'abcd' WHERE id IN (3,6);
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 1,7 are deleted */
C1: SELECT * FROM t1 ORDER BY 1,2;
C1: commit;
/* expect: 2 rows (id=3,6) updated message, C2 select - id = 3,5 are updated, but id = 1,7 are still existed */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 3,6 are updated, whereas id = 1,7 are deleted */
