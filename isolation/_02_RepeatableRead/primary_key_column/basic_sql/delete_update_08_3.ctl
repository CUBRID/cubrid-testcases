...
/* test case */
C1: DELETE FROM t1 WHERE col IN ('mno','pqr');
MC: wait until C1 ready;
...
C1: commit;
/* expect: 0 row updated message should generated once C2 ready, C2 select - id = 7 is not updated, id = 5,6 are still existed and unchanged  */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 5,6 are deleted, id = 7 is not updated */
