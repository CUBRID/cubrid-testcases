...
Priority: 2
Reference case: cc_basic/_02_RepeatableRead/primary_key_column/basic_sql/delete_delete_03.ctl
...
C1 where clause is not on PK (sequence scan), C2 where clause is only on PK (index scan)
C1 commit, C2 commit
...
/* test case */
C1: DELETE FROM t1 WHERE col = 'jkl';
MC: wait until C1 ready;
...
C1: commit;
/* expect: 1 row (id=2) deleted message, C2 select - id = 2 is deleted, but id = 4 is still existed */
MC: wait until C2 ready;
