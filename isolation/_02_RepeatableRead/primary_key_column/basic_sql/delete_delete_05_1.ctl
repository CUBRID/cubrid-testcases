...
/* test case */
C1: DELETE FROM t1 WHERE id = 1 OR col = 'abc';
MC: wait until C1 ready;
/* expect: C2 select - all records are selected */
C2: SELECT * FROM t1 order by 1,2;
C2: DELETE FROM t1 WHERE id = 3 OR col = 'ghi';
/* expect: C2 doesn't need to wait once C1 completed */
MC: wait until C2 ready;
/* expect: C1 select - id = 1 is deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 row (id=3) deleted message, C2 select - id = 3 is deleted, but id = 1 is still existed */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
