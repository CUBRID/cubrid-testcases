...
MC: wait until C1 ready;
C2: UPDATE t1 SET col = 'abcd' WHERE col = 'def' or id = 4;
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 2 is deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 0 row updated message should generated once C2 ready, C2 select - id = 4 is not updated, id = 2 is still existed and unchanged  */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 2 is deleted, id = 4 is not updated */
