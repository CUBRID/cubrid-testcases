...
C1, C2 where clause are all on pk (i.e. index scan), C1 range-type search (i.e. index range scan) - case 2 
...
C1: commit;
/* expect: 1 row (id=3) deleted message, C2 select - id = 3 is deleted, but id = 1,2 are not updated */
C2: SELECT * FROM t1 ORDER BY 1,2;
C2: commit;
/* expect: the instances of id = 1,2 is updated & id = 3 is deleted */
