...
MC: wait until C1 ready;
C2: create index idx_t2_3 on t2(ln(class_id)) with online parallel 2;
...
C2: commit;
MC: wait until C2 ready;
C1: select /*+ recompile */ * from t2 where ln(class_id)>=0 using index idx_t2_3(+) order by 1 desc ;
C1: select /*+ recompile */ * from t2 where ln(class_id)>=0 using index none order by 1 desc ;
C1: commit work;
MC: wait until C1 ready;
