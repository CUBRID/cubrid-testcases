...
C1: create table t2(class_id bigint, class_name varchar(10),stu_id int, constraint foreign key(stu_id) references t1(stu_id)) PARTITION BY HASH (stu_id) PARTITIONS 10;
...
MC: wait until C1 ready;
C2: create index idx_t2_3 on t2(mod(stu_id,7)) with online parallel 2;
...
C2: commit;
MC: wait until C2 ready;
C1: select /*+ recompile */ * from t2 where mod(stu_id,7)<=7 using index idx_t2_3(+) order by 1 desc ;
C1: select /*+ recompile */ * from t2 where mod(stu_id,7)<=7 using index none order by 1 desc ;
C1: commit work;
MC: wait until C1 ready;
