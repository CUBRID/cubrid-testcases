...
C3: select CUME_DIST() OVER(order by id,col),CUME_DIST() OVER(PARTITION BY id order by col),DENSE_RANK() OVER(order by id,col),DENSE_RANK() OVER(PARTITION BY id order by col),FIRST_VALUE(col) OVER(PARTITION BY id order by col),LAST_VALUE(col) OVER(PARTITION BY id order by col),LAG (col, 1) OVER(order by id,col),LEAD (col, 1) OVER(order by id,col),NTH_VALUE(col, 2) IGNORE NULLS OVER(PARTITION BY id ORDER BY id NULLS FIRST),NTILE(5) OVER (ORDER BY id,col),PERCENT_RANK() OVER(ORDER BY id,col),PERCENT_RANK() OVER(PARTITION BY col ORDER BY id),RANK() OVER (PARTITION BY col ORDER BY id DESC),ROW_NUMBER() OVER (PARTITION BY col ORDER BY id DESC),STDDEV_POP(id) OVER(PARTITION BY id order by col), STDDEV_SAMP(id) OVER(PARTITION BY col order by id ),VAR_POP(id) OVER(PARTITION BY col order by id),VAR_SAMP(id) OVER(PARTITION BY col order by id) from t order by 1 desc,2,3,4,5,6,7,8 limit 3 ;
