...
MC: wait until C1 ready;
C2: create index idx_t2_3 on t2(abs(stu_id),class_name,class_id) with online parallel 2;
...
C2: commit;
MC: wait until C2 ready;
C1: select /*+ recompile */ * from t2 where abs(stu_id)>=8 using index idx_t2_3(+)  ;
C1: commit work;
MC: wait until C1 ready;
