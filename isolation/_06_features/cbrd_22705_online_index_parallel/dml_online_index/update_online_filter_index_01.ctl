...
MC: wait until C1 ready;
C2: create index idx_t2_3 on t2(class_name,stu_id,class_id) where stu_id<0 and class_name like 'a%' with online parallel 5;
...
C2: commit;
MC: wait until C2 ready;
C1: select /*+ recompile */ *  from t2 where stu_id<0 and class_name like 'a%' using index idx_t2_3 order by 1;
C1: select /*+ recompile */ *  from t2 where stu_id<0 and class_name like 'a%' using index none order by 1;
C1: commit work;
MC: wait until C1 ready;
