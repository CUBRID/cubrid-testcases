...
MC: wait until C1 ready;
C2: create index idx_t2_3 on t2(class_name,stu_id,class_id) where stu_id<0 and class_name not like 'a%' with online parallel 5;
MC: wait until C2 blocked;
C3: alter table t2 add unique(class_name,stu_id,class_id desc);
...
MC: wait until C2 ready;
C1: select /*+ recompile */ *  from t2 where stu_id<0 and class_name not like 'a%' using index idx_t2_3 order by 1;
C1: commit work;
MC: wait until C1 ready;
