...
C2: commit;
MC: wait until C2 ready;
C1: select /*+ recompile */ * from t2 where to_datetime_tz(col) >='2000-12-1 14:00:00' using index idx_t2_3(+) order by 1 desc ;
C1: select /*+ recompile */ * from t2 where to_datetime_tz(col) >='2000-12-1 14:00:00' using index none order by 1 desc ;
C1: commit work;
MC: wait until C1 ready;
