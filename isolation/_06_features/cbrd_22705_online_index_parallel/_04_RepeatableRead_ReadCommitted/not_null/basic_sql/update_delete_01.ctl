...
2) C1 instances should be updated based on its own snapshot
   C2 instances should be deleted based on its own snapshot (Visibility Test)
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id BETWEEN 5 AND 6;
MC: wait until C1 ready;
...
C1: commit;
MC: wait until C1 ready;
/* expect: C2 select - id = 1(C2) is deleted, id = 5,6(C1) are updated */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready; 
/* expect: id = 5,6 are updated, id = 1 is deleted  */
