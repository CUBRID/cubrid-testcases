...
2) C1 instances will be updated
   C2 instances won't be deleted after reevaluation (Visibility/Reevaluation Test) 
...
/* test case */
C1: UPDATE t1 SET col = 'abc' WHERE id >= 4 and id <= 6;
MC: wait until C1 ready;
...
C1: commit;
/* expect: no instance deleted message should generated once C2 ready, C2 select - no instance is deleted(C2), id = 4,5,6(C1) are updated */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: id = 4,5,6 are updated, no instance is deleted */
