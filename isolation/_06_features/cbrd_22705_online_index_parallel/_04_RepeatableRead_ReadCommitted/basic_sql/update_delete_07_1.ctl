...
/* test case */
C1: UPDATE t1 SET col = 'mno' WHERE col = 'abc' or tag = 'F';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE (id >= 3 and id <= 4) or col = 'mno';
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 1,6,7 are updated */
C1: SELECT * FROM t1 order by 1,2;
MC: wait until C1 ready;
/* expect: C2 select - id = 3,4,5 are deleted */
...
C2: commit;
MC: wait until C2 ready;
/* expect: id = 1,6,7 are updated, id = 3,4,5 are deleted */
