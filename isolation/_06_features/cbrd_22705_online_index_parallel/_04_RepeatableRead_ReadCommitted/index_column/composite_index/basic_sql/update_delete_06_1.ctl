...
C1 where clause is on index (i.e. index scan), C2 where clause not on index (i.e. heap scan)
C2 commit, C1 commit
...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id IN (2,6,7) or tag = 'A';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE tag IN ('F','G');
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,2,6,7 are updated */
...
C2: commit;
/* expect: id = 1,2 are updated, id = 6,7 are deleted */
