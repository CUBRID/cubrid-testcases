...
/* test case */
C1: DELETE FROM t1 WHERE tag = 'C' or tag = 'D';
MC: wait until C1 ready;
C2: UPDATE t1 SET id = 0, tag = 'D' WHERE tag IN ('B','C','F');
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 3,4 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 2 row (id=2,6) updated message should be generated once C2 ready, C2 select - id = 2,6 are updated */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 3,4 are deleted, id=2,6 are updated */
