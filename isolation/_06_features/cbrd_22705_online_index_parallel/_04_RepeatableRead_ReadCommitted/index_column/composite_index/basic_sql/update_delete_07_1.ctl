...
/* test case */
C1: UPDATE t1 SET tag = 'X' WHERE id >= 2 and id <= 6 and col != 'ghi';
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE tag IN ('A','C','X');
/* expect: no transactions need to wait */
MC: wait until C2 ready;
/* expect: C1 select - id = 2,4,5,6 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: C2 select - id = 2,4,5,6 are updated, id = 1,3 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: the instances of id = 2,4,5,6 are updated, id = 1,3 are deleted */
