...
/* test case */
C1: UPDATE t1 SET col = 'abc' WHERE tag IN ('E','F');
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE (col IN ('abc') and id = 1) and 0 = (select sleep(2));
/* expect: no transactions need to wait */
/* expect: C1 select - id = 5,6 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: C2 finished execution after C1 commit, 1 rows (id=1)deleted message, C2 select - id = 1 are deleted */
MC: wait until C1 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C1 ready;
/* expect: the instances of id = 5,6 are updated, id = 1 are deleted */
