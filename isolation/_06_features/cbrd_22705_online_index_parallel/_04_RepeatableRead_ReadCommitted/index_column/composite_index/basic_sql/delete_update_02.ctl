...
C2 updates index columns
C2 commit, C1 commit
Metrics: data size = small, index = composite index, where clause = simple, schema = single table

Test Point:
1) C1 and C2 will not be waiting 
2) C2 instances should be updated first, C1 instances should be deleted
...
/* test case */
C1: DELETE FROM t1 WHERE id IN (2,5,6,7) and col != 'abc';
MC: wait until C1 ready;

C2: UPDATE t1 SET id = 2 WHERE tag BETWEEN 'C' and 'D';
...
/* expect: C1 select - id = 2,5,6 are deleted, id = 3,4 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;

/* expect: the instances of id = 2,5,6 are updated & id = 3,4 are deleted */
