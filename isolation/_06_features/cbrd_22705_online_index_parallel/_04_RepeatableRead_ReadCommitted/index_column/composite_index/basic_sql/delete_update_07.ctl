...
/* test case */
C1: DELETE FROM t1 WHERE id = 1 or id IN (2,5) and col != 'mno';
MC: wait until C1 ready;
C2: UPDATE t1 SET id = 2, col = 'abc' WHERE id = 3;
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 1,2 are deleted */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 rows (id = 3)updated message should generated once C2 ready, C2 select - id = 3 is updated */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
/* expect: id = 1,2 are deleted, id = 3 is updated */
