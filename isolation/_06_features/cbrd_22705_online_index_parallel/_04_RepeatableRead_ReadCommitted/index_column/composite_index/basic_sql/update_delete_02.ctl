...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id BETWEEN 2 AND 6 and tag != 'D' and tag != 'E';
MC: wait until C1 ready;
...
/* expect: C1 select - id = 2,3,6 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
MC: wait until C1 ready;

/* expect: C2 select - id = 2,3,6 are updated, id = 1,7 are deleted */
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;

/* expect: the instances of id = 2,3,6 are updated & id = 1,7 are deleted */
