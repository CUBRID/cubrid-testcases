...
/* test case */
C1: UPDATE t1 SET col = 'abcd' WHERE id IN (3,5,6);
MC: wait until C1 ready;
C2: DELETE FROM t1 WHERE col IN ('ghi') or tag = 'F';
/* expect: C2 needs to wait once C1 completed */
MC: wait until C2 blocked;
/* expect: C1 select - id = 3,5,6 are updated */
C1: SELECT * FROM t1 order by 1,2;
C1: commit;
/* expect: 1 row (id=2)deleted message should be generated once C2 ready, C2 select - id = 6 is deleted */
MC: wait until C2 ready;
C2: SELECT * FROM t1 order by 1,2;
C2: commit;
MC: wait until C2 ready;
/* expect: id = 3,5 are updated, id = 6 is deleted */
