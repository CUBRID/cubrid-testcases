...
/* test case 1 */
C1: CREATE INDEX idx1 on tb_a(id,col) with online parallel 2;
C1: CREATE INDEX idx2 on tb_b(id,col) with online parallel 2;
...
MC: wait until C2 ready;
C1: select index_name, is_unique, is_reverse, class_name, owner_name, key_count, is_primary_key, is_foreign_key, filter_expression, have_function, comment, status from db_index where class_name like 'tb_%';
C1: CREATE INDEX idx_all on tb_a(id desc,col desc) with online parallel 3;
MC: wait until C1 blocked;
C2: commit;
MC: wait until C1 ready;
C1: select index_name, is_unique, is_reverse, class_name, owner_name, key_count, is_primary_key, is_foreign_key, filter_expression, have_function, comment, status from db_index where class_name like 'tb_%' order by 1;
